...
export class UsersController {
  ...
  @Get("roles/:id")
  async getRole(@Tenant() tenant: TenantContext, @Param("id") id: string) {
    try {
  ...
  async setDefaultBranch(
    ...
    try {
      ...
      const data = await this.usersService.setDefaultBranch(
        id,
        entityId,
        tenant,
      );
  ...
  @Delete("roles/:id")
  async deleteRole(@Tenant() tenant: TenantContext, @Param("id") id: string) {
    try {
