...
import 'package:zerpai_erp/modules/accountant/models/accountant_metadata_model.dart';
...
import 'package:zerpai_erp/shared/widgets/shortcut_handler.dart';
...
class _ChartOfAccountsCreationPageState
  ...
  AccountNode? _findAccountById(List<AccountNode> nodes, String id) {
    for (final node in nodes) {
      ...
      final child = _findAccountById(node.children, id);
      if (child != null) return child;
    }
    return null;
  }

  void _initializeEditForm(AccountNode account) {
  ...
  void _onSave() async {
    ...
    final requiredAction = _editingAccount == null ? 'create' : 'edit';
    final canSave =
        user != null &&
        PermissionService.hasModuleAction(
          user,
          'chart_of_accounts',
    ...
    if (!canSave) {
      ZerpaiToast.error(
        context,
        'You do not have permission to $requiredAction Chart of Accounts.',
      );
      return;
    }
  ...
  Widget _buildBody(BuildContext context) {
    ...
    final routeAccountId = GoRouterState.of(context).pathParameters['id'];
    final extra = GoRouterState.of(context).extra;
    if (!_didInitFromExtra) {
      if (extra is Map<String, dynamic>) {
        final account = extra['account'];
        if (account is AccountNode) {
          _initializeEditForm(account);
      ...
      if (!_didInitFromExtra &&
          routeAccountId != null &&
          routeAccountId.isNotEmpty) {
        final routeAccount = _findAccountById(
          accountsState.roots,
          routeAccountId,
        );
        if (routeAccount != null) {
          _initializeEditForm(routeAccount);
          _didInitFromExtra = true;
        }
      }

      if (!_didInitFromExtra &&
          (routeAccountId == null || routeAccountId.isEmpty)) {
        _didInitFromExtra = true;
      }
    }
                  ...
                  children: [
                    Tooltip(
                      ...
                      child: ElevatedButton(
                        onPressed: (_onSaveInProgress || !canSave)
                            ? null
                            : _onSave,
      ...
      TextSpan(
        children: [
          TextSpan(
            ...
            style: TextStyle(
              fontSize: 13,
              color: isRequired ? AppTheme.errorRed : AppTheme.textBody,
