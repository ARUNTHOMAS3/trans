...
const double _pricingSchemeCardWidth =
    (__branchPriceListFieldWidth - _pricingSchemeCardGap) / 2;
...
class BranchPriceListAddScreen extends ConsumerStatefulWidget {
  final BranchPriceList? template;
  const BranchPriceListAddScreen({super.key, this.template});

  @override
  ConsumerState<BranchPriceListAddScreen> createState() =>
      _BranchPriceListAddScreenState();
}

class _BranchPriceListAddScreenState
    extends ConsumerState<BranchPriceListAddScreen> {
  static const String _listRoute = AppRoutes.branchPriceLists;
  ...
  final List<Map<String, dynamic>> _availableBranches = [];
  ...
  void initState() {
    ...
    _loadBranchSelectorData();
  }

  Future<void> _loadBranchSelectorData() async {
    try {
      ...
      final branchesRes = results[0];
      final businessTypesRes = results[1];

      setState(() {
        if (branchesRes.success && branchesRes.data is List) {
          _availableBranches
            ..clear()
            ..addAll(
              (branchesRes.data as List)
                  .whereType<Map<String, dynamic>>()
                  .where((branch) => _branchEntityId(branch).isNotEmpty)
                  .toList(),
            );
        }
        if (businessTypesRes.success && businessTypesRes.data is List) {
          _businessTypes
            ..clear()
            ..addAll(
              (businessTypesRes.data as List)
                  .whereType<Map>()
                  .map(
                    (item) => <String, String>{
  ...
  String _branchEntityId(Map<String, dynamic> branch) {
    final direct = (branch['entity_id'] ?? branch['entityId'] ?? '')
        .toString()
        .trim();
    if (direct.isNotEmpty) return direct;

    final nested = branch['entity'];
    if (nested is Map) {
    ...
    if (nested is List && nested.isNotEmpty) {
      ...
      if (first is Map) {
        return (first['id'] ?? '').toString().trim();
      }
    }
    return '';
  }

  String _branchRowId(Map<String, dynamic> branch) =>
      (branch['id'] ?? '').toString().trim();

  Map<String, dynamic>? _findAvailableBranchById(String id) {
    for (final branch in _availableBranches) {
      if (_branchEntityId(branch) == id) return branch;
      if (_branchRowId(branch) == id) return branch;
    }
    return null;
  }

  List<String> _normalizedSelectedBranchEntityIds() {
    final normalized = <String>[];
    for (final selectedId in _associatedBranches) {
      final branch = _findAvailableBranchById(selectedId);
      if (branch != null) {
        final entityId = _branchEntityId(branch);
        if (entityId.isNotEmpty) normalized.add(entityId);
        continue;
      }
      if (selectedId.trim().isNotEmpty) normalized.add(selectedId.trim());
    }
  ...
  List<String> _visibleAssociatedBranchDropdownItems() {
    final selected = _associatedBranches.toSet();
    return _associatedBranchDropdownItems
        .where((id) => !selected.contains(id))
        .toList();
  }

  bool _isFirstAssociatedBranchInGroup(String id) {
    final items = _visibleAssociatedBranchDropdownItems();
  ...
  List<String> _associatedBranchIdsForGroup(String groupLabel) {
    final branchIds = _availableBranches
        .where((branch) => _associatedBranchGroupLabel(branch) == groupLabel)
        .map(_branchEntityId)
  ...
  String _associatedBranchGroupLabel(Map<String, dynamic> branch) {
    return _isMasterBranch(branch)
        ? 'Master Branch'
        : _branchBusinessTypeGroupLabel(branch);
  }
  ...
  List<String> get _associatedBranchDropdownItems {
    ...
    final ordered = <String>[...masterBranches.map(_branchEntityId)];
    ...
    for (final groupLabel in sortedGroupLabels) {
      ...
      ordered.addAll(branches.map(_branchEntityId));
    }

    return ordered.where((id) => id.isNotEmpty).toSet().toList();
  }
      ...
      child: SizedBox(
        ...
        child: FormDropdown<String>(
          ...
          displayStringForValue: (id) {
            final branch = _findAvailableBranchById(id);
            return branch == null ? id : _branchName(branch);
          },
          ...
          itemBuilder: (item, isSelected, isHovered) {
            final branch = _findAvailableBranchById(item);
            final branchName = branch == null ? item : _branchName(branch);
            final showHeader = _isFirstAssociatedBranchInGroup(item);
            final groupLabel = _associatedBranchGroupLabelForId(item);
                    ...
                    child: GestureDetector(
                      onTap: () {
                        final nextValues = <String>{
                          ..._associatedBranches,
                          ..._associatedBranchIdsForGroup(groupLabel),
                        };
                        final orderedValues = _associatedBranchDropdownItems
                            .where(nextValues.contains)
                            .toList();
                        setState(() {
                          _associatedBranches
                            ..clear()
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Text(
                            branchName.isEmpty ? item : branchName,
                            style: TextStyle(
                              fontSize: 13,
                ...
                children: [
                  ...
                  Flexible(
                    child: Column(
                      ...
                      children: [
                        Text(
                          'When a price list is applied, the discount percentage will be applied',
                        ...
                        Text(
                          'only if discount is enabled at the line-item level.',
                          textAlign: TextAlign.center,
                          style: TextStyle(
                            fontSize: 13,
                            height: 1.35,
        ...
        children: [
          ...
          Expanded(
            ...
            child: _buildBulkRateHoverCell(
              ...
              child: _buildCustomRateCell(itemId, baseRate: baseRate),
            ),
          ),
          if (_isDiscountEnabled) ...[
            _buildTableDivider(),
            Expanded(
              ...
              child: _buildBulkRateHoverCell(
                ...
                child: _buildDiscountCell(itemId, baseRate: baseRate),
              ),
            ),
          ],
        ],
              ...
              child: Column(
                children: [
                  for (
                    ...
                    _buildVolumeRangeLine(itemId, rangeIndex, baseRate),
  ...
  Widget _buildVolumeRangeLine(String itemId, int rangeIndex, double baseRate) {
    return Container(
      ...
      children: [
        Expanded(
          child: TextFormField(
            ...
            onChanged: (value) {
              if (!_isDiscountEnabled || baseRate == null || baseRate <= 0)
                return;
              final customRate = _parseDecimalInput(value.trim());
              if (customRate == null) return;
              final discount = ((baseRate - customRate) / baseRate) * 100;
              final discountController = _discountControllerFor(itemId);
              final formatted = _formatInputDecimal(discount);
              if (discountController.text != formatted) {
                discountController.text = formatted;
              }
            },
          ),
        ),
  ...
  Widget _buildDiscountCell(String itemId, {double? baseRate}) {
    return Row(
      children: [
        Expanded(
          child: TextFormField(
            ...
            onChanged: (value) {
              ...
              final discount = _parseDecimalInput(value.trim());
              if (discount == null) return;
              final customRate = (baseRate * (1 - (discount / 100)))
                  .clamp(0, double.infinity)
                  .toDouble();
              final customRateController = _customRateControllerFor(itemId);
              final formatted = _formatInputDecimal(customRate);
              if (customRateController.text != formatted) {
                customRateController.text = formatted;
              }
            },
          ),
        ),
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final normalizedBranchEntityIds = _normalizedSelectedBranchEntityIds();
      if (_associatedBranches.isNotEmpty && normalizedBranchEntityIds.isEmpty) {
        throw Exception(
          'Selected branches could not be mapped to branch entities. Refresh branch list and reselect branches.',
        );
      }
      ...
      await ref
          ...
          .createBranchPriceList(
            branchPriceList,
            branchEntityIds: normalizedBranchEntityIds,
          );
      ...
      if (mounted) {
        ...
        ZerpaiToast.error(
          context,
          'Unable to save price list: ${errorMessage.isEmpty ? 'Please try again.' : errorMessage}',
        );
      }
    } finally {
      if (mounted) {
  ...
  List<BranchPriceListItemRate> _buildItemRates(List<Item> items) {
    ...
    for (var index = 0; index < items.length; index++) {
      ...
      final discount = _isDiscountEnabled
          ? _parseDecimalInput(discountText)
          : null;
    ...
    setState(() {
      for (var cursor = index; cursor < count - 1; cursor++) {
        ...
        _moveVolumeController(
          ...
          from: _volumeDiscountKey(itemId, cursor + 1),
          to: _volumeDiscountKey(itemId, cursor),
        );
      }
  ...
  void _disposeVolumeRangeControllers(String itemId, int index) {
    ...
    _discountControllers.remove(_volumeDiscountKey(itemId, index))?.dispose();
  }
