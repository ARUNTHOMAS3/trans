...
flowchart LR
    ...
    MW -->|reads header| ENT[x-entity-id]
    ENT --> CTX[TenantContext]
    CTX --> SVC[Service Layer via @Tenant() decorator]
    SVC -->|global tables - NO entity_id filter| GLOBAL[(global lookup tables)]
