...
> **Source**: `current schema.txt` (PostgreSQL DDL dump, 84 base tables, extracted 2026-04-03)
...
| Sales | sales_orders, sales_order_items, sales_order_attachments, sales_payments, sales_payment_links, sales_eway_bills |
| Customers | customers, customer_contact_persons |
| Purchases / Vendors | purchases_purchase_orders, purchases_purchase_order_items, purchases_purchase_order_attachments, vendors, vendor_contact_persons, vendor_bank_accounts, item_vendor_mappings |
...
| Settings, Org & Users | organization, users, settings_roles, settings_branding, settings_branches, settings_branch_users, settings_branch_transaction_series, settings_user_location_access, settings_transaction_series, transactional_sequences, settings_email_templates |
| Kerala LSGD (Local Govt) | settings_districts, settings_local_bodies, settings_wards, settings_lsgd_seed_stage |
...
### `organization`
**What it is**: The root of multi-tenancy. Every org-scoped table links back here via `org_id`. Created when a business signs up.
**Why it exists**: The central legal entity profile. Stores identity, banking, compliance (Drug/FSSAI/MSME), and default localization settings.
**Key data collected**:
- `name`, `slug` (unique URL-safe identifier), `system_id` — core branding
- `industry`, `logo_url` — classification
- Financial: `base_currency`, `fiscal_year`, `timezone`, `date_format`, `date_separator`
- Compliance:
    - **Drug License**: `is_drug_registered`, `drug_licence_type`, `drug_license_20`, `drug_license_21`, `drug_license_20B`, `drug_license_21B`
    - **FSSAI**: `is_fssai_registered`, `fssai_number`
    - **MSME**: `is_msme_registered`, `msme_registration_type`, `msme_number`
- Payment Stub:
    - `has_separate_payment_stub_address`
    - `payment_stub_attention`, `payment_stub_street_1`, `payment_stub_street_2`, `payment_stub_city`, `payment_stub_pincode`, `payment_stub_phone`, `payment_stub_fax`
    - **Kerala LSGD**: `payment_stub_district_id`, `payment_stub_local_body_id`, `payment_stub_ward_id`
...
**Why it exists**: Businesses with multiple outlets need independent profiles for GST, address, and local government (LSGD) hierarchy.
**Key data collected**:
- `name`, `branch_code`, `system_id`
- `branch_type` — FOFO / COCO / FICO / FOCO
- `gstin` + Registration details
- `district_id`, `local_body_id`, `ward_id` — LSGD hierarchy (Kerala)
- `parent_branch_id` — for hierarchical warehouse models
...
**Why it exists**: Each user belongs to an organization (`org_id`) and is assigned a base `role`.
...
**Key data collected**: `label`, `permissions` (jsonb).
...
### `settings_branch_transaction_series`
**What it is**: Mapping transaction series to branches.
**Why it exists**: Controls which invoice/order series are available in which branch.
...
| Table | Has org_id | Has branch/outlet | Notes |
|-------|-----------|---------------|-------|
...
| All `purchases_*` tables | ✅ | ✅ | Mandatory multi-tenancy |
| All `sales_*` tables | ✅ | ✅ | Mandatory multi-tenancy |
...
| `transactional_sequences` | ❌ | ✅ | Per-outlet/branch sequences |
| `warehouses` | ✅ | ✅ | Per-org warehouses |
| `settings_branches` | ✅ (org_id col) | — | The branch definition itself |
| `users` | ✅ | ❌ | Org-scoped users |
...
*Last updated: 2026-04-03 | Source: `current schema.md` (84 base tables)*
