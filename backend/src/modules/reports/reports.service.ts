...
export class ReportsService {
  ...
  private async resolveDashboardEntityScope(tenant: TenantContext): Promise<{
    ...
    if (!currentEntityId) {
      return { entityIds: [], scopeLabel: "No entity selected", entityBreakdown: [] };
    }
    ...
    const { data: selectedEntity } = await supabase
      ...
      .maybeSingle();
    const entityRows: Array<{ id: string; type: "ORG" | "BRANCH"; ref_id: string; parent_id: string | null }> =
    ...
    const entityBreakdown = entityRows.map((row) => {
      if (row.type === "ORG") {
        const meta = orgMap.get(row.ref_id) ?? { name: "Organization", place: "" };
        return {
          entityId: row.id,
