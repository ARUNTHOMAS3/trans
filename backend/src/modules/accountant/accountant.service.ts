...
export class AccountantService {
    ...
    if (txs) {
      txs.forEach((tx) => {
        ...
        transactionCountMap.set(
          tx.account_id,
          (transactionCountMap.get(tx.account_id) || 0) + 1,
        );
      });
    }
  ...
  async create(data: any, tenant: TenantContext) {
    ...
    if (data.parentId)
      await this.validateParent(data.parentId, data.accountGroup, tenant);
  ...
  async remove(id: string, tenant: TenantContext) {
    ...
    if (usage.inUse)
      throw new ConflictException(`Cannot delete: ${usage.reason}`);
  ...
  private async validateParent(
    ...
    if (parent.accountGroup !== currentGroup)
      throw new ConflictException(
        "Sub-account must belong to same group as parent.",
      );
  }
  ...
  private async handleOpeningBalanceAdjustment(
    ...
    const adjustmentAccountId =
      await this.ensureOpeningBalanceAdjustmentAccount(tenant);
    const transactions = [
      {
        entity_id: tenant.entityId,
  ...
  private async ensureOpeningBalanceAdjustmentAccount(tenant: TenantContext) {
    ...
    const created = await this.create(
      {
        systemAccountName: "Opening Balance Adjustments",
        userAccountName: "Opening Balance Adjustments",
        accountGroup: "Equity",
        accountType: "Equity",
        isSystem: true,
        isDeletable: false,
  ...
  async findManualJournals(tenant: TenantContext) {
    const rows = await db
      ...
      .where(
        and(
          eq(accountsManualJournals.entityId, tenant.entityId as string),
  ...
  async createManualJournal(dto: any, tenant: TenantContext) {
    ...
    return await db.transaction(async (tx) => {
      const journalNumber = await this.getAndIncrementJournalNumber(
        settings,
        tx,
      );
  ...
  private async getAndIncrementJournalNumber(settings: any, tx: any) {
    ...
    await tx
      ...
      .set({ nextNumber: settings.next_number + 1 })
  ...
  async updateManualJournal(id: string, dto: any, tenant: TenantContext) {
    return await db.transaction(async (tx) => {
      const [journal] = await tx
        ...
        .returning();
      if (!journal)
        throw new BadRequestException("Journal not found or not draft");
      const items = await this.replaceManualJournalItems(
        id,
        dto.items || [],
        tx,
  ...
  async updateManualJournalStatus(
    id: string,
    status: string,
    tenant: TenantContext,
  ) {
  ...
  async reverseManualJournal(id: string, tenant: TenantContext) {
    const original = await this.findManualJournal(id, tenant);
    const reversedItems = original.items.map((it: any) => ({
      ...it,
      debit: it.credit,
      credit: it.debit,
    }));
    return this.createManualJournal(
      { ...original, items: reversedItems, journal_status: "draft" },
      tenant,
    );
  }
  ...
  async createTemplateFromManualJournal(id: string, tenant: TenantContext) {
    ...
    return this.createJournalTemplate(
      {
        templateName: `Template from ${journal.journalNumber}`,
        items: journal.items,
      },
      tenant,
    );
  ...
  async saveFiscalYear(dto: any, tenant: TenantContext) {
    const { data } = await this.supabaseService
      .getClient()
      .from("fiscal_years")
      .insert({ ...dto, entity_id: tenant.entityId })
      .select()
      .single();
    return data;
  }
  ...
  async updateJournalNumberSettings(dto: any, tenant: TenantContext) {
    ...
    const { data } = await this.supabaseService
      .getClient()
      .from("journal_number_settings")
      .upsert({ ...current, ...dto, entity_id: tenant.entityId })
      .select()
      .single();
    return data;
  }
  ...
  async findJournalTemplates(tenant: TenantContext) {
    const { data } = await this.supabaseService
      ...
      .select("*, items:journal_template_items(*)")
      .eq("entity_id", tenant.entityId);
    return data || [];
  }
  ...
  async updateJournalTemplate(id: string, dto: any, tenant: TenantContext) {
    const [t] = await db
      ...
      .where(
        and(
          eq(accountsJournalTemplates.id, id),
          eq(accountsJournalTemplates.entityId, tenant.entityId as string),
        ),
      )
      .returning();
    return t;
  ...
  async deleteJournalTemplate(id: string, tenant: TenantContext) {
    await db
      .delete(accountsJournalTemplates)
      .where(
        and(
          eq(accountsJournalTemplates.id, id),
          eq(accountsJournalTemplates.entityId, tenant.entityId as string),
  ...
  async findContacts(tenant: TenantContext) {
    const { data: c } = await this.supabaseService
      .getClient()
      .from("customers")
      .select("id, display_name")
      .eq("entity_id", tenant.entityId);
    const { data: v } = await this.supabaseService
      .getClient()
      .from("vendors")
      .select("id, display_name")
      .eq("entity_id", tenant.entityId);
    return [
      ...(c || []).map((x) => ({ ...x, type: "customer" })),
      ...(v || []).map((x) => ({ ...x, type: "vendor" })),
    ];
  }
  ...
  async findRecurringJournals(tenant: TenantContext) {
    const { data } = await this.supabaseService
      ...
      .select("*, items:recurring_journal_items(*)")
      .eq("entity_id", tenant.entityId);
    return data || [];
  }
    ...
    const [j] = await db
      ...
      .where(
        and(
          ...
          eq(accountsRecurringJournals.entityId, tenant.entityId as string),
        ),
      )
      .returning();
  ...
  async generateManualJournalFromRecurring(
    ...
    return this.createManualJournal(
      { ...j, journal_date: date || new Date().toISOString() },
      tenant,
    );
  }
  ...
  async findTransactionLocks(tenant: TenantContext) {
    return await db
      ...
      .where(eq(transactionLocks.entityId, tenant.entityId as string));
  }
  ...
  async unlockModule(name: string, tenant: TenantContext) {
    await db
      ...
      .where(
        and(
          eq(transactionLocks.moduleName, name),
          eq(transactionLocks.entityId, tenant.entityId as string),
        ),
      );
    return { success: true };
  }
  ...
  private mapManualJournal(j: any) {
    return j;
  }
  ...
  private mapDrizzleManualJournals(rows: any[]) {
    ...
    for (const row of rows) {
      if (!map.has(row.journal.id))
        map.set(row.journal.id, { ...row.journal, items: [] });
      map.get(row.journal.id).items.push({
        ...row.item,
        account: row.account,
        contact_name: row.customerName || row.vendorName,
      });
    }
  ...
  private async replaceManualJournalItems(
    ...
    await tx
      ...
      .where(eq(accountsManualJournalItems.manualJournalId, journalId));
    if (!items.length) return [];
    return await tx
  ...
  private async postJournalToTransactions(
    id: string,
    tenant: TenantContext,
    tx?: any,
    preload?: any,
  ...
  private getPersistableDraftItems(items: any[]) {
    return items;
  }
