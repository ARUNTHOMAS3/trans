...
export class GstController {
  ...
  async getTaxpayerDetails(
    ...
    if (!gstin || gstin.trim().length === 0) {
      return res.status(HttpStatus.BAD_REQUEST).json({ message: 'gstin query param is required' });
    }
    ...
    try {
      ...
      return res.status(HttpStatus.BAD_REQUEST).json({ message: error.message ?? 'GSTIN lookup failed' });
    }
  }
}
