...
export class ProductsService {
  ...
  private async getCompositeItemBranchInventorySetting(
  ...
  private summarizeHistoryEntry(
    ...
    switch (tableName) {
      ...
      case "product_branch_inventory_settings":
        return {
          section: "Inventory",
          summary:
            actionLabel === "INSERT"
              ? "Branch reorder settings created"
              : actionLabel === "DELETE"
                ? "Branch reorder settings deleted"
                : "Branch reorder settings updated",
        };
      case "composite_item_branch_inventory_settings":
        return {
          section: "Inventory",
          summary:
            actionLabel === "INSERT"
              ? "Composite branch reorder settings created"
              : actionLabel === "DELETE"
                ? "Composite branch reorder settings deleted"
                : "Composite branch reorder settings updated",
        };
      case "branch_inventory":
        return {
          ...
          summary:
            actionLabel === "INSERT"
              ? "Branch inventory row created"
              : actionLabel === "DELETE"
                ? "Branch inventory row deleted"
  ...
  async create(
    ...
    await this.syncProductBranchInventorySetting(
      product.id,
      reorderPoint,
      reorderTermId,
      userId,
  ...
  async findOne(id: string, tenant?: TenantContext) {
    ...
    return this.mapProduct(data, tenant);
  }
  ...
  async searchProducts(q?: string, limit: number = 30, _branchId?: string) {
    ...
    const wordPrefixRegex = new RegExp(
      `(^|[\\s\\-_/().])${escapeRegex(lower)}`,
      "i",
    );

    const rankRow = (row: any) => {
      const name = (row.product_name ?? "").toString();
      const sku = (row.sku ?? "").toString();
      const ean = (row.ean ?? "").toString();
      const itemCode = (row.item_code ?? "").toString();
      ...
      const nameStarts = nameLower.startsWith(lower);
      const wordStarts = wordPrefixRegex.test(nameLower);

      const codeStarts =
        itemCodeLower.startsWith(lower) ||
        skuLower.startsWith(lower) ||
        eanLower.startsWith(lower);

      const namePos = nameLower.indexOf(lower);
      const namePosScore = namePos === -1 ? Number.MAX_SAFE_INTEGER : namePos;

      return {
        exactScore: exactMatch ? 0 : 1,
        nameStartsScore: nameStarts ? 0 : 1,
        wordStartsScore: wordStarts ? 0 : 1,
        codeStartsScore: codeStarts ? 0 : 1,
        namePosScore,
        nameLengthScore: name.length,
        alphaScore: nameLower,
      };
    };
    ...
    const ranked = [...(data ?? [])]
      .sort((a, b) => {
        ...
        if (ra.exactScore !== rb.exactScore) {
          return ra.exactScore - rb.exactScore;
        }
        if (ra.nameStartsScore !== rb.nameStartsScore) {
          return ra.nameStartsScore - rb.nameStartsScore;
        }
        if (ra.wordStartsScore !== rb.wordStartsScore) {
          return ra.wordStartsScore - rb.wordStartsScore;
        }
        if (ra.codeStartsScore !== rb.codeStartsScore) {
          return ra.codeStartsScore - rb.codeStartsScore;
        }
        if (ra.namePosScore !== rb.namePosScore) {
          return ra.namePosScore - rb.namePosScore;
        }
        if (ra.nameLengthScore !== rb.nameLengthScore) {
          return ra.nameLengthScore - rb.nameLengthScore;
        }
        return ra.alphaScore.localeCompare(rb.alphaScore);
      })
      .slice(0, limit);
  ...
  async getBatches(productId: string) {
    ...
    return (data || []).map((batch) => {
      ...
      return {
        ...batch,
        mrp: latestLayer?.mrp ?? (batch as any).mrp ?? 0,
        ptr: latestLayer?.purchase_rate ?? (batch as any).ptr ?? 0,
      };
    });
  }
  ...
  async createBatch(productId: string, body: any) {
    ...
    const payload = {
      product_id: productId,
      batch_no: body.batch_no || body.batch,
      expiry_date: body.expiry_date || body.exp,
        ...
        try {
          const [fallbackCreated] = await db
            ...
            .values([
              {
                productId,
                batchNumber: payload.batch_no,
                expiryDate: payload.expiry_date,
                mrp: body.mrp !== undefined ? Number(body.mrp) : 0,
                ptr: body.ptr !== undefined ? Number(body.ptr) : 0,
          ...
          if (fallbackCreated) {
            return {
              ...
              product_id: fallbackCreated.productId,
              batch: fallbackCreated.batchNumber,
              batch_no: fallbackCreated.batchNumber,
              exp: fallbackCreated.expiryDate,
              expiry_date: fallbackCreated.expiryDate,
              unit_pack: fallbackCreated.unitPack,
              is_manufacture_details: fallbackCreated.isManufactureDetails,
              manufacture_batch_number: fallbackCreated.manufactureBatchNumber,
              manufacture_exp: fallbackCreated.manufactureExpiryDate,
              is_active: fallbackCreated.isActive,
              created_at: fallbackCreated.createdAt,
              updated_at: fallbackCreated.updatedAt,
              mrp: fallbackCreated.mrp,
              ptr: fallbackCreated.ptr,
            };
          }
    ...
    return {
      ...data,
      mrp: body.mrp !== undefined ? Number(body.mrp) : null,
      ptr: body.ptr !== undefined ? Number(body.ptr) : null,
    };
  }

  async getCompositeItems(tenant?: TenantContext) {
    const supabase = this.supabaseService.getClient();
    ...
    try {
      ...
      return Promise.all(
        (data || []).map((item) => this.mapCompositeItem(item, tenant)),
      );
    } catch (error) {
    ...
    switch (key) {
      ...
      case "reorder-terms":
        ...
        checks.push({
          ...
          column: "reorder_term_id",
          description: "branch inventory settings",
          filterActive: true,
        });
        checks.push({
          table: "composite_item_branch_inventory_settings",
          column: "reorder_term_id",
          description: "composite branch inventory settings",
          filterActive: true,
        });
        break;
      case "accounts":
  ...
  async getReorderTerms(tenant?: TenantContext) {
    return this.getScopedReorderTerms(tenant);
  }
  ...
  async deleteReorderTerm(id: string, tenant?: TenantContext) {
    const supabase = this.supabaseService.getClient();
    const scope = this.resolveScope(tenant);
  ...
  async syncReorderTerms(items: any[], tenant?: TenantContext) {
    ...
    if (idsToDisable.length > 0) {
      ...
      if (inUseItems.length > 0) {
        ...
        throw new ConflictException(
          `Cannot delete "${itemNames.join(", ")}" because it is associated with active branch inventory settings`,
        );
      }
  ...
  async getLookupBootstrap(tenant?: TenantContext) {
    const safeLookup = async <T>(
      ...
      try {
        ...
        console.warn(
          `[lookup-bootstrap] fallback for ${name}: ${error?.message ?? "Unknown lookup bootstrap error"}`,
        );
        return [];
      }
    };
  ...
  private async mapProduct(product: any, tenant?: TenantContext) {
    ...
    try {
      const branchSetting = await this.getProductBranchInventorySetting(
        product.id,
        tenant,
      );
      if (branchSetting) {
        product.reorder_point = Number(branchSetting.reorder_point ?? 0);
        product.reorder_term_id = branchSetting.reorder_term_id ?? null;
      }
    } catch (e) {
      console.error(
        `Failed to overlay branch reorder settings for ${product.id}`,
        e,
      );
    }
  ...
  async getQuickStats(productId: string) {
    ...
    try {
      ...
      if (productData) {
        hasProduct = true;
        lastPurchasePrice = Number(productData.last_purchase_price || 0);
      }
    } catch (drizzleError) {
      // Drizzle can fail when direct PG connectivity is intermittent.
      // Fall back to Supabase client so item detail does not hard-fail.
      console.warn(
        "getQuickStats Drizzle query failed; using Supabase fallback:",
        drizzleError instanceof Error ? drizzleError.message : drizzleError,
      );
    }
    ...
    if (!hasProduct) {
      try {
        ...
        if (data?.id) {
          hasProduct = true;
          lastPurchasePrice = Number((data as any).cost_price || 0);
        }
      } catch (fallbackError) {
        console.warn(
          "getQuickStats Supabase fallback failed; returning safe defaults:",
