...
This is archived on March 03 2026 16:30
...
- ackend/src/common/interceptors/audit.interceptor.ts
  ...
  - Why: The purchases*purchase_orders tables were created in Supabase as part of the SQL DDL migration provided. The audit interceptor held the old legacy name that predates the purchases* module prefix convention now used project-wide.
...
## Inventory Picklists — Backend Schema Fix: Missing Drizzle Table Exports (March 31, 2026)

### Summary

NestJS backend was throwing 4 TypeScript compile errors on startup because `picklists.service.ts` imported `inventoryPicklists` and `inventoryPicklistItems` from `src/db/schema.ts`, but those exports did not exist. The Drizzle `db` client uses wildcard import (`import * as schema from "./schema"`), so any table queried via `db.query.*` or imported directly must be exported from `src/db/schema.ts`. The tables existed in Supabase (from a prior DDL migration) but were never registered in the runtime Drizzle schema file.
...
- `backend/src/db/schema.ts`
  - Added `inventoryPicklists` export mapped to `inventory_picklists` DB table. Columns: `id`, `picklistNumber`, `date`, `status`, `assignee`, `location`, `notes`, `createdAt`, `updatedAt`.
  - Added `inventoryPicklistItems` export mapped to `inventory_picklist_items` DB table. Columns: `id`, `picklistId` (FK to inventoryPicklists), `productId` (FK to products), `salesOrderId` (FK to sales_orders), `batchNo`, `quantityOrdered`, `quantityPicked`, `locationBin`, `status`, `createdAt`, `updatedAt`.
  - Both tables are now available on `db.query.inventoryPicklists` and `db.query.inventoryPicklistItems` automatically via the wildcard import in `db.ts`.

### Validation

- All 4 TS compile errors cleared; backend watcher resumed clean compilation.

Timestamp of Log Update: March 31, 2026 - 11:21 AM (IST)

---

## Git — Merge feature/purchase-receives-item-population into main (March 31, 2026)

### Summary

Merged `feature/purchase-receives-item-population` into `main` and deleted the feature branch from both local and remote.

### Actions

- Staged and committed all pending changes on feature branch (15 files, 709 insertions).
- Resolved merge conflicts in `backend/drizzle/relations.ts`, `backend/drizzle/schema.ts`, `log.md`, and `settings_users_user_creation.dart` by accepting feature branch version in all cases.
- Merged feature branch into `main` with `--no-ff` merge commit.
- Deleted local feature branch with `git branch -D`.
- Deleted remote feature branch with `git push origin --delete`.
- Pushed `main` to `origin/main`.

Timestamp of Log Update: March 31, 2026 - 11:21 AM (IST)

## Settings — Restrict Access To (Branch & Warehouse Selection) (March 31, 2026)

### Summary

Implemented the 'Restrict Access To' UI logic for User Creation/Invitation, allowing admins to define granular organizational boundaries. Built a hierarchical selection tree and default location assignment logic with strict ERP validation.
...
- lib/modules/settings/users/presentation/settings_users_user_creation.dart
  - Integrated the 'Restrict Access To' dual-pane UI.
  - Implemented \_buildNestedTree: A hierarchical location selector that nests Warehouses under their parent Branches using parentOutletId.
  - Added real-time search filtering for the location tree.
  - Built the 'Associated Values' right-hand panel showing a numbered summary of selected locations.
  - Implemented explicit 'Default Business' and 'Default Warehouse' assignment logic, filtered by the current selection set.
- lib/modules/settings/users/providers/user_access_provider.dart
  - Corrected init() to fetch from Supabase /branches and /warehouses endpoints.
  - Added parentOutletId mapping from branch_id for warehouses to support the UI tree hierarchy.
  - Added isValid getter: enforces that at least one location is selected and both mandatory defaults are assigned before saving.
  - Implemented toggleOutlet, toggleAll, and selectVisible selection logic.
- lib/core/pages/settings_users_roles_support.dart
  - Verified SettingsLocationRecord model compatibility with parentOutletId and JSON mapping.

### Architectural Decisions

- Hierarchical selection: Chose a nested tree over a flat list to reflect the real-world relationship where warehouses belong to specific branches.
- Assignment validation: Enforced that 'Default' locations MUST be part of the 'Selected' set.
- Mock RBAC compatibility: The UI collects a structured location_access payload ready for backend persistence.

### Validation

- dart analyze lib/modules/settings/users/presentation/settings_users_user_creation.dart
- dart analyze lib/modules/settings/users/providers/user_access_provider.dart
- Result: No issues found.

Timestamp of Log Update: March 31, 2026 - 17:42 PM (IST)

---

## Role Creation Segmented Access Refactor (March 31, 2026)

### Summary

Re-architected the Role Creation segmented access UI to adopt Zoho's distinct card-per-module tabular layout, resolving the massive monolithic table approach. Converted advanced "More Permissions" dropdowns to direct inline checkbox rows, introduced a standard Breadcrumb stepper, and isolated Reports into their own distinct toggleable card.

### Frontend Files

- lib/modules/settings/users_roles/settings_users_roles_role_creation.dart
  - Removed single monolithic table approach in favor of iterative `_buildModuleTable` per module (`PermissionSectionMeta`).
  - Extracted section titles into specialized pale grey table headers.
  - Promoted "More Permissions" overrides out of `MenuAnchor` dropdowns into inline sub-rows generated from `row.overrides`, styled horizontally matching the primary row.
  - Implemented `[✓] General > Segmented Access Control` static breadcrumb layout beneath the page title divider to mirror navigation progress.
  - Structured `_buildReportsCard` with a boolean top-level Switch (`Enable full access for all reports`) and specialized column constraints.
  - Aligned all checkbox triggers with `notifier.toggleAdvancedOverride` and `notifier.togglePermission`.
...
- **Layout Parity**: Extracted sections into distinct borders to align with Zoho's spacing layout.
- **Usability Enhancement**: Displaying secondary permission overrides explicitly underneath main headers reduces clicks needed for fine-grained roles.
- **Reports Separation**: Differentiated the reports grid since reports don't match the standard CRUD column template (Create/Edit), providing clear contextual boundaries.

### Validation

- Tested rendering across all permission definitions (`RolePermissionScheme.getMetadata()`).
- Replaced monolithic table approach while preserving all underlying state modifications and backend interoperability.

Timestamp of Log Update: March 31, 2026 - 19:16 (IST)

---

## Role Creation UI — Checkbox Alignment, Default Pre-selection & Audit (March 31, 2026)

### Summary

Fixed three distinct issues on the New Role creation page to achieve pixel-perfect parity with the Zoho Inventory design: (1) pre-selected checkboxes on load, (2) row label misalignment against column headers, and (3) report table header/checkbox column mismatch.

### Frontend Files

- lib/modules/settings/users_roles/providers/role_permission_scheme.dart
  - Added getDefaultPermissions() static method: iterates all PermissionSectionMeta rows and builds a Map<String, Set<String>> pre-seeding {'full', ...row.actions} for every module key.
  - Added getDefaultReportPermissions() static method: pre-seeds {'full_access', 'view', 'export', 'schedule', 'share'} for all four report categories (SALES, PURCHASES, INVENTORY, ACCOUNTANT).
  - Restored missing getReportCategories() and class closing brace after the addition.

- lib/modules/settings/users_roles/providers/role_creation_provider.dart
  - Added import 'role_permission_scheme.dart' to resolve Undefined name 'RolePermissionScheme' errors.
  - Updated RoleCreationNotifier super-constructor to pass permissions: RolePermissionScheme.getDefaultPermissions() and
    eportPermissions: RolePermissionScheme.getDefaultReportPermissions(), so every checkbox appears checked on first load.

- lib/modules/settings/users_roles/settings_users_roles_role_creation.dart
  - **Row label alignment fix**: Removed redundant padding: EdgeInsets.only(left: 16) from the Expanded label inside \_buildRow. \_HoverRow already applies horizontal: 16 padding to its container; the extra inner padding caused a 32px total indent on the row label while the Particulars header sat at 16px — creating a visible misalignment.
  - **Report header alignment fix**: Changed \_reportHeaderCell from Expanded(flex: 1) to SizedBox(width: 64). The report data rows use \_checkCell() which wraps checkboxes in SizedBox(64); header cells using Expanded(flex: 1) were proportionally-sized (varies with table width) instead of fixed-width — so headers never aligned with the checkboxes below them.
  - Standardised \_reportHeaderCell label font size to match \_headerStyle (previously reduced to 10px unnecessarily).
...
- **Default-selected permissions**: Choosing opt-out (all enabled, admin removes unwanted) over opt-in (start empty) follows the Zoho Inventory model and significantly reduces the setup time for common ERP roles.
- **Fixed-width column discipline**: The SizedBox(64) + SizedBox(120) pattern for checkbox columns is the single source of truth for both header and data rows. Any future column addition must follow this fixed-width rule — never Expanded — to guarantee pixel-perfect vertical alignment regardless of table container width.
- **Audit approach**: Full code-level audit was performed before applying UI fixes: verified each PermissionRowMeta action set, confirmed ogglePermission dependency engine (View forced on any check, clear-all on View uncheck), and verified report oggleReportPermission full-access cascade are all correct.

### Validation

- dart analyze lib/modules/settings/users_roles/providers/role_permission_scheme.dart → No errors
- dart analyze lib/modules/settings/users_roles/providers/role_creation_provider.dart → No errors
- dart analyze lib/modules/settings/users_roles/settings_users_roles_role_creation.dart → No errors

Timestamp of Log Update: March 31, 2026 - 20:13 (IST)

---

---

## Git Synchronization & Code Sanitization (April 1, 2026)

Accomplished a comprehensive git synchronization and codebase cleanup after a remote repository merge and local environment reset.

- **Dependency Verification**: Verified both Flutter and NestJS dependencies after the merge.
- **Schema Audit**: Perposed a database schema audit to resolve PG/Drizzle mismatch errors (inventory_picklists and inventory_picklist_items). User requested to skip these for now.
- **Environment Cleanup**: Identified legacy Supabase SQL migrations vs active Drizzle schema source of truth.
- **Log Synchronization**: Restored and updated the project activity logs after local reset.

### Validation

- **Timestamp**: April 1, 2026 - 11:45 (IST)
- **Status**: Backend running (with known legacy warnings), Frontend active.

---

---

## Accountant — High-Density UI & GST Compliance Synchronization (April 2, 2026)

### Summary

Achieved full horizontal UI synchronization across the Accountant manual records module (Manual Journals, Recurring Journals, and Journal Templates). Integrated professional ERP high-density layouts, dynamic row interactions, and mandatory GST compliance warning systems to ensure all journal entry workflows meet Zerpai's 'Gold Standard' for SME precision.

### Frontend Files

- lib/modules/accountant/manual_journals/presentation/manual_journal_create_screen.dart
  - Rebuilt the '\_buildItemsTable' with a high-density row structure (~48px height) and borderless input fields.
  - Integrated '\_GstWarningWidget' with dynamic row expansion (75px) to alert users when selecting GST/Tax accounts.
  - Implemented resizable, borderless description zones using 'CustomTextField' for enhanced keyboard-first data entry.
  - Updated summary block to include real-time difference tracking with error-state coloring (abs >= 0.01).
- lib/modules/accountant/recurring_journals/presentation/recurring_journal_create_screen.dart
  - Synchronized UI components with the manual journal entry pattern, ensuring a unified user experience.
  - Added GST account detection and compliance warning overlays.
  - Replaced legacy table headers and row layouts with the flex-based '\_cell' architecture for pixel-perfect alignment.
- lib/modules/accountant/manual_journals/presentation/manual_journal_template_create_screen.dart
  - Fully re-architected the template creation screen to match the high-density professional standard.
  - Added support for dynamic row heights and GST compliance warnings within templates.
  - Resolved syntax errors and missing helper method definitions resulting from architectural synchronization.
  - Standardized the multi-select 'More Information' toggle behavior for project and tag mappings.
...
- **Uniformity Over Forking**: Chose to explicitly synchronize templates and recurring journals to the same high-density pattern as manual journals to eliminate cognitive load for accountants switching between entry modes.
- **Compliance-First UX**: Implemented '\_GstWarningWidget' as a floating overlay via 'LayerLink' and 'CompositedTransformFollower'. This provides non-intrusive yet mandatory guidance against direct tax account manipulation, enforcing ERP best practices in the UI layer.
- **Density vs. Clarity**: Maintained an 11px bold header font with 'AppTheme.tableHeaderBg' while providing an 80px fixed-width action column, ensuring high data density without compromising on visual hierarchy or touch targets.

### Validation

- Verified 'manual_journal_create_screen.dart' via Flutter analyzer.
- Verified 'manual_journal_template_create_screen.dart' syntax and layout consistency.
- Confirmed '\_isGstAccount' regex covers 'gst', 'cgst', 'sgst', 'igst', and 'input tax credit'.

Timestamp of Log Update: April 2, 2026 - 15:55 (IST)

## Accountant — Journal Interface standardization & Modal UI (April 3, 2026)

### Summary

Standardized the footer action bars and "Save as Template" modal across the Manual and Recurring Journal modules to align with the "Left-to-Right" button layout pattern. This refactor ensures UI consistency with other high-priority ERP modules and addresses usability feedback regarding button sequence and styling.

### Frontend Files

- lib/modules/accountant/manual_journals/presentation/manual_journal_create_screen.dart
  - Refactored the '\_buildFooter' to use 'ZButton' components (38px height) for consistent branding.
  - Reordered footer buttons to the Left-to-Right pattern: **Save and Publish** (Primary), **Save as Draft** (Secondary), and **Cancel** (Secondary).
  - Repositioned the **Make Recurring** utility action to the far right using 'Spacer()'.
  - Overhauled the **Save as Template** dialog:
    - Reordered buttons to Left-to-Right (**Save** then **Cancel**).
    - Replaced generic buttons with 'ZButton.primary' and 'ZButton.secondary'.
    - Aligned the button group to 'MainAxisAlignment.start' (Left) within the dialog footer.
- lib/modules/accountant/recurring_journals/presentation/recurring_journal_create_screen.dart
  - Synchronized footer layout with the manual journal screen, adopting the 'ZButton' architecture and Left-to-Right sequence.
- lib/modules/accountant/recurring_journals/presentation/widgets/recurring_journals_list_panel.dart
  - Enabled sorting for **Frequency** and **Status** columns via '\_filteredAndSorted' logic.
  - Resolved a redundant 'default' clause in the sorting switch to satisfy strict linting rules.
  - Fixed search UI regression by updating 'prefixIcon' to use 'LucideIcons.search'.
...
- **Consistency over Convention**: Standardized all primary action gates to follow a Left-to-Right flow. This reduces cognitive friction for users accustomed to Zerpai's other transaction modules (Sales/Purchases) which follow the same sequence.
- **Component Standard**: Mandated the use of 'ZButton' in all module footers to eliminate variations in button height and corner radius, ensuring a pixel-perfect "rectangle box" appearance.
- **UI Performance**: Maintained exhaustive switch statements in sorting logic to allow the Dart compiler potentially better optimization paths and to ensure no unhandled enum states.

### Validation

- Verified 'manual_journal_create_screen.dart' via manual code review and UI preview.
- Resolved all linting warnings in 'recurring_journals_list_panel.dart'.
- Confirmed 'ZButton' integration across both journal creation screens.

Timestamp of Log Update: April 3, 2026 - 16:40 (IST)
