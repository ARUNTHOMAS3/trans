# Zerpai PRD: Database Schema Reference
**Last Updated: 2026-04-20 12:46:08**

> **Authoritative Source**: This document is synchronized with `current schema.md`.
> **Tenancy Model**: Unified Polymorphic `entity_id` referencing `organisation_branch_master`.

---

## Table Inventory

The following tables represent the core ERP data structure as of March 30, 2026.

| Category | Tables |
| :--- | :--- |
| **Core Tenancy** | `organisation_branch_master`, `organization`, `branches` |
| **Inventory** | `products`, `batch_master`, `branch_inventory`, `warehouses`, `bin_master`, `zone_master` |
| **Accounting** | `accounts`, `manual_journals`, `transactions`, `fiscal_years`, `currencies`, `tax_rates` |
| **Sales** | `customers`, `sales_orders`, `sales_payments`, `price_lists` |
| **Purchasing** | `vendors`, `purchase_orders`, `payment_terms` |
| **Compliance** | `gstin_registration_types`, `gst_treatments`, `hsn_sac_codes`, `drug_schedules` |
| **Global Lookups** | `states`, `countries`, `lsgd_districts`, `lsgd_local_bodies`, `lsgd_wards` |

---

## Unified Entity Tenancy (`entity_id`)

All business-owned transactional data is scoped using the `entity_id` column.

- **Type**: `uuid NOT NULL`
- **Constraint**: `FOREIGN KEY (entity_id) REFERENCES organisation_branch_master(id)`
- **Behavior**:
  - Middleware injects `entity_id` from the `x-entity-id` header.
  - Queries are automatically filtered by this ID to ensure data isolation.
  - Global lookup tables (e.g., `products`, `tax_rates`) do **not** have an `entity_id` and are shared.
...
CREATE TABLE public.branch_inventory (
  ...
  CONSTRAINT branch_inventory_entity_id_fkey FOREIGN KEY (entity_id) REFERENCES public.organisation_branch_master(id),
  CONSTRAINT branch_inventory_product_id_fkey FOREIGN KEY (product_id) REFERENCES public.products(id)
);
...
CREATE TABLE public.customers (
  ...
  CONSTRAINT customers_price_list_id_fkey FOREIGN KEY (currency_id) REFERENCES public.currencies(id),
