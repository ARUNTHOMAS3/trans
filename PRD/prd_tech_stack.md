...
- `backend/src/common/middleware/tenant.middleware.ts` and org/outlet query handling preserve the multi-tenant request model while dev auth remains disabled.
...
- Dev and staging remain auth-free, but the schema and services must stay auth-ready.
- Product master data is global and must not be scoped by `org_id` in the product entity itself.
- Business transactional records generally require `org_id`; outlet-operational records require `outlet_id` and may also carry `org_id`.
- API requests use tenant headers `X-Org-Id` and `X-Outlet-Id`, and several controllers/services also accept `org_id` / `outlet_id` in query/body payloads.
...
- CORS headers explicitly allow org/outlet tenant headers.
...
- Do not scope the `products` master itself by `org_id`.
