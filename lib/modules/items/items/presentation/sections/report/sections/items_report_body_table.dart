...
extension _ItemsReportBodyTable on _ItemsReportBodyState {
  ...
  List<ItemRow> _sortedItems() {
    ...
    int Function(ItemRow)? relevanceScore;
    ...
    if (_searchQuery.isNotEmpty) {
      ...
      int relevanceScoreFn(ItemRow item) {
        final name = item.name.toLowerCase();
        final itemCode = (item.itemCode ?? '').toLowerCase();
        final sku = (item.sku ?? '').toLowerCase();
        final hsn = (item.hsn ?? '').toLowerCase();
        final brand = (item.brand ?? '').toLowerCase();
        final category = (item.category ?? '').toLowerCase();
        final manufacturer = (item.manufacturer ?? '').toLowerCase();
        final vendor = (item.preferredVendor ?? '').toLowerCase();
        final salesDesc = (item.salesDescription ?? '').toLowerCase();
        final purchaseDesc = (item.purchaseDescription ?? '').toLowerCase();

        // Lower score => higher relevance.
        if (name == query) return 0;
        if (name.startsWith(query)) return 1;
        if (itemCode == query || sku == query) return 2;
        if (itemCode.startsWith(query) || sku.startsWith(query)) return 3;
        if (name.contains(query)) return 4;
        if (itemCode.contains(query) || sku.contains(query)) return 5;
        if (brand.startsWith(query) ||
            category.startsWith(query) ||
            manufacturer.startsWith(query) ||
            vendor.startsWith(query) ||
            hsn.startsWith(query)) {
          return 6;
        }
        if (brand.contains(query) ||
            ...
            hsn.contains(query) ||
            salesDesc.contains(query) ||
            purchaseDesc.contains(query)) {
          return 7;
        }
        return 99;
      }

      relevanceScore = relevanceScoreFn;

      list.sort((a, b) {
        final scoreA = relevanceScoreFn(a);
        final scoreB = relevanceScoreFn(b);
