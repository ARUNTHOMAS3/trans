...
extension _ItemDetailOverview on _ItemDetailScreenState {
  Widget _buildOverviewTab(
    ...
    final reorderTermMeta = state.reorderTerms
        .cast<Map<String, dynamic>?>()
        .firstWhere(
          (rt) => rt?['id']?.toString() == item.reorderTermId,
          orElse: () => null,
        );
    final reorderTermLabel = (() {
      if (reorderTermMeta == null) return 'N/A';
      final rawName =
          (reorderTermMeta['term_name'] ??
                  reorderTermMeta['name'] ??
                  reorderTermMeta['label'])
              ?.toString()
              .trim();
      if (rawName == null || rawName.isEmpty) return 'N/A';
      final qty = reorderTermMeta['quantity'];
            ...
            builder: (context, constraints) {
              ...
              final right = Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 32),
                  _buildReorderPointSection(item, reorderTermLabel),
                ],
              );
  ...
  void _openImagePreview({required int startIndex}) {
    ...
    showGeneralDialog(
      ...
      pageBuilder: (ctx, _, __) => _DetailImagePreview(
        images: _itemImages,
        startIndex: startIndex.clamp(0, _itemImages.length - 1),
        onSetPrimary: (index) {
          updateState(() {
            final img = _itemImages.removeAt(index);
            _itemImages.insert(0, img);
            _primaryImageIndex = 0;
          });
          _updateItemImages();
...
class _DetailImagePreview extends StatefulWidget {
  ...
  @override
  State<_DetailImagePreview> createState() => _DetailImagePreviewState();
}

class _DetailImagePreviewState extends State<_DetailImagePreview> {
  late int _currentIndex;
  final ScrollController _thumbScrollController = ScrollController();
  static const double _scrollDeltaThreshold = 8;
  static const double _footerZoneHeight = 100;
  ...
  void _handlePointerSignal(PointerSignalEvent event) {
    ...
    if (event.localPosition.dy >=
        (MediaQuery.of(context).size.height - _footerZoneHeight)) {
      return;
    }
    final delta = event.scrollDelta;
    final horizontal = delta.dx.abs();
    final vertical = delta.dy.abs();
    if (horizontal < _scrollDeltaThreshold &&
        vertical < _scrollDeltaThreshold) {
      return;
    }
    final shouldGoNext = horizontal >= vertical ? delta.dx > 0 : delta.dy > 0;
    if (shouldGoNext) {
      _nextImage();
    } else {
      _previousImage();
  ...
  void _handleThumbnailPointerSignal(PointerSignalEvent event) {
    ...
    if (delta.abs() < _scrollDeltaThreshold) return;

    final target = (_thumbScrollController.offset + delta).clamp(
