...
class CompositionSection extends StatefulWidget {
  ...
  final List<Map<String, dynamic>> productTypeOptions;
  ...
  final Future<List<Map<String, dynamic>>> Function(String query)?
  onStrengthSearch;
  final Future<Map<String, dynamic>> Function(String name)? onCreateStrength;
  final Future<Map<String, dynamic>> Function(String name)? onCreateContent;
  final Future<Map<String, dynamic>> Function(String name)? onCreateBuyingRule;
  final Future<Map<String, dynamic>> Function(String name)?
  onCreateDrugSchedule;
  final Future<Map<String, dynamic>> Function(String name)? onCreateProductType;
  final Future<List<Map<String, dynamic>>> Function(String query)?
  onBuyingRuleSearch;
  final Future<List<Map<String, dynamic>>> Function(String query)?
  ...
  const CompositionSection({
    ...
    this.onSyncProductTypes,
    this.onDeleteCheck,
    this.onContentSearch,
    this.onStrengthSearch,
    this.onCreateStrength,
    this.onCreateContent,
    this.onCreateBuyingRule,
    this.onCreateDrugSchedule,
    this.onCreateProductType,
    this.onBuyingRuleSearch,
    this.onDrugScheduleSearch,
    this.onProductTypeSearch,
    this.onRefresh,
  });
...
class _CompositionSectionState extends State<CompositionSection> {
  ...
  String? _firstNonEmptyLabel(Map<String, dynamic>? row) {
  ...
  void didUpdateWidget(CompositionSection oldWidget) {
    ...
    if (widget.initialProductType != oldWidget.initialProductType) {
      _selectedProductType = widget.initialProductType;
    }
        ...
        onSelect: (value) {
          if (value is Map) {
            ...
            if (id != null && id.isNotEmpty) {
              _searchResultsCache[id] = Map<String, dynamic>.from(value);
            }
            onSelect(id);
      ...
      children: [
        ...
        _singleFieldRow(
          ...
          child: _managedDropdown(
            ...
            onCreateOne: widget.onCreateBuyingRule,
        ...
        _singleFieldRow(
          ...
          child: _managedDropdown(
            ...
            onCreateOne: widget.onCreateDrugSchedule,
        ...
        _singleFieldRow(
          ...
          child: _managedDropdown(
            ...
            items: widget.productTypeOptions,
            hint: 'Select product type',
            manageLabel: 'Product Type',
            lookupKey: 'product-types',
            onSync: widget.onSyncProductTypes,
            onCreateOne: widget.onCreateProductType,
            onSearch: widget.onProductTypeSearch,
            onChanged: (v) {
              setState(() => _selectedProductType = v);
              widget.onProductTypeChanged(v);
            },
          ),
        ),
      ],
  ...
  Widget _contentField(int i, ItemComposition row) => _managedDropdown(
    ...
    onCreateOne: widget.onCreateContent,
  ...
  Widget _strengthField(int i, ItemComposition row) => _managedDropdown(
    ...
    onCreateOne: widget.onCreateStrength,
  ...
  Widget _managedDropdown({
    ...
    if (value != null && !localItems.any((i) => i['id'] == value)) {
      ...
      final cachedLabel = _firstNonEmptyLabel(_searchResultsCache[value]);
      final label = cachedLabel ?? widget.lookupCache[value] ?? fallbackLabel;
      ...
      if (label != null) {
        ...
        // Keep value selectable. Do not inject empty `name` because that
        // blocks fallback resolution from search/cache and can render blank.
        localItems.add({'id': value});
      }
    }
    ...
    return FormDropdown<String>(
      ...
      displayStringForValue: (val) {
        ...
        final matchLabel = _firstNonEmptyLabel(match);
        if (matchLabel != null) {
          return matchLabel;
        }

        // 2. Try search results cache
        final cachedLabel = _firstNonEmptyLabel(_searchResultsCache[val]);
        if (cachedLabel != null) {
          return cachedLabel;
        }

        // 3. Try global lookup cache (from pre-joined fields)
        final lookupLabel = widget.lookupCache[val]?.trim();
        if (lookupLabel != null && lookupLabel.isNotEmpty) return lookupLabel;
        return '';
      },
      showSettings: true,
      ...
      itemBuilder: (id, isSelected, isHovered) {
        ...
        final label =
            _firstNonEmptyLabel(match) ??
            _firstNonEmptyLabel(_searchResultsCache[id]) ??
            (() {
              final text = widget.lookupCache[id]?.trim();
