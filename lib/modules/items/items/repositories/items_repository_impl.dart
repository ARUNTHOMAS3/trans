...
class ItemsRepositoryImpl implements ItemRepository {
  ...
  Future<int> deleteItemsBulk(Set<String> ids) async {
    ...
    try {
      AppLogger.info(
        'Bulk deleting items via API',
        data: {'count': ids.length},
        module: 'items_repository',
      );

      final idList = ids.toList();
      final deletedCount = await _apiService.updateProductsBulk(idList, {
        'is_active': false,
      });
  ...
  Future<List<TransactionData>> getItemStockTransactions(String itemId) async {
    try {
      return await _apiService.getProductStockTransactions(itemId);
    } catch (e, st) {
      AppLogger.error(
        'Failed to fetch item stock transactions from API',
        error: e,
        stackTrace: st,
        data: {'itemId': itemId},
        module: 'items_repository',
