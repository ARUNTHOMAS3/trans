...
class ItemsController extends StateNotifier<ItemsState> {
  ...
  Future<List<Item>> searchProductsNoState(String query) async {
    ...
    try {
      ...
      AppLogger.error(
        'Failed to search products without state',
        error: e,
        module: 'items',
      );
      return [];
  ...
  Item _mergeWithExistingItem(Item incoming) {
    ...
    return incoming.copyWith(
      ...
      repName: incoming.repName ?? existing.repName,
      preferredVendorName:
  ...
  Future<void> loadLookupData({bool force = false}) async {
    ...
    try {
      ...
      if (bootstrap.isNotEmpty) {
        ...
        repsRaw = List<Map<String, dynamic>>.from(
          bootstrap['reps'] ?? const [],
        );
        ...
        drugSchedulesRaw = List<Map<String, dynamic>>.from(
          futureResults[15] as List<Map<String, dynamic>>,
        );
        productTypesRaw = List<Map<String, dynamic>>.from(
      ...
      final productTypes = productTypesRaw
          .map(
            (pt) => {
              ...pt,
              'name':
  ...
  Future<int> deleteItemsBulk(Set<String> ids) async {
    if (ids.isEmpty) return 0;
    ...
    try {
      ...
      state = state.copyWith(isSaving: true, validationErrors: {});
      final deleted = await repo.deleteItemsBulk(ids);

      AppLogger.info(
        'Bulk items deleted successfully',
        module: 'items',
      ...
      AppLogger.error('Failed to bulk delete items', error: e, module: 'items');
      state = state.copyWith(error: e.userMessage, isSaving: false);
      return 0;
    } catch (e) {
      AppLogger.error(
        'Unexpected error bulk deleting items',
        error: e,
        module: 'items',
      );
      state = state.copyWith(
  ...
  Future<List<Map<String, dynamic>>> syncCategories(
    ...
    final results = await _syncGeneric(
      'Categories',
      () => _lookupsService.syncCategories(items),
    );
  ...
  Future<List<Map<String, dynamic>>> refreshCategories({
    bool useCache = false,
  }) async {
    final fetched = await _lookupsService.getCategories(useCache: useCache);
    final categories = fetched
        .map((c) => {...c, 'name': c['name'] ?? 'Unknown'})
        .toList();
    state = state.copyWith(categories: categories, error: null);
    return categories;
  ...
  Future<Map<String, dynamic>> createCategory(String name) async {
    final created = await _lookupsService.createCategory(name);
    final normalized = {
      ...created,
    ...
    final currentMap = <String, Map<String, dynamic>>{
      for (final c in state.categories) c['id']?.toString() ?? '': c,
    };
    final id = normalized['id']?.toString() ?? '';
    if (id.isNotEmpty) {
      currentMap[id] = normalized;
      state = state.copyWith(
        categories: currentMap.values
  ...
  Future<Map<String, dynamic>> createManufacturer(String name) async {
    final created = await _lookupsService.createManufacturer(name);
    final normalized = {
      ...created,
      'name': created['name'] ?? created['manufacturer_name'] ?? name,
    };

    final currentMap = <String, Map<String, dynamic>>{
      for (final m in state.manufacturers) m['id']?.toString() ?? '': m,
    };
    final id = normalized['id']?.toString() ?? '';
    if (id.isNotEmpty) {
      currentMap[id] = normalized;
      state = state.copyWith(
        manufacturers: currentMap.values
            .where((e) => (e['id'] ?? '').toString().isNotEmpty)
            .toList(),
        error: null,
      );
  ...
  Future<List<Map<String, dynamic>>> syncBrands(
    ...
    final results = await _syncGeneric(
      'Brands',
      () => _lookupsService.syncBrands(items),
    );
    final brands = results
        .map((b) => {...b, 'name': b['name'] ?? b['brand_name'] ?? 'Unknown'})
        .toList();
    state = state.copyWith(brands: brands, error: null);
    return brands;
  }

  Future<Map<String, dynamic>> createBrand(String name) async {
    final created = await _lookupsService.createBrand(name);
    final normalized = {
      ...created,
      'name': created['name'] ?? created['brand_name'] ?? name,
    };

    final currentMap = <String, Map<String, dynamic>>{
      for (final b in state.brands) b['id']?.toString() ?? '': b,
    };
    final id = normalized['id']?.toString() ?? '';
    if (id.isNotEmpty) {
      currentMap[id] = normalized;
      state = state.copyWith(
        brands: currentMap.values
            .where((e) => (e['id'] ?? '').toString().isNotEmpty)
            .toList(),
        error: null,
      );
  ...
  Future<Map<String, dynamic>> createStorageLocation(String name) async {
    final created = await _lookupsService.createStorageLocation(name);
    final normalized = {
      ...created,
      'name':
          created['name'] ??
          created['display_text'] ??
          created['location_name'] ??
          name,
    };

    final currentMap = <String, Map<String, dynamic>>{
      for (final s in state.storageLocations) s['id']?.toString() ?? '': s,
    };
    final id = normalized['id']?.toString() ?? '';
    if (id.isNotEmpty) {
      currentMap[id] = normalized;
      state = state.copyWith(
        storageLocations: currentMap.values
            .where((e) => (e['id'] ?? '').toString().isNotEmpty)
            .toList(),
        error: null,
      );
  ...
  Future<List<Map<String, dynamic>>> syncAccounts(
    ...
    final results = await _syncGeneric(
      'Accountant',
      () => _lookupsService.syncAccounts(items),
    );
    final accounts = results
        ...
        .toList();
    state = state.copyWith(accounts: accounts, error: null);
    return accounts;
  }
  ...
  Future<List<Map<String, dynamic>>> syncRacks(
    ...
    final results = await _syncGeneric(
      'Racks',
      () => _lookupsService.syncRacks(items),
    );
    final racks = results
        .map((r) => {...r, 'name': r['rack_name'] ?? r['name'] ?? 'Unknown'})
        .toList();
    state = state.copyWith(racks: racks, error: null);
    return racks;
  }
  ...
  Future<Map<String, dynamic>> createRack(String name) async {
    ...
    final currentMap = <String, Map<String, dynamic>>{
      for (final r in state.racks) r['id']?.toString() ?? '': r,
    };
    final id = normalized['id']?.toString() ?? '';
    if (id.isNotEmpty) {
      currentMap[id] = normalized;
      state = state.copyWith(
        racks: currentMap.values
            .where((e) => (e['id'] ?? '').toString().isNotEmpty)
            .toList(),
        error: null,
      );
  ...
  Future<List<Map<String, dynamic>>> syncReorderTerms(
    ...
    final reorderTerms = results.map((r) {
    ...
    state = state.copyWith(reorderTerms: reorderTerms, error: null);
    return reorderTerms;
  }
  ...
  Future<List<Map<String, dynamic>>> syncContents(
    ...
    final results = await _syncGeneric(
      'Contents',
      () => _lookupsService.syncContents(items),
    );
    final contents = results.map((c) {
    ...
    state = state.copyWith(contents: contents, error: null);
    return contents;
  }
  ...
  Future<Map<String, dynamic>> createContent(String name) async {
    ...
    final currentMap = <String, Map<String, dynamic>>{
      for (final c in state.contents) c['id']?.toString() ?? '': c,
    };
    final id = normalized['id']?.toString() ?? '';
    if (id.isNotEmpty) {
      currentMap[id] = normalized;
      state = state.copyWith(
        contents: currentMap.values
            .where((e) => (e['id'] ?? '').toString().isNotEmpty)
            .toList(),
        error: null,
      );
  ...
  Future<List<Map<String, dynamic>>> syncStrengths(
    ...
    try {
      final results = await _lookupsService.syncStrengths(items);
      _lookupsService.clearLookupsCache();

      final strengths = results
          .map(
            (s) => {
              ...s,
              'name':
      ...
      state = state.copyWith(strengths: strengths, error: null);
      return strengths;
    } catch (e) {
      state = state.copyWith(error: "Failed to sync Strengths: $e");
      rethrow;
    }
  }

  Future<Map<String, dynamic>> createStrength(String name) async {
    final created = await _lookupsService.createStrength(name);
    final normalized = {
      ...created,
      'name':
          created['name'] ??
          created['item_strength'] ??
          created['strength_name'] ??
          name,
    };

    final currentMap = <String, Map<String, dynamic>>{
      for (final s in state.strengths) s['id']?.toString() ?? '': s,
    };
    final id = normalized['id']?.toString() ?? '';
    if (id.isNotEmpty) {
      currentMap[id] = normalized;
  ...
  Future<List<Map<String, dynamic>>> syncBuyingRules(
    ...
    final buyingRules = results.map((b) {
    ...
    state = state.copyWith(buyingRules: buyingRules, error: null);
    return buyingRules;
  }
  ...
  Future<Map<String, dynamic>> createBuyingRule(String name) async {
    ...
    final currentMap = <String, Map<String, dynamic>>{
      for (final b in state.buyingRules) b['id']?.toString() ?? '': b,
    };
    final id = normalized['id']?.toString() ?? '';
    if (id.isNotEmpty) {
      currentMap[id] = normalized;
      state = state.copyWith(
        buyingRules: currentMap.values
            .where((e) => (e['id'] ?? '').toString().isNotEmpty)
            .toList(),
        error: null,
      );
  ...
  Future<List<Map<String, dynamic>>> syncDrugSchedules(
    ...
    final drugSchedules = results.map((d) {
    ...
    state = state.copyWith(drugSchedules: drugSchedules, error: null);
  ...
  Future<Map<String, dynamic>> createDrugSchedule(String name) async {
    ...
    final currentMap = <String, Map<String, dynamic>>{
      for (final d in state.drugSchedules) d['id']?.toString() ?? '': d,
    };
    final id = normalized['id']?.toString() ?? '';
    if (id.isNotEmpty) {
      currentMap[id] = normalized;
      state = state.copyWith(
        drugSchedules: currentMap.values
            .where((e) => (e['id'] ?? '').toString().isNotEmpty)
            .toList(),
        error: null,
      );
  ...
  Future<List<Map<String, dynamic>>> syncProductTypes(
    ...
    final results = await _syncGeneric(
      'Product Types',
      () => _lookupsService.syncProductTypes(items),
    );
    final productTypes = results
        .map(
          (pt) => {
            ...pt,
        ...
        .toList();
    state = state.copyWith(productTypes: productTypes, error: null);
    return productTypes;
  }

  Future<Map<String, dynamic>> createProductType(String name) async {
    final created = await _lookupsService.createProductType(name);
    final normalized = {
      ...created,
      'name': created['name'] ?? created['product_type_name'] ?? name,
    };

    final currentMap = <String, Map<String, dynamic>>{
      for (final pt in state.productTypes) pt['id']?.toString() ?? '': pt,
    };
    final id = normalized['id']?.toString() ?? '';
    if (id.isNotEmpty) {
      currentMap[id] = normalized;
      state = state.copyWith(
        productTypes: currentMap.values
            .where((e) => (e['id'] ?? '').toString().isNotEmpty)
            .toList(),
        error: null,
      );
  ...
  Future<List<Map<String, dynamic>>> searchReps(String query) async {
    final results = await _lookupsService.searchReps(query);
    final mapped = results
        .map((r) => {...r, 'name': r['name'] ?? r['rep_name'] ?? 'Unknown'})
        .toList();

    final Map<String, Map<String, dynamic>> currentMap = {
      for (var r in state.reps) r['id']: r,
    };
    bool added = false;
    for (var r in mapped) {
    ...
    if (added) {
      state = state.copyWith(reps: currentMap.values.toList());
    }

    return mapped;
  }

  Future<Map<String, dynamic>> createRep({
    ...
    final created = await _lookupsService.createRep({
      'name': name.trim(),
      if (number != null && number.trim().isNotEmpty) 'number': number.trim(),
      if (brandId != null && brandId.trim().isNotEmpty) 'brand_id': brandId,
    ...
    final normalized = {
      ...created,
      'name': created['name'] ?? created['rep_name'] ?? name.trim(),
    };

    final currentMap = <String, Map<String, dynamic>>{
      for (final rep in state.reps)
        if (rep['id'] != null) rep['id'].toString(): rep,
    };
    final newId = normalized['id']?.toString();
    if (newId != null && newId.isNotEmpty) {
      currentMap[newId] = normalized;
    }
    state = state.copyWith(reps: currentMap.values.toList(), error: null);
    return normalized;
  }
  ...
  Future<List<Map<String, dynamic>>> searchProductTypes(String query) async {
    final results = await _lookupsService.searchLookups('product-types', query);
    final mapped = results
        .map(
          (pt) => {
            ...pt,
    ...
    final Map<String, Map<String, dynamic>> currentMap = {
      for (var pt in state.productTypes) pt['id']: pt,
    };
    for (var pt in mapped) {
      currentMap[pt['id']] = pt;
    }
  ...
  Future<List<Map<String, dynamic>>> _syncGeneric(
    ...
    try {
      ...
      // Keep cache coherent while avoiding full bootstrap refetch.
      _lookupsService.clearLookupsCache();
      return results;
    } catch (e) {
  ...
  Map<String, String> _getUpdatedCache(Item item) {
    ...
    add(item.productTypeId, item.productTypeName);
