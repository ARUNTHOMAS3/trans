...
class _PriceListEditScreenState extends ConsumerState<PriceListEditScreen> {
        ...
        child: _tableInputField(
          ...
          onChanged: (val) {
            ...
            if (baseRate > 0) {
              ...
              _getDiscountController(itemId, newDiscount).text =
        ...
        child: _tableInputField(
          ...
          onChanged: (val) {
            ...
            if (discount != null) {
              ...
              _getRateController(itemId, newCustomRate).text =
      ...
      children: [
        ...
        Expanded(
          ...
          child: Padding(
            ...
            child: _tableInputField(
              controller: _getVolCtrl(_volStartControllers, '${itemId}_$idx', range.startQuantity),
              hintText: 'Start',
              textAlign: TextAlign.right,
              onChanged: (v) => _updateVolumeRange(itemId, idx,
                  itemName: item.productName as String?,
                  sku: item.sku as String?,
                  salesRate: baseRate,
        ...
        Expanded(
          ...
          child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 4),
            child: _tableInputField(
              controller: _getVolCtrl(_volEndControllers, '${itemId}_$idx', range.endQuantity),
              hintText: 'End',
              textAlign: TextAlign.right,
              onChanged: (v) => _updateVolumeRange(itemId, idx,
                  itemName: item.productName as String?,
        ...
        Expanded(
          ...
          child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 4),
            child: _tableInputField(
              controller: _getVolCtrl(_volRateControllers, '${itemId}_$idx', range.customRate),
              prefixText: '\u20B9',
              textAlign: TextAlign.right,
              onChanged: (v) => _updateVolumeRange(itemId, idx,
                  itemName: item.productName as String?,
        ...
        if (_isDiscountEnabled)
          Expanded(
            ...
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 4),
              child: _tableInputField(
                controller: _getVolCtrl(_volDiscountControllers, '${itemId}_$idx', range.discountPercentage),
                suffixText: '%',
                textAlign: TextAlign.right,
                onChanged: (v) => _updateVolumeRange(itemId, idx,
                    itemName: item.productName as String?,
  ...
  void _clearVolCaches(String itemId) {
    for (final cache in [_volStartControllers, _volEndControllers, _volRateControllers, _volDiscountControllers]) {
      cache.removeWhere((k, v) {
        if (k.startsWith('${itemId}_')) { v.dispose(); return true; }
        return false;
      });
    }
  }
  ...
  void _updateVolumeRange(
    ...
    final current = _itemRateOverrides[itemId] ??
        PriceListItemRate(
          ...
          sku: sku,
          salesRate: salesRate,
          volumeRanges: [const PriceListVolumeRange(startQuantity: 1, customRate: 0)],
        );
    final ranges = current.volumeRanges ?? [const PriceListVolumeRange(startQuantity: 1, customRate: 0)];
  ...
  Widget _buildLabeledField({
    ...
    children: [
      ...
      const SizedBox(width: 12),
      // Use Flexible instead of Expanded so Row children that themselves
      // contain Rows/Flexible widgets don't receive unbounded constraints.
