...
class WarehouseStockData {
  ...
  WarehouseStockData copyWith({
    ...
    return WarehouseStockData(
      ...
      salesOrderLineId: salesOrderLineId ?? this.salesOrderLineId,
  ...
  factory WarehouseStockData.fromJson(Map<String, dynamic> json) {
    return WarehouseStockData(
      id: json['id']?.toString() ?? json['sales_order_line_id']?.toString(),
      warehouseId:
      ...
      stock: (json['currentStock'] as num? ?? json['stock'] as num? ?? 0)
          .toDouble(),
      quantityOnHand:
          (json['quantityOnHand'] as num? ??
                  json['quantity_on_hand'] as num? ??
                  0)
              .toDouble(),
      ...
      salesOrderId:
          json['salesOrderId'] as String? ?? json['sales_order_id'] as String?,
      salesOrderLineId:
          json['salesOrderLineId'] as String? ??
          json['sales_order_line_id'] as String? ??
          json['id']?.toString(),
      salesOrderNumber:
      ...
      quantityOrdered:
          (json['quantityOrdered'] as num? ?? json['quantity_ordered'] as num?)
              ?.toDouble(),
      quantityToPick:
          (json['quantityToPick'] as num? ?? json['quantity_to_pick'] as num?)
              ?.toDouble(),
      quantityPicked:
          (json['quantityPicked'] as num? ?? json['quantity_picked'] as num?)
              ?.toDouble(),
      preferredBin:
          json['preferredBin'] as String? ?? json['preferred_bin'] as String?,
      availableQuantity:
          (json['availableQuantity'] as num? ??
...
final productStockInWarehouseProvider =
...
final stockByWarehouseProvider =
    FutureProvider.family<List<WarehouseStockData>, String>((
      ref,
      warehouseId,
    ) async {
      ...
      try {
        ...
        if (response.success) {
          ...
          final result = itemsData
              .map(
                (itemJson) => WarehouseStockData.fromJson(
          ...
          // ignore: avoid_print
          print(
            '[stockByWarehouseProvider] Loaded ${result.length} items for warehouse $warehouseId',
          );
          return result;
        } else {
          return [];
        ...
        // ignore: avoid_print
        print(
          '[stockByWarehouseProvider] Error loading items for $warehouseId: $e',
        );
        return [];
      }
    });
