...
class _InventoryPackagesListScreenState
  ...
  void _initializeColumns() {
    _allColumns = [
      ...
      ColumnConfig(id: 'quantity', label: 'QUANTITY', orderIndex: 8),
      ColumnConfig(id: 'picklist#', label: 'PICKLIST#', orderIndex: 9),
    ];
  ...
  Future<void> _loadColumnSettings() async {
    try {
      ...
      final jsonStr = prefs.getString('package_table_columns_config');
      if (jsonStr != null) {
  ...
  Future<void> _saveColumnSettings() async {
    try {
      ...
      await prefs.setString('package_table_columns_config', jsonStr);
    } catch (e) {
  ...
  Widget _buildCell(InventoryPackage pkg, String colId, {required double width}) {
    ...
    switch (colId) {
      ...
      case 'customer_name':
        ...
        break;
      case 'quantity':
        text = (pkg.quantity ?? 0.0).toStringAsFixed(2);
        break;
      case 'picklist#':
        text = pkg.picklistNumbers.join(', ');
        break;
    }
...
class _PackagePdfView extends ConsumerStatefulWidget {
  final InventoryPackage package;
  const _PackagePdfView({required this.package});

  @override
  ConsumerState<_PackagePdfView> createState() => _PackagePdfViewState();
}
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          ...List.generate(widget.package.items.length, (index) {
...
// Removed _HeaderMenuButton class as it's now replaced by ZTableHeaderMenu reusable.
...
//   if (s.contains('confirm') || s.contains('pack')) return AppTheme.primaryBlue;
//   if (s.contains('ship') || s.contains('deliver')) return AppTheme.successDark;
//   if (s.contains('draft') || s.contains('progress')) return AppTheme.warningOrange;
