...
class _InventoryPackagesCreateScreenState
  ...
  void _clearRowControllers() {
    ...
    for (final ctrl in _normalRowControllers) {
      ctrl.dispose();
    }
  ...
  Future<void> _loadPackageData() async {
    ...
    try {
      ...
      setState(() {
        ...
        _items = loadedItems;
        _isManualMode = pkg.isManualMode;

        _salesOrderItems = loadedItems.map((item) {
        ...
        _normalRowControllers.addAll(
          List.generate(_items.length, (i) {
            final ctrl = TextEditingController(
              text: _items[i].qtyToPack.toInt().toString(),
            );
            if (_isEditMode) {
              ctrl.addListener(_checkForChanges);
            }
            return ctrl;
          }),
        );
  ...
  Future<void> _savePackage() async {
    ...
    try {
      ...
      if (success && mounted) {
        ...
        ZerpaiToast.error(context, state.error ?? 'Failed to save package');
      }
    } catch (e) {
      ...
      for (var p in picklists) {
        ...
        if (fullPicklist != null) {
          for (var pi in fullPicklist.items) {
            ...
            // Extract batches from picklist item — skip allocations without a batch_no
            ...
            collectedItems.add(
              _PackageItem(
                ...
                salesOrderNumber: effectiveSalesOrderNumber,
                sourceItemId: pi.id, // Store unique picklist item ID
                batches: itemBatches,
              ),
            );
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                const SizedBox(width: 12),
                Text(
                  _isEditMode ? 'Edit Package' : 'New Package',
                                  ...
                                  child: _buildFormRow(
                                    ...
                                    child: _isEditMode
                                        ? CustomTextField(
                                            ...
                                            fillColor: AppTheme.bgDisabled,
                                            controller: TextEditingController(
                                              text: _selectedSalesOrderDataList
                                                  .map((s) => s.saleNumber)
                                                  .join(', '),
                                            ),
                                  ...
                                  child: _buildFormRow(
                                    ...
                                    child: _isEditMode
                                        ? CustomTextField(
                                            ...
                                            fillColor: AppTheme.bgDisabled,
                                            controller: TextEditingController(
                                              text: _selectedPicklistValues
                                                  .map((p) => p.picklistNumber)
                                                  .join(', '),
                                            ),
