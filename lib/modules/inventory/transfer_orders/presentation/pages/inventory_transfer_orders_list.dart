...
class _TransferOrderDetailPanelState extends State<_TransferOrderDetailPanel> {
  ...
  String? _currentBranchIdFromConfig() {
    try {
      final box = Hive.box('config');
      return (box.get('selected_branch_id') as String?)?.trim();
    } catch (_) {
      return null;
    }
  }

  String? _currentEntityIdFromConfig() {
    try {
      final box = Hive.box('config');
      return (box.get('selected_entity_id') as String?)?.trim();
    } catch (_) {
      return null;
    }
  }
  ...
  void _handleMarkAsReceived(StockTransfer transfer) async {
    ...
    try {
      final user = _currentUserFromConfig();
      final branchId = _currentBranchIdFromConfig();
      final entityId = _currentEntityIdFromConfig();
      final currentStatus = normalizeTransactionStatus(transfer.status);
      if (currentStatus == 'draft') {
        final initiateDecision = TransactionStatusTransitionGuard.canTransition(
          user: user,
          transactionType: 'inventory.transfer',
          fromStatus: currentStatus,
          toStatus: 'initiated',
          branchId: branchId,
          requiredPermission: 'inventory.transfer.edit',
          enforcePermission: false,
        );
        if (!initiateDecision.allowed) {
          if (mounted) ZerpaiToast.error(context, initiateDecision.reason);
          return;
        }
        ...
        if (user != null) {
          final event = TransactionStatusTransitionGuard.buildAuditEvent(
            ...
            afterStatus: 'initiated',
            actor: user,
            reason: 'Transfer initiated before receive',
            permissionUsed: initiateDecision.requiredPermission,
            branchId: branchId,
            metadata: <String, dynamic>{
              'entity_context': entityId,
              'branch_context': branchId,
              'warehouse_context': transfer.fromWarehouseId,
            },
          );
          AppLogger.info(
            'Inventory transfer status transition',
            module: 'inventory_transfer_orders',
            userId: user.id,
            orgId: user.orgId,
            data: event.toJson(),
      ...
      final approveDecision = TransactionStatusTransitionGuard.canTransition(
        user: user,
        transactionType: 'inventory.transfer',
        fromStatus: currentStatus == 'draft' ? 'initiated' : currentStatus,
        toStatus: 'received',
        branchId: branchId,
        requiredPermission: 'inventory.transfer.edit',
        enforcePermission: false,
      ...
      if (!approveDecision.allowed) {
        if (mounted) ZerpaiToast.error(context, approveDecision.reason);
        return;
      }
      ...
      if (user != null) {
        final event = TransactionStatusTransitionGuard.buildAuditEvent(
          transactionType: 'inventory.transfer',
          transactionId: transfer.id,
          beforeStatus: currentStatus == 'draft' ? 'initiated' : currentStatus,
          afterStatus: 'received',
          actor: user,
          reason: 'Transfer marked as received',
          permissionUsed: approveDecision.requiredPermission,
          branchId: branchId,
          metadata: <String, dynamic>{
            'entity_context': entityId,
            'branch_context': branchId,
            'warehouse_context': transfer.toWarehouseId,
          },
        );
        AppLogger.info(
          'Inventory transfer status transition',
