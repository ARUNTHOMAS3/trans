...
class RetainerInvoicesListPage extends ConsumerStatefulWidget {
  const RetainerInvoicesListPage({super.key});

  @override
  ConsumerState<RetainerInvoicesListPage> createState() =>
      _RetainerInvoicesListPageState();
}

class _RetainerInvoicesListPageState
    extends ConsumerState<RetainerInvoicesListPage> {
  ...
  String _sortByField = 'Created Time';
  ...
  List<ColumnConfig> _columns = [
    ColumnConfig(id: 'DATE', label: 'Date', isLocked: true, orderIndex: 0),
    ColumnConfig(id: 'LOCATION', label: 'Location', orderIndex: 1),
    ColumnConfig(
      id: 'RETAINER INVOICE NUMBER',
      label: 'Retainer Invoice Number',
      isLocked: true,
      orderIndex: 2,
    ),
    ColumnConfig(id: 'REFERENCE#', label: 'Reference#', orderIndex: 3),
    ColumnConfig(
      id: 'CUSTOMER NAME',
      label: 'Customer Name',
      isLocked: true,
    ...
    ColumnConfig(
      id: 'RETAINER DRAW STATUS',
      label: 'Retainer Draw Status',
      orderIndex: 7,
    ),
    ColumnConfig(id: 'AMOUNT', label: 'Amount', isLocked: true, orderIndex: 8),
    ColumnConfig(id: 'BALANCE', label: 'Balance', orderIndex: 9),
    ColumnConfig(id: 'ISSUED DATE', label: 'Issued Date', orderIndex: 10),
    ColumnConfig(
      id: 'PROJECT',
      label: 'Project',
      isVisible: false,
    ...
    ColumnConfig(id: 'QUOTE', label: 'Quote', isVisible: false, orderIndex: 12),
  ];
  ...
  Map<String, double> get _columnWidths => __columnWidths ??= {
    'DATE': 200.0,
    'LOCATION': 220.0,
    'RETAINER INVOICE NUMBER': 220.0,
    'REFERENCE#': 120.0,
    'CUSTOMER NAME': 180.0,
    'PROJECT/ESTIMATE': 120.0,
    'STATUS': 100.0,
    'RETAINER DRAW STATUS': 180.0,
    'AMOUNT': 120.0,
    'BALANCE': 120.0,
    'ISSUED DATE': 120.0,
    'PROJECT': 120.0,
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(retainerInvoicesProvider.notifier).loadInvoices();
    });
  }
    ...
    filtered.sort((a, b) {
      ...
      switch (_sortByField) {
        ...
        case 'Last Modified Time':
          ...
          break;
        case 'Retainer Invoice Number':
          cmp = a.invoiceNo.compareTo(b.invoiceNo);
          break;
        case 'Reference#':
        ...
        case 'Total':
          ...
          break;
        case 'Balance':
          cmp = a.amountRemaining.compareTo(b.amountRemaining);
          break;
        default:
          cmp = a.date.compareTo(b.date);
      }
                ...
                children: [
                  ...
                  ElevatedButton.icon(
                    onPressed: () => context.go(
                      '/$_orgId${AppRoutes.salesRetainerInvoicesCreate}',
                    ),
                    icon: const Icon(
                      LucideIcons.plus,
                      size: 14,
                                ...
                                Expanded(
                                  child: ListView.separated(
                                    ...
                                    itemBuilder: (_, idx) {
                                      ...
                                      final isDraft =
                                          inv.status == RetainerStatus.draft;
                                                            ...
                                                            onPressed: () async {
                                                              final ok = await showZerpaiConfirmationDialog(
                                                                ...
                                                                title:
                                                                    'Mark as Paid',
                                                                message:
                                                                    'Mark ${inv.invoiceNo} as fully paid? This cannot be undone.',
                                                                confirmLabel:
                                                                    'Mark Paid',
                                                                variant:
                                                                    ZerpaiConfirmationVariant
                                                          ...
                                                          ZRowAction(
                                                            ...
                                                            onPressed: () async {
                                                              final ok = await showZerpaiConfirmationDialog(
                                                                context,
                                                                title:
                                                                    'Void Retainer Invoice',
                                                                message:
                                                                    'Are you sure you want to void ${inv.invoiceNo}? This action cannot be undone.',
                                                                confirmLabel:
                                                                    'Void Invoice',
                                                                variant:
                                                                    ZerpaiConfirmationVariant
  ...
  Widget _dateThHeader(List<RetainerInvoice> allInvoices) {
    final width = _columnWidths['DATE'] ?? 200.0;
    final allSelected =
        ...
        children: [
          ...
          Positioned(
            ...
            child: _ColumnResizer(
              width: width,
              onDragStart: () => setState(() => _activeDraggingKey = 'DATE'),
              onDragEnd: () => setState(() => _activeDraggingKey = null),
              onResize: (newWidth) {
                setState(() {
                  _columnWidths['DATE'] = newWidth;
                });
              },
            ),
          ),
  ...
  Widget _dateTdCell(RetainerInvoice inv, DateFormat dateF) {
    final width = _columnWidths['DATE'] ?? 200.0;
    return Container(
      width: width,
    ...
    return MenuAnchor(
      ...
      menuChildren: [
        SubmenuButton(
          ...
          menuChildren: [
            _buildSortSubmenuItem('Date'),
            _buildSortSubmenuItem('Retainer Invoice Number'),
            _buildSortSubmenuItem('Reference#'),
            _buildSortSubmenuItem('Customer Name'),
            _buildSortSubmenuItem('Total'),
            _buildSortSubmenuItem('Balance'),
            _buildSortSubmenuItem('Issued Date'),
            _buildSortSubmenuItem('Created Time'),
            _buildSortSubmenuItem('Last Modified Time'),
          ],
          child: const Text('Sort by', style: TextStyle(fontSize: 13)),
        ),
        MenuItemButton(
          onPressed: () {
            ZerpaiToast.success(context, 'Import Retainer Invoices clicked');
          },
          style: _menuItemButtonStyle(),
