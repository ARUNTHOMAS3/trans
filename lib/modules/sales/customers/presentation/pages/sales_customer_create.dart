...
class _SalesCustomerCreateScreenState
    ...
    with
        ...
        LicenceValidationMixin<SalesCustomerCreateScreen> {
  ...
  bool get _isEditMode =>
      widget.initialCustomer != null || widget.customerId != null;
  String? get _editingCustomerId =>
      widget.customerId ?? widget.initialCustomer?.id;
  String get _draftKey =>
      _editingCustomerId == null ? 'create' : 'edit:${_editingCustomerId!}';
  ...
  void initState() {
    ...
    _tabController = TabController(
      length: customerType == 'Individual'
          ? _individualTabKeys.length
          : _businessTabKeys.length,
      vsync: this,
  ...
  bool get _isBranchScopedRole {
    final user = ref.watch(authUserProvider);
    if (user == null) return false;
    final role = user.role.trim().toLowerCase();
    final tenantType = (user.activeTenantType ?? '').trim().toLowerCase();
    if (tenantType == 'branch') return true;
  ...
  String _buildErrorToastMessage([Map<String, String>? source]) {
    final entries = (source ?? fieldErrors).entries
  ...
  void _validateEmailField([String? value]) {
    ...
    if (enablePortal && email.isEmpty) {
      _setFieldError(
        'email',
        'Email is required when portal access is enabled',
      );
    ...
    try {
      ...
      if (mounted) {
        ...
        if (backendFieldErrors.isNotEmpty) {
          ...
          ZerpaiToast.error(
            context,
            _buildErrorToastMessage(backendFieldErrors),
          );
        } else {
      ...
      if (mounted) {
        setState(() {
          ...
          if (currencies.isNotEmpty) {
            final selectedCurrencyId = widget.initialCustomer?.currencyId
                ?.trim();
            final matchedCurrency =
                selectedCurrencyId == null || selectedCurrencyId.isEmpty
                ? null
    ...
    try {
      ...
      setState(() {
        ...
        if (!hasSelectedTerm && paymentTerms.trim().isNotEmpty) {
          _paymentTermsList = [
            ..._paymentTermsList,
            <String, dynamic>{'id': paymentTerms, 'term_name': paymentTerms},
          ];
        }
      });
    } catch (e) {
