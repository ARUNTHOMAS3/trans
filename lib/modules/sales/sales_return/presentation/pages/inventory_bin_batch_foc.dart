...
class PicklistBatchRowController {
  ...
  // The controllers above hold display text (bin code, batch ref). These hold
  // the corresponding row ids, which is what `sales_return_receive_batches`
  // stores — `batch_id`, `bin_id` and `warehouse_id` are NOT NULL uuids there.
  String? binId;
  String? batchId;
  String? layerId;
  ...
  void initState() {
    ...
    if (widget.savedBatchData != null && widget.savedBatchData!.isNotEmpty) {
      for (var batchData in widget.savedBatchData!) {
        ...
        // Restore the ids too, or reopening and re-saving a batch would drop
        // them and the receive insert would fail on its NOT NULL uuids.
        row.binId = (batchData['binId'] ?? '').isEmpty
            ? null
            : batchData['binId'];
        row.batchId = (batchData['batchId'] ?? '').isEmpty
            ? null
            : batchData['batchId'];
        row.layerId = (batchData['layerId'] ?? '').isEmpty
            ? null
            : batchData['layerId'];
                                          ...
                                          child: FormDropdown<String>(
                                            ...
                                            onChanged: (val) {
                                              setState(() {
                                                ...
                                                // Keep the id alongside the code —
                                                // the receive row stores bin_id.
                                                row.binId = (binsAsync.value ?? [])
                                                    .where((b) =>
                                                        (b['bin_code'] ?? '')
                                                            .toString() ==
                                                        val)
                                          ...
                                          onChanged: (v) {
                                            setState(() {
                                              if (v != null) {
                                                ...
                                                // batch_master.id — stored as
                                                // batch_id on the receive row.
                                                row.batchId =
                                                    v['id']?.toString();
                                                // Optional stock layer this
                                                // batch's pricing came from.
                                                row.layerId =
                                                    v['layer_id']?.toString();
                  ...
                  ElevatedButton(
                    onPressed: () {
                      ...
                      final batchDataList = _rows
                          .map(
                            (row) => {
                              // Ids first — these are what the receive rows
                              // persist; the text fields below are display only.
                              'binId': row.binId ?? '',
                              'batchId': row.batchId ?? '',
                              if (row.layerId != null &&
                                  row.layerId!.isNotEmpty)
                                'layerId': row.layerId!,
