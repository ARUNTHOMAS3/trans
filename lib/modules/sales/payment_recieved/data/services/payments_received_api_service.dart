...
class PaymentsReceivedApiService {
  ...
  Future<List<InvoiceCustomerOption>> getInvoiceCustomers() async {
    ...
    if (response.statusCode != 200) {
      throw Exception('Failed to load invoice customers (${response.statusCode})');
    }
  ...
  Future<List<Map<String, String>>> getUnpaidInvoices(String customerId) async {
    ...
    if (response.statusCode != 200) {
      throw Exception('Failed to load unpaid invoices (${response.statusCode})');
    }
    final body = response.data;
    final List<dynamic> rows = body is List
        ? body
    ...
    return rows.whereType<Map>().map((r) {
      ...
      return <String, String>{
        ...
        'invoiceAmount':
            money.format(double.tryParse((m['invoice_amount'] ?? 0).toString()) ?? 0),
        'amountDue':
            money.format(double.tryParse((m['amount_due'] ?? 0).toString()) ?? 0),
      };
    }).toList();
  }
