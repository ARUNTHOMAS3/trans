...
final salesCustomerByIdProvider = FutureProvider.family<SalesCustomer, String>((
  ref,
  customerId,
) async {
  final apiService = ref.watch(salesOrderApiServiceProvider);
...
final salesCustomerDetailContextProvider =
    FutureProvider.family<SalesCustomerDetailContext, String>((
      ref,
      customerId,
    ) async {
      final apiService = ref.watch(salesOrderApiServiceProvider);
      return apiService.getCustomerDetailContext(customerId);
    });
...
final salesInvoicesProvider = FutureProvider<List<SalesOrder>>((ref) async {
  final invoices = await ref.watch(salesOrderApiServiceProvider).getInvoices();
  return invoices.map(SalesOrder.fromJson).toList();
});
...
class SalesOrderController extends StateNotifier<AsyncValue<List<SalesOrder>>> {
  ...
  void deleteOrdersLocally(List<String> ids) {
    if (state.hasValue) {
      ...
      final newList = currentList.where((order) => !ids.contains(order.id)).toList();
  ...
  Future<SalesCustomer> updateCustomer(String id, Map<String, dynamic> data) async {
    try {
      final updatedCustomer = await _apiService.updateCustomer(id, data);
      // Refresh the customers provider so the UI updates
      ref.invalidate(salesCustomersProvider);
      ref.invalidate(salesCustomerByIdProvider(id));
      ref.invalidate(salesCustomerDetailContextProvider(id));
  ...
  Future<Map<String, dynamic>> bulkUpdateCustomers(
    ...
    try {
      final result = await _apiService.bulkUpdateCustomers(customerIds, data);
      ref.invalidate(salesCustomersProvider);
