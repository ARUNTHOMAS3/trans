...
class BranchPriceListAddScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<BranchPriceListAddScreen> createState() => _BranchPriceListAddScreenState();
}

class _BranchPriceListAddScreenState extends ConsumerState<BranchPriceListAddScreen> {
  ...
  String _itemKey(Item item, int index) {
    final key = item.id ?? item.sku ?? item.itemCode ?? item.productName;
    final trimmed = key.trim();
    return trimmed.isEmpty ? 'item-$index' : trimmed;
  }

  String _itemName(Item item) {
    final name = item.productName ?? item.sku ?? item.itemCode;
