...
class BranchPriceListNotifier
  ...
  BranchPriceListNotifier(this._service, {required bool isAuthenticated})
    : super(
        isAuthenticated
            ...
            : const AsyncValue.data(<BranchPriceList>[]),
      ) {
    if (isAuthenticated) {
  ...
  Future<void> createBranchPriceList(
    BranchPriceList priceList, {
    List<String> branchEntityIds = const [],
  }) async {
    try {
      final createdPriceList = await _service.createBranchPriceList(
        priceList,
        branchEntityIds: branchEntityIds,
      );
      state.whenData((priceLists) {
  ...
  Future<void> updateBranchPriceList(
    BranchPriceList priceList, {
    List<String> branchEntityIds = const [],
  }) async {
    try {
      final updatedPriceList = await _service.updateBranchPriceList(
        priceList,
        branchEntityIds: branchEntityIds,
      );
      state.whenData((priceLists) {
