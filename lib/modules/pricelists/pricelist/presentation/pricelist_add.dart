...
const double _priceListFieldWidth = 320;
const double _priceListTypeCardWidth = 240;
const double _priceListLabelWidth = 150;
const double _priceListFieldHeight = 34;
const double _priceListRowGap = 14;
const double _pricingSchemeCardGap = 12;
const double _pricingSchemeCardWidth =
    (_priceListFieldWidth - _pricingSchemeCardGap) / 2;
...
class PriceListAddScreen extends ConsumerStatefulWidget {
  final PriceList? template;
  const PriceListAddScreen({super.key, this.template});

  @override
  ConsumerState<PriceListAddScreen> createState() => _PriceListAddScreenState();
}

class _PriceListAddScreenState extends ConsumerState<PriceListAddScreen> {
  static const String _listRoute = '/price-lists';
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            SizedBox(width: _priceListLabelWidth),
            Flexible(
              fit: FlexFit.loose,
              child: Row(
                mainAxisSize: MainAxisSize.min,
  ...
  List<PriceListItemRate> _buildItemRates(List<Item> items) {
    if (_pricingScheme == 'volume_pricing') {
      return _buildVolumeItemRates(items);
    }

    final rates = <PriceListItemRate>[];
  ...
  List<PriceListItemRate> _buildVolumeItemRates(List<Item> items) {
    final rates = <PriceListItemRate>[];
    for (var index = 0; index < items.length; index++) {
      final item = items[index];
      final itemId = _itemKey(item, index);
      final ranges = <PriceListVolumeRange>[];
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            SizedBox(width: _priceListLabelWidth, child: labelWidget),
