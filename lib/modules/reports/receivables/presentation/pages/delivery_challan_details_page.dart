...
import 'package:zerpai_erp/modules/reports/receivables/presentation/widgets/delivery_challan_details_table.dart';
...
const String _deliveryChallanDetailsTitle = 'Delivery Challan Details';
const String _deliveryChallanDetailsBasis = 'Accrual';
const List<String> _deliveryChallanGroupByOptions = <String>[
  'Date',
  'Customer Name',
  'Currency',
];

typedef DeliveryChallanDetailsParams = ({
  String startDate,
  String endDate,
  String basis,
  int page,
...
final deliveryChallanDetailsProvider = FutureProvider.family<
    Map<String, dynamic>, DeliveryChallanDetailsParams>((ref, params) async {
  final repo = ref.watch(reportsRepositoryProvider);
  return repo.getDeliveryChallanDetails(
    params.startDate,
    params.endDate,
    basis: params.basis,
    page: params.page,
...
class DeliveryChallanDetailsPage extends ConsumerStatefulWidget {
  const DeliveryChallanDetailsPage({super.key});

  @override
  ConsumerState<DeliveryChallanDetailsPage> createState() =>
      _DeliveryChallanDetailsPageState();
}

class _DeliveryChallanDetailsPageState
    extends ConsumerState<DeliveryChallanDetailsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final queryParams = (
      ...
      basis: _deliveryChallanDetailsBasis,
      page: _page,
      pageSize: _pageSize,
    );
    final reportAsync = ref.watch(deliveryChallanDetailsProvider(queryParams));
    final reportData = reportAsync.valueOrNull;
    final rows = DeliveryChallanDetailsRow.fromResponse(reportData, dateFormat);
    final totals = DeliveryChallanDetailsTotals.fromResponse(reportData);
    ...
    return ReportViewScaffold(
      ...
      reportTitle: _deliveryChallanDetailsTitle,
      dateLabel: dateLabel,
      companyName: '',
      reportHeading: _DeliveryChallanDetailsHeading(dateLabel: dateLabel),
      filters: [
      ...
      onSchedule: () => ReportScheduleDialog.show(
        context,
        reportName: _deliveryChallanDetailsTitle,
      ),
      onReload: _runReport,
      ...
      settingsTooltip: 'Customize the Delivery Challan Details report.',
      scheduleTooltip: 'Schedule the Delivery Challan Details report.',
      tableHeaderActions: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          ReportGroupBySection(
            selectedValue: _groupBy,
            options: _deliveryChallanGroupByOptions,
      ...
      errorMessage: reportAsync.hasError
          ...
          : null,
      onRetry: () => ref.invalidate(deliveryChallanDetailsProvider(queryParams)),
      isEmpty: false,
      emptyTitle: 'No data to display',
      emptyMessage: 'No data to display',
      currentNavigationCategory: 'Receivables',
      currentNavigationReport: _deliveryChallanDetailsTitle,
      onReportSelected: (reportName, category) {
        if (reportName == _deliveryChallanDetailsTitle) return;
        openReportFromReportsModule(context, reportName, category: category);
      },
      reportContent: DeliveryChallanDetailsTable(
        rows: rows,
        totals: totals,
...
class _DeliveryChallanDetailsHeading extends StatelessWidget {
  final String dateLabel;

  const _DeliveryChallanDetailsHeading({required this.dateLabel});
