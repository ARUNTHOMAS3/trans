...
import 'package:zerpai_erp/modules/reports/inventory/presentation/widgets/stock_movement_table.dart';
...
class StockMovementReportData {
  ...
  const StockMovementReportData({required this.rows, required this.totals});
}

final stockMovementRowsProvider = FutureProvider.autoDispose
    .family<StockMovementReportData, int>((ref, refreshKey) async {
      final repository = ref.watch(reportsRepositoryProvider);
      final response = await repository.getStockMovement(
        startDate: '2026-07-01',
        endDate: '2026-07-31',
        limit: 500,
      );
      final rows = List<Map<String, dynamic>>.from(response['data'] ?? const [])
          .map(StockMovementRow.fromJson)
          .where((row) => row.transactionDate.trim().isNotEmpty)
          .toList(growable: false);
      final meta = response['meta'] is Map
          ? Map<String, dynamic>.from(response['meta'] as Map)
          : const <String, dynamic>{};
      final totals = meta['totals'] is Map
          ? StockMovementRow.fromTotals(
              Map<String, dynamic>.from(meta['totals'] as Map),
            )
          : StockMovementRow.totalFromRows(rows);
      return StockMovementReportData(rows: rows, totals: totals);
    });
...
class _StockMovementPageState extends ConsumerState<StockMovementPage> {
  ...
  void _clearPendingAfterSuccessfulLoad(
    AsyncValue<StockMovementReportData> rowsAsync,
  ) {
  ...
  Widget build(BuildContext context) {
    final rowsAsync = ref.watch(stockMovementRowsProvider(_refreshKey));
    _clearPendingAfterSuccessfulLoad(rowsAsync);
    final reportData = rowsAsync.valueOrNull;
    final rows = reportData?.rows ?? const <StockMovementRow>[];
    final totals = reportData?.totals ?? StockMovementRow.totalFromRows(rows);
    ...
    return ReportViewScaffold(
      ...
      showSchedule: true,
      onSchedule: () =>
          ReportScheduleDialog.show(context, reportName: 'Stock Movement'),
      onReload: _runReport,
      ...
      settingsTooltip: 'Customize the Stock Movement report.',
      scheduleTooltip: 'Schedule the Stock Movement report.',
      tableHeaderActions: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          _StockMovementGroupBySection(
      ...
      emptyTitle: 'No stock movement rows found',
      emptyMessage:
          'There are no stock movement rows for the selected criteria.',
      currentNavigationCategory: 'Inventory',
      currentNavigationReport: 'Stock Movement',
      onReportSelected: (reportName, category) {
        if (reportName == 'Stock Movement') return;
        openReportFromReportsModule(context, reportName);
      },
      reportContent: StockMovementTable(
        rows: rows,
        totals: totals,
        page: _page,
        pageSize: _pageSize,
...
class _StockMovementGroupBySection extends StatefulWidget {
  ...
  @override
  State<_StockMovementGroupBySection> createState() =>
      _StockMovementGroupBySectionState();
}

class _StockMovementGroupBySectionState
    extends State<_StockMovementGroupBySection> {
  ...
  @override
  void didUpdateWidget(covariant _StockMovementGroupBySection oldWidget) {
