...
import 'package:zerpai_erp/modules/reports/inventory/presentation/widgets/assembly_details_table.dart';
...
typedef AssemblyDetailsRequest = ({
  int refreshKey,
  DateTime startDate,
  DateTime endDate,
});

final assemblyDetailsRowsProvider = FutureProvider.autoDispose
    .family<List<AssemblyDetailsRow>, AssemblyDetailsRequest>((
      ref,
      request,
    ) async {
      final repository = ref.watch(reportsRepositoryProvider);
      final response = await repository.getAssemblyDetails(
      ...
      if (rows.isEmpty) return const <AssemblyDetailsRow>[];
      return rows
          .map(AssemblyDetailsRow.fromJson)
          .where((row) => row.assemblyName.trim().isNotEmpty)
          .toList(growable: false);
    });

class AssemblyDetailsPage extends ConsumerStatefulWidget {
  const AssemblyDetailsPage({super.key});

  @override
  ConsumerState<AssemblyDetailsPage> createState() =>
      _AssemblyDetailsPageState();
}

class _AssemblyDetailsPageState extends ConsumerState<AssemblyDetailsPage> {
  ...
  void _clearPendingAfterSuccessfulLoad(
    AsyncValue<List<AssemblyDetailsRow>> rowsAsync,
  ) {
  ...
  Widget build(BuildContext context) {
    ...
    final rows = rowsAsync.valueOrNull ?? const <AssemblyDetailsRow>[];

    return ReportViewScaffold(
      categoryLabel: 'Inventory',
      reportTitle: 'Assembly Details Report',
      ...
      onSchedule: () => ReportScheduleDialog.show(
        context,
        reportName: 'Assembly Details Report',
      ),
      onReload: _runReport,
      ...
      settingsTooltip: 'Customize the Assembly Details report.',
      scheduleTooltip: 'Schedule the Assembly Details report.',
      tableHeaderActions: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          _AssemblyDetailsGroupBySection(
      ...
      emptyTitle: 'No assembly details found',
      emptyMessage: 'No data to display',
      currentNavigationCategory: 'Inventory',
      currentNavigationReport: 'Assembly Details',
      onReportSelected: (reportName, category) {
        if (reportName == 'Assembly Details') return;
        openReportFromReportsModule(context, reportName);
      },
      reportContent: AssemblyDetailsTable(
        rows: rows,
        page: _page,
        pageSize: _pageSize,
...
class _AssemblyDetailsGroupBySection extends StatefulWidget {
  ...
  @override
  State<_AssemblyDetailsGroupBySection> createState() =>
      _AssemblyDetailsGroupBySectionState();
}

class _AssemblyDetailsGroupBySectionState
    extends State<_AssemblyDetailsGroupBySection> {
  ...
  @override
  void didUpdateWidget(covariant _AssemblyDetailsGroupBySection oldWidget) {
