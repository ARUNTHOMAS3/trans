...
import 'package:zerpai_erp/modules/reports/payables/presentation/widgets/payable_summary_table.dart';
...
const String _reportTitle = 'Payable Summary';

class PayableSummaryRow {
  final String status;
  final DateTime date;
  ...
  final double totalBcy;
  final double balanceBcy;
...
final payableSummaryRowsProvider =
    FutureProvider.family<List<PayableSummaryRow>, PayableSummaryParams>((
  ref,
  params,
) async {
  final repo = ref.watch(reportsRepositoryProvider);
  final List<Future<List<PayableSummaryRow>>> futures = [];
  ...
  if (params.entities.contains('Bill')) {
    futures.add(repo.getBillDetails(
      ...
      return rows.map((json) {
        return PayableSummaryRow(
          ...
          date: parseDateTime(json['billDateRaw'] ?? json['billDate']),
          transactionNumber: json['billNumber']?.toString() ?? '',
          vendorName: json['vendorName']?.toString() ?? '',
          transactionType: 'Bill',
          customerName: '-',
          totalBcy: toDouble(json['billAmount']),
          balanceBcy: toDouble(json['balanceAmount']),
          currencyCode: 'INR',
          createdBy: '-',
          dueDate: parseDateTimeNullable(json['dueDateRaw'] ?? json['dueDate']),
        );
      }).toList();
    }));
  }

  if (params.entities.contains('Vendor Credits')) {
    ...
    futures.add(repo.getExpenseDetails(
      ...
      return rows.map((json) {
        return PayableSummaryRow(
          ...
          totalBcy: toDouble(json['amountWithTaxValue'] ?? json['amountValue']),
          balanceBcy: 0.0,
          currencyCode: json['currencyCode']?.toString() ?? 'INR',
          createdBy: json['createdByName']?.toString() ?? '-',
        );
      }).toList();
    }));
  }
...
class PayableSummaryPage extends ConsumerStatefulWidget {
  const PayableSummaryPage({super.key});

  @override
  ConsumerState<PayableSummaryPage> createState() => _PayableSummaryPageState();
}

class _PayableSummaryPageState extends ConsumerState<PayableSummaryPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final rowsAsync = ref.watch(payableSummaryRowsProvider(params));
    final rows = rowsAsync.valueOrNull ?? const <PayableSummaryRow>[];
    ...
    return ReportViewScaffold(
      ...
      settingsTooltip: 'Customize the Payable Summary report.',
      tableHeaderActions: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          _PayableSummaryGroupBySection(
      ...
      currentNavigationReport: 'Payable Summary',
      onReportSelected: (reportName, category) {
        if (reportName == 'Payable Summary' && category == 'Payables') {
          return;
        }
        openReportFromReportsModule(context, reportName, category: category);
      },
      reportContent: PayableSummaryTable(
        rows: rows,
        page: _page,
        pageSize: _pageSize,
...
class _PayableSummaryGroupBySection extends StatefulWidget {
  ...
  const _PayableSummaryGroupBySection({
  ...
  @override
  State<_PayableSummaryGroupBySection> createState() =>
      _PayableSummaryGroupBySectionState();
}

class _PayableSummaryGroupBySectionState
    extends State<_PayableSummaryGroupBySection> {
  ...
  @override
  void didUpdateWidget(covariant _PayableSummaryGroupBySection oldWidget) {
