...
import 'package:zerpai_erp/modules/reports/purchases_expenses/presentation/widgets/expenses_by_employee_table.dart';
import 'package:zerpai_erp/modules/reports/repositories/reports_repository.dart';

typedef ExpensesByEmployeeRequest = ({
  int refreshKey,
  DateTime startDate,
  DateTime endDate,
});

final expensesByEmployeeRowsProvider = FutureProvider.autoDispose
    .family<List<ExpensesByEmployeeRow>, ExpensesByEmployeeRequest>((
      ref,
      request,
    ) async {
      final repository = ref.watch(reportsRepositoryProvider);
      final response = await repository.getExpensesByEmployee(
      ...
      return rows.map(ExpensesByEmployeeRow.fromJson).toList(growable: false);
    });

class ExpensesByEmployeePage extends ConsumerStatefulWidget {
  const ExpensesByEmployeePage({super.key});

  @override
  ConsumerState<ExpensesByEmployeePage> createState() =>
      _ExpensesByEmployeePageState();
}

class _ExpensesByEmployeePageState
    extends ConsumerState<ExpensesByEmployeePage> {
  static const String _reportTitle = 'Expenses by Employee';
  ...
  void _clearPendingAfterSuccessfulLoad(
    AsyncValue<List<ExpensesByEmployeeRow>> rowsAsync,
  ) {
  ...
  Widget build(BuildContext context) {
    ...
    final rowsAsync = ref.watch(
      expensesByEmployeeRowsProvider((
    ...
    final rows = rowsAsync.valueOrNull ?? const <ExpensesByEmployeeRow>[];
    ...
    return ReportViewScaffold(
      ...
      settingsTooltip: 'Customize the Expenses by Employee report.',
      tableHeaderActions: const ReportCustomizeColumnsButton(count: 5),
      isLoading: rowsAsync.isLoading && !rowsAsync.hasValue,
      errorMessage: rowsAsync.hasError ? rowsAsync.error.toString() : null,
      onRetry: _runReport,
      isEmpty: rows.isEmpty,
      emptyTitle: 'No employee expense rows found',
      ...
      reportContent: ExpensesByEmployeeTable(
        rows: rows,
        page: _page,
        pageSize: _pageSize,
