...
class ChartOfAccountsNotifier extends StateNotifier<ChartOfAccountsState> {
  ...
  ChartOfAccountsNotifier(this._repository, {required bool isAuthenticated})
  ...
  Future<void> performBackendSearch(String query) async {
    if (!mounted) return;
    if (query.isEmpty) {
      state = state.copyWith(isBackendSearch: false, backendSearchResults: []);
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      state = state.copyWith(isLoading: false, backendSearchResults: results);
    } catch (e) {
      if (!mounted) return;
      state = state.copyWith(isLoading: false);
    }
  ...
  void selectAccount(String? id) async {
    ...
    try {
      ...
      if (!mounted) return;
      final recentTransactions = (results[0] as List<AccountTransaction>)
