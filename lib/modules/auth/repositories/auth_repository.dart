...
class AuthRepository {
  ...
  static const _selectedEntityNameKey = 'selected_entity_name';
  static const _selectedEntityTypeKey = 'selected_entity_type';
  static const _selectedOrgIdKey = 'selected_org_id';
  static const _selectedBranchIdKey = 'selected_branch_id';
  ...
  Future<void> _clearStoredData() async {
    try {
      ...
      await _box.delete(_selectedEntityNameKey);
      await _box.delete(_selectedEntityTypeKey);
      await _box.delete(_selectedOrgIdKey);
      await _box.delete(_selectedBranchIdKey);
    } catch (e) {
  ...
  Future<void> setSelectedTenant({
    ...
    await _box.put(_selectedTenantIdKey, normalizedId);
    await _box.put(_selectedTenantTypeKey, normalizedType);
    ...
    if (entityName != null && entityName.trim().isNotEmpty) {
      await _box.put(_selectedEntityNameKey, entityName.trim());
    } else {
      await _box.delete(_selectedEntityNameKey);
    }
    if (normalizedType.isNotEmpty) {
      await _box.put(_selectedEntityTypeKey, normalizedType);
    } else {
      await _box.delete(_selectedEntityTypeKey);
    }
    ...
    if (orgId != null && orgId.trim().isNotEmpty) {
      await _box.put(_selectedOrgIdKey, orgId.trim());
    } else {
      await _box.delete(_selectedOrgIdKey);
    }
    if (branchId != null && branchId.trim().isNotEmpty) {
      await _box.put(_selectedBranchIdKey, branchId.trim());
    } else {
      await _box.delete(_selectedBranchIdKey);
    }
  }
