...
class _BranchRow {
  ...
  factory _BranchRow.fromJson(Map<String, dynamic> j) => _BranchRow(
    ...
    parentBranchId: j['parent_branch_id']?.toString(),
...
class _SettingsLocationsPageState extends ConsumerState<SettingsLocationsPage> {
  ...
  Map<String, int> _zoneCounts = <String, int>{};
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final gstTypesRes = await _apiClient.get(
      ...
      // Load branches
      final branchesRes = await _apiClient.get(
        '/branches',
        queryParameters: <String, dynamic>{'org_id': orgId},
      );
      if (!mounted) return;
      final List<dynamic> rows = branchesRes.success && branchesRes.data is List
          ? branchesRes.data as List<dynamic>
          : <dynamic>[];
      final parsedBranches = rows
          .whereType<Map<String, dynamic>>()
          .map(_BranchRow.fromJson)
          .toList();
      final zoneCounts = await BinLocationsService.instance.getZoneCounts(
        orgId: orgId,
        branchIds: parsedBranches.map((branch) => branch.id),
      );
      setState(() {
        _branches = parsedBranches;
        _zoneCounts = zoneCounts;
        _gstRegistrationTypeOptions = gstTypeOptions;
        _isLoading = false;
      });
    } catch (_) {
      if (!mounted) return;
      setState(() {
        _isLoading = false;
        _branches = <_BranchRow>[];
        _zoneCounts = <String, int>{};
      });
    }
  }
        ...
        children: [
          ...
          if (_isLoading)
            const Padding(
              ...
              child: ZTableSkeleton(rows: 8, columns: 4),
            )
          else if (_error != null)
            _buildErrorState()
  ...
  List<Widget> _buildTreeRows() {
    final parents = _branches
        .where((o) => o.parentBranchId == null || o.parentBranchId!.isEmpty)
        .toList();
    final childMap = <String, List<_BranchRow>>{};
    for (final o in _branches) {
      if (o.parentBranchId != null && o.parentBranchId!.isNotEmpty) {
        childMap.putIfAbsent(o.parentBranchId!, () => []).add(o);
      }
    }
    final rows = <Widget>[];
  ...
  Widget _buildTableRow(
    ...
    final bool isHovered = _hoveredBranchId == branch.id;
    final Color accentColor = ref.watch(appBrandingProvider).accentColor;
    ...
    Widget statusDot = Container(
      ...
      decoration: BoxDecoration(
        ...
        color: branch.isActive ? const Color(0xFF22A95E) : Colors.transparent,
        border: Border.all(
    ...
    return MouseRegion(
      onEnter: (_) => setState(() => _hoveredBranchId = branch.id),
      onExit: (_) => setState(() => _hoveredBranchId = null),
      child: Container(
        padding: const EdgeInsets.symmetric(
          horizontal: AppTheme.space20,
        ...
        child: Row(
          children: [
            ...
            Expanded(
              flex: 1,
              child: Text(
                branch.isWarehouse ? 'Warehouse' : 'Business',
                style: const TextStyle(fontSize: 13, color: AppTheme.textBody),
              ),
            ),
            ...
            Expanded(
              flex: 1,
              child: Text(
                _zoneCounts[branch.id] != null && _zoneCounts[branch.id]! > 0
                    ? '${_zoneCounts[branch.id]} Zones'
                    : '—',
                style: const TextStyle(
                  fontSize: 13,
                  color: AppTheme.textSecondary,
              ...
              child: Row(
                ...
                children: [
                  PopupMenuButton<String>(
                    ...
                    itemBuilder: (_) => _buildMenuItems(branch),
  ...
  List<PopupMenuEntry<String>> _buildMenuItems(_BranchRow branch) {
    return <PopupMenuEntry<String>>[
      const PopupMenuItem(value: 'edit', child: Text('Edit')),
      if (!branch.isWarehouse && branch.gstin.isEmpty)
        const PopupMenuItem(
      ...
      if (branch.isActive && branch.isWarehouse)
        const PopupMenuItem(
          value: 'mark_inactive',
  ...
  Future<void> _onMenuSelected(String value, _BranchRow branch) async {
    ...
    switch (value) {
      ...
      case 'mark_active':
      case 'mark_inactive':
        ZerpaiToast.info(context, 'Location status update is coming soon');
        break;
      case 'bin_locations':
      ...
      case 'associate_gstin':
        _showAssociateGstinDialog(branch);
        break;
                            ...
                            onPressed: () async {
                              ...
                              try {
                                final res = await _apiClient.patch(
                                  '/branches/${branch.id}/contacts?org_id=$orgId',
                                  data: {
  ...
  Future<void> _confirmDelete(_BranchRow branch) async {
    ...
    try {
      final res = await _apiClient.delete(
        '/branches/${branch.id}?org_id=$orgId',
      );
      if (!mounted) return;
      if (res.success) {
  ...
  Future<void> _showAssociateGstinDialog(_BranchRow branch) async {
    ...
    final existingGstins = _branches
        .where((o) => o.id != branch.id && o.gstin.isNotEmpty)
        .map((o) => o.gstin)
        .toSet()
        .toList();
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Text(
                            'Associate GSTIN to ${branch.name}',
                            style: const TextStyle(
                              fontSize: 16,
                              fontWeight: FontWeight.w700,
                              color: AppTheme.textPrimary,
