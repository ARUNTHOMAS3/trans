...
class _SettingsOrganizationBrandingPageState
  ...
  static const List<_AccentOption> _accentOptions = <_AccentOption>[
    _AccentOption(label: 'Green', color: AppTheme.successGreen),
    _AccentOption(label: 'Blue', color: AppTheme.infoBlue),
    _AccentOption(label: 'Purple', color: Color(0xFF8B5CF6)),
    _AccentOption(label: 'Red', color: AppTheme.errorRed),
    _AccentOption(label: 'Orange', color: AppTheme.warningOrange),
  ];
  ...
  Color _selectedAccentColor = AppTheme.successGreen;
  ...
  Future<void> _loadBranding() async {
    ...
    try {
      ...
      if (orgId.isEmpty) {
        if (!mounted) return;
        setState(() {
          _organizationName = user?.orgName.trim() ?? '';
          _isLoading = false;
        });
        return;
      }
  ...
  Future<void> _uploadLogo() async {
    ...
    final String orgId = _orgId.isNotEmpty
        ? _orgId
        : (ref.read(authUserProvider)?.orgId.trim() ?? '');
    if (orgId.isEmpty) {
      if (mounted) {
        ZerpaiToast.error(
          context,
