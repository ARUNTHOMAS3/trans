...
class _SettingsBranchCreatePageState
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final normalizedPhone = _phoneCtrl.text
          .replaceAll(RegExp(r'[^0-9]'), '')
          .trim();
      final normalizedPaymentStubPhone = _paymentStubPhoneController.text
          .replaceAll(RegExp(r'[^0-9]'), '')
          .trim();
      ...
      final body = <String, dynamic>{
        ...
        if (_hasSeparatePaymentStubAddress)
          'payment_stub_address': jsonEncode({
            ...
            'phone': normalizedPaymentStubPhone,
      ...
      if (res.success) {
        ...
        final orgSystemId = match?.group(1) ?? resolveOrgSystemId(context);
        context.go('/$orgSystemId${AppRoutes.settingsBranches}');
      } else {
        final message = (res.message?.isNotEmpty == true)
            ? res.message!
            : 'Failed to save branch.';
        final isUniqueConstraintError =
            message.toLowerCase().contains('duplicate key value') ||
            message.toLowerCase().contains('unique constraint');
        if (!_isEditing && isUniqueConstraintError) {
      ...
      final parsedError = ZerpaiBuilders.parseErrorMessage(e, 'branch');
      final isUniqueConstraintError =
          parsedError.toLowerCase().contains('duplicate key value') ||
          parsedError.toLowerCase().contains('unique constraint');

      if (mounted && !_isEditing && isUniqueConstraintError) {
        final reconciled = await _reconcileCreatedBranchAfterDuplicateError();
        if (reconciled) return;
      }

      if (mounted) {
        ZerpaiToast.error(context, parsedError);
      }
    } finally {
  ...
  Future<bool> _reconcileCreatedBranchAfterDuplicateError() async {
    final apiClient = ref.read(apiClientProvider);
    final user = ref.read(authUserProvider);
    final orgId = (user?.orgId.isNotEmpty == true) ? user!.orgId : '';
    final branchCode = _branchCodeCtrl.text.trim().toUpperCase();
    final branchName = _nameCtrl.text.trim();
    final branchEmail = _emailCtrl.text.trim().toLowerCase();
    ...
    try {
      ...
      final rows = (checkRes.success && checkRes.data is List)
      ...
      final createdExists = rows.any((row) {
        ...
        final createdRecently =
            rowCreatedAt != null &&
            DateTime.now().difference(rowCreatedAt).inMinutes <= 5;
        final codeAndNameMatch = rowCode == branchCode && rowName == branchName;
        final nameAndEmailMatch =
            rowName == branchName &&
            branchEmail.isNotEmpty &&
            rowEmail == branchEmail;
        final recentNameMatch = rowName == branchName && createdRecently;
        return codeAndNameMatch || nameAndEmailMatch || recentNameMatch;
      });
      if (createdExists && mounted) {
        ZerpaiToast.success(context, 'Branch created successfully.');
        final uri = GoRouterState.of(context).uri;
        final match = RegExp(r'^/(\d{10,20})').firstMatch(uri.path);
        final orgSystemId = match?.group(1) ?? resolveOrgSystemId(context);
        context.go('/$orgSystemId${AppRoutes.settingsBranches}');
      ...
      // Fall through to regular error toast when reconciliation probe fails.
    }
    return false;
  }
