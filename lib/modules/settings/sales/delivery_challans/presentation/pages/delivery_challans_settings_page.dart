...
class _DeliveryChallanStatusRecord {
  ...
  _DeliveryChallanStatusRecord copyWith({
    String? customStatusFor,
    String? statusName,
    String? description,
    Color? labelColor,
  }) {
    return _DeliveryChallanStatusRecord(
...
class _StatusDialogResult {
  ...
  final _DeliveryChallanStatusRecord? record;
  final bool deleted;
}
...
class DeliveryChallansSettingsPage extends ConsumerStatefulWidget {
  const DeliveryChallansSettingsPage({
    super.key,
    this.initialTab = DeliveryChallansSettingsTab.preferences,
  });

  final DeliveryChallansSettingsTab initialTab;

  @override
  ConsumerState<DeliveryChallansSettingsPage> createState() =>
      _DeliveryChallansSettingsPageState();
}
...
class _DeliveryChallansSettingsPageState
  ...
  late DeliveryChallansSettingsTab _activeTab;
  ...
  List<SettingsSearchItem> _buildSearchItems() {
    return <SettingsSearchItem>[
      SettingsSearchItem(
        ...
        keywords: const <String>['delivery challans', 'preferences'],
        onSelected: () => _openTab(DeliveryChallansSettingsTab.preferences),
      ),
      SettingsSearchItem(
        group: 'Delivery Challans',
        label: 'Delivery Challan Cycle',
        subtitle: 'Delivery Challans',
        keywords: const <String>['cycle', 'rules', 'manual orders'],
        onSelected: () =>
            _openTab(DeliveryChallansSettingsTab.deliveryChallanCycle),
      ),
      SettingsSearchItem(
        group: 'Delivery Challans',
        label: 'Statuses',
        ...
        onSelected: () => _openTab(DeliveryChallansSettingsTab.statuses),
      ),
      SettingsSearchItem(
        group: 'Delivery Challans',
  ...
  void _openTab(DeliveryChallansSettingsTab tab) {
    final route = switch (tab) {
      DeliveryChallansSettingsTab.deliveryChallanCycle =>
        AppRoutes.settingsDeliveryChallansCycle,
      DeliveryChallansSettingsTab.statuses =>
        AppRoutes.settingsDeliveryChallansStatuses,
      _ => AppRoutes.settingsDeliveryChallans,
    };
    if (_activeTab != tab) {
  ...
  Widget build(BuildContext context) {
    ...
    return CallbackShortcuts(
      ...
      child: ColoredBox(
        ...
        child: Column(
          children: [
            _DeliveryChallansSettingsHeader(
              orgName: orgName,
                              ...
                              child: SingleChildScrollView(
                                ...
                                child: Align(
                                  ...
                                  child:
                                      _activeTab ==
                                          DeliveryChallansSettingsTab.statuses
                                      ? const _DeliveryChallansStatusesContent()
                                      : ConstrainedBox(
                                          ...
                                          child:
                                              _activeTab ==
                                                  DeliveryChallansSettingsTab
                                                      .deliveryChallanCycle
                                              ? const _DeliveryChallansCycleContent()
                                              : _DeliveryChallansPreferencesContent(
                                                  termsController:
                                                      _termsController,
...
class _DeliveryChallansPreferencesContent extends StatelessWidget {
...
class _DeliveryChallansCycleContent extends StatelessWidget {
  const _DeliveryChallansCycleContent();
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text(
          'Configure delivery challan cycle preference for manual delivery challans',
          style: AppTheme.bodyText.copyWith(
            fontSize: 15,
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      'Create delivery challan cycle rules for marketplace orders',
                      style: AppTheme.pageTitle.copyWith(
                        fontSize: 16,
                        fontWeight: FontWeight.w500,
                        color: const Color(0xFF20263A),
                    ...
                    const SizedBox(height: 10),
                    Text(
                      'Create delivery challan cycle rules for different marketplaces to create transactions like invoices, packages, and shipments automatically after an order is received from a marketplace.',
                      style: AppTheme.bodyText.copyWith(
                        fontSize: 14,
                        height: 1.5,
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              ...const [
                'You have to execute the delivery challan cycle from the delivery challan details page for orders that have been imported manually.',
                'The custom fields in transactions will be left empty, even if the custom fields are mandatory.',
                'If a delivery challan contains items with either serial number tracking or batch tracking, you cannot create packages or invoices while automating the delivery challan cycle.',
                'You have to disable any functions that might interfere with the delivery challan cycle. Contact support for more details.',
...
class _DeliveryChallansStatusesContent extends StatefulWidget {
  const _DeliveryChallansStatusesContent();

  @override
  State<_DeliveryChallansStatusesContent> createState() =>
      _DeliveryChallansStatusesContentState();
}

class _DeliveryChallansStatusesContentState
    extends State<_DeliveryChallansStatusesContent> {
  ...
  Future<void> _handleEditStatus(
    ...
    int index,
    _DeliveryChallanStatusRecord record,
  ) async {
    final result = await _showEditStatusDialog(context, record);
    if (result == null) return;

    final next = List<_DeliveryChallanStatusRecord>.from(
      _deliveryChallanStatusesNotifier.value,
    );
    if (result.deleted) {
      next.removeAt(index);
      _deliveryChallanStatusesNotifier.value = next;
      if (context.mounted) {
  ...
  @override
  Widget build(BuildContext context) {
    return ValueListenableBuilder<List<_DeliveryChallanStatusRecord>>(
      valueListenable: _deliveryChallanStatusesNotifier,
                ...
                Flexible(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          'Select the transactions that has to be created after a delivery challan.',
                          style: AppTheme.bodyText.copyWith(
                            fontSize: 12.5,
...
class _NewStatusDialog extends StatefulWidget {
  ...
  final String title;
  final _DeliveryChallanStatusRecord? initialRecord;
...
class _NewStatusDialogState extends State<_NewStatusDialog> {
  ...
  void _saveStatus() {
    ...
    Navigator.of(context).pop(
      _StatusDialogResult(
        record: _DeliveryChallanStatusRecord(
...
class _DeliveryChallansSettingsHeader extends StatelessWidget {
  const _DeliveryChallansSettingsHeader({
...
class _DeliveryChallansPageTitle extends StatelessWidget {
  const _DeliveryChallansPageTitle({this.trailing});
...
class _StatusesHeaderButton extends StatelessWidget {
  ...
  Future<void> _handleNewStatus(BuildContext context) async {
    ...
    _deliveryChallanStatusesNotifier.value = <_DeliveryChallanStatusRecord>[
      ..._deliveryChallanStatusesNotifier.value,
      result.record!,
    ];
    if (context.mounted) {
...
class _DeliveryChallansTabsRow extends StatelessWidget {
  const _DeliveryChallansTabsRow({
    required this.activeTab,
    required this.onTabSelected,
  });

  final DeliveryChallansSettingsTab activeTab;
  final ValueChanged<DeliveryChallansSettingsTab> onTabSelected;

  static const List<(String, DeliveryChallansSettingsTab)> _tabs =
      <(String, DeliveryChallansSettingsTab)>[
        ('Preferences', DeliveryChallansSettingsTab.preferences),
        ('Fields', DeliveryChallansSettingsTab.fields),
        (
          'Delivery Challan Cycle',
          DeliveryChallansSettingsTab.deliveryChallanCycle,
        ),
        ('Validation Rules', DeliveryChallansSettingsTab.validationRules),
        ('Record Locking', DeliveryChallansSettingsTab.recordLocking),
        ('Statuses', DeliveryChallansSettingsTab.statuses),
        ('Buttons', DeliveryChallansSettingsTab.buttons),
        ('Related Lists', DeliveryChallansSettingsTab.relatedLists),
      ];
