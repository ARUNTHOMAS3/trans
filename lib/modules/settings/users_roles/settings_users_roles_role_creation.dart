...
class SettingsUsersRolesRoleCreation extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SettingsUsersRolesRoleCreation> createState() =>
      _SettingsUsersRolesRoleCreationState();
}

class _SettingsUsersRolesRoleCreationState
    extends ConsumerState<SettingsUsersRolesRoleCreation> {
  bool _loading = false;
  String? _loadError;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(roleCreationProvider.notifier).reset();
      if ((widget.roleId ?? '').isNotEmpty) {
        _loadRole();
      }
    });
  }
  ...
  Future<void> _loadRole() async {
    ...
    try {
      ...
      ref.read(roleCreationProvider.notifier).hydrateFromRole(
            ...
            permissionsPayload: payload['permissions'] is Map
  ...
  Future<void> _saveRole() async {
    final state = ref.read(roleCreationProvider);
    if (state.isDefaultRole) {
      ZerpaiToast.error(
        context,
        'This is a default role and cannot be edited or deleted.',
      );
      return;
    }
    if (state.roleName.trim().isEmpty) {
      ZerpaiToast.error(context, 'Role name is required.');
      return;
    }
    ...
    try {
      ...
      if ((widget.roleId ?? '').isEmpty) {
        await apiClient.post('users/roles', data: payload);
      } else {
        await apiClient.put('users/roles/${widget.roleId}', data: payload);
      }

      if (!mounted) return;
      ZerpaiToast.success(
        context,
        (widget.roleId ?? '').isEmpty
          ...
          if (state.isDefaultRole)
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: const Color(0xFFF1D28A)),
  ...
  Widget _buildStickyHeader(
    ...
    return Container(
      ...
      child: Row(
        children: [
          Text(
            widget.roleId == null ? 'New Role' : 'Edit Role',
        ...
        children: [
          _GeneralFormRow(
            ...
            child: CustomTextField(
              ...
              onChanged: state.isDefaultRole ? (_) {} : notifier.setRoleName,
      ...
      child: Column(
        ...
        children: [
              ...
              child: Row(
                children: [
                  ...
                  Switch(
                    ...
                    onChanged: state.isDefaultRole
                        ? null
                        : notifier.toggleFullAccessReports,
          ...
          if (isExpanded && !state.fullAccessReports) ...[
            _buildReportsHeader(notifier, categories, state.isDefaultRole),
        ...
        children: [
          Padding(
            ...
            child: _ZohoCheckbox(
              ...
              onTap: readOnly
                  ? null
                  : () => notifier.togglePermission(row.key, 'view', ['view']),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(
