  ...
  factory ExpenseMileageModel.fromJson(Map<String, dynamic> json) {
    return ExpenseMileageModel(
      ...
      distanceUnit: (json['distance_unit'] ?? json['distanceUnit'] ?? '')
          .toString(),
      ratePerKm:
          (json['rate_per_km'] as num?)?.toDouble() ??
          double.tryParse((json['rate_per_km'] ?? '0').toString()) ??
          0,
      odometerStart:
          (json['odometer_start'] as num?)?.toDouble() ??
          double.tryParse((json['odometer_start'] ?? '').toString()),
      odometerEnd:
          (json['odometer_end'] as num?)?.toDouble() ??
          double.tryParse((json['odometer_end'] ?? '').toString()),
    );
  }
