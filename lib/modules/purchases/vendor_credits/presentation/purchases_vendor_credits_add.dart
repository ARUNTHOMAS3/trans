...
class VendorCreditsAddPage extends ConsumerStatefulWidget {
  final String? vendorCreditId;
  final String? billId;
  const VendorCreditsAddPage({super.key, this.vendorCreditId, this.billId});

  @override
  ConsumerState<VendorCreditsAddPage> createState() =>
      _VendorCreditsAddPageState();
}

class _VendorCreditsAddPageState extends ConsumerState<VendorCreditsAddPage> {
  ...
  void initState() {
    ...
    if (widget.billId != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        try {
          final bill = await ref.read(purchaseBillProvider(widget.billId!).future);
          _prefillFromBill(bill);
        } catch (e) {
          debugPrint('Failed to load bill for prefill: $e');
        }
      });
    } else {
      _addItem();
  ...
  void _prefillFromBill(PurchasesBill bill) async {
    try {
      final customers = await ref.read(customersProvider.future);
      final vendorObj = customers.firstWhere(
        (c) => c.id == bill.vendorId || c.displayName == bill.vendorName,
      ...
      setState(() {
        _selectedVendorObj = vendorObj;
        _selectedSourceOfSupply = bill.placeOfSupply;
        _selectedDestinationOfSupply = bill.placeOfSupply;
        _selectedBill = bill.billNumber;
        _isReverseCharge = bill.isReverseCharge;
        
        if (bill.warehouseId != null) {
          final warehousesAsync = ref.read(warehousesProvider);
          warehousesAsync.whenData((list) {
            try {
              final wh = list.firstWhere(
                (w) => w.id == bill.warehouseId || w.name == bill.warehouseName,
              );
        ...
        _adjustmentController.text = bill.adjustment.toStringAsFixed(2);
        
        _items.clear();
        for (final li in bill.lineItems) {
          final item = _VCLineItem();
          ...
          item.qtyController.text = li.quantity.toStringAsFixed(2);
          item.rateController.text = li.rate.toStringAsFixed(2);
          item.discountController.text = li.discount.toString();
          item.discountIsPercent = li.discountType == '%';
          item.descriptionController.text = li.description ?? '';
          item.selectedTax = li.taxName;
          item.selectedTaxRate = li.taxAmount;
          item.selectedAccount = li.accountId;
      ...
      debugPrint('Error prefilling from bill: $e');
    }
  }
...
class _VCCurrencyBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      height: _VendorCreditsAddPageState._fieldHeight,
      padding: const EdgeInsets.symmetric(horizontal: 10),
      decoration: BoxDecoration(
        color: AppTheme.backgroundColor,
        border: Border.all(color: AppTheme.borderLight),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: FormDropdown<Item>(
                          ...
                          height: _VendorCreditsAddPageState._tableFieldHeight,
                          hideBorderDefault: true,
                          allowClear: false,
...
class _VCManageTaxInfoDialogState extends ConsumerState<_VCManageTaxInfoDialog> {
  ...
  @override
  Widget build(BuildContext context) {
    final placeLabel = widget.placeOfSupply.isNotEmpty ? widget.placeOfSupply : '—';
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        const SizedBox(height: 10),
                                        const Text('₹0.00', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
                                      ],
                                    ),
                                  ),
                                ),
                              ...
                              Padding(
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    const SizedBox(height: 14),
                                    const _VCPanelDetailRow(label: 'Credit Limit', value: '₹0.00'),
                                    const SizedBox(height: 14),
                                    const _VCPanelDetailRow(label: 'Payment Terms', value: '—'),
                                    const SizedBox(height: 14),
                                    _VCPanelDetailRow(label: 'GST Treatment', value: _gstTreatmentLabel),
                                    const SizedBox(height: 14),
                                    _VCPanelDetailRow(label: 'Place of Supply', value: v.placeOfSupply ?? '—'),
                                    const SizedBox(height: 14),
                          ...
                          Container(
                            ...
                            child: Column(
                              children: [
                                ...
                                const SizedBox(height: 10),
                                _VCPanelDetailRow(label: 'Phone', value: v.phone ?? v.mobilePhone ?? '—'),
                                const SizedBox(height: 10),
                                _VCPanelDetailRow(label: 'Email', value: v.email ?? '—'),
                              ],
                            ),
                          ),
                        ],
