...
class _PRCreateState
  ...
  double _itemDetailsColumnWidth() {
    ...
    final headerWidth = max(
      _measureTextWidth('ITEMS & DESCRIPTION', _itemHeaderMeasureStyle),
      _measureTextWidth('Add all Items', _itemHeaderMeasureStyle),
    );
    ...
    // Header only needs horizontal padding (12*2 = 24)
    // Item rows need: padding(24) + image(36) + gap(10) + dropdown arrow+padding(60) + extra breathing(30) = 160
    final headerNeeded = headerWidth + 24;
    final itemNeeded = itemWidth + 160.0;
  ...
  double _batchCardWidth(BatchInfo batch) {
    ...
    const horizontalPadding = 8.0; // EdgeInsets.all(4) gives 8px total
    const extraBreathingRoom = 10.0; // Minimal buffer for bounds constraint
    return (maxLineWidth + horizontalPadding + extraBreathingRoom).clamp(
      80.0,
      1200.0,
    );
  }
  ...
  double _dynamicQtyToReceiveColumnWidth() {
    ...
    // Computed based exactly on TableBodyCell: LeftPad(12) + RightPad(12) + SizedBox(94) + SizedBox(6) = 124.0
    const fixedContentWidth = 124.0;
    const gapBetweenBatchCards = 2.0;
    ...
    final requiredWidth = _items.fold<double>(baseWidth, (currentMax, item) {
      ...
      final spacing = item.batches.length * gapBetweenBatchCards;

      final rowWidth = fixedContentWidth + cardsWidth + spacing;

      return max(currentMax, rowWidth);
    });

    return requiredWidth.clamp(baseWidth, 2400.0);
  }

  double _dynamicBinColumnWidth() {
    const baseWidth = 160.0;
    if (_items.isEmpty || _binMode != 'item') return baseWidth;

    double maxBinWidth = baseWidth;
    for (int i = 0; i < _items.length; i++) {
      final bin = i < _preferredBins.length ? _preferredBins[i] : null;
      final text = bin ?? "Select Bin";
      final measure = _measureTextWidth(
        text,
        const TextStyle(fontSize: 13, color: _textPrimary, fontFamily: 'Inter'),
      );
      // Cell padding: 12 + 12 = 24
      // Dropdown icon + gap = 24 + 8 = 32
      // Breathing room = 24
      final requiredWidth = measure + 24 + 32 + 24;
      if (requiredWidth > maxBinWidth) {
        maxBinWidth = requiredWidth;
      }
    }
    return maxBinWidth.clamp(baseWidth, 800.0);
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _loadNextReceiveNumber();
    });
  }

  Future<void> _loadNextReceiveNumber() async {
    try {
      final repository = ref.read(purchaseReceiveRepositoryProvider);
      final next = await repository.getNextPurchaseReceiveNumber(
        prefix: _receiveNumberPrefix,
      );

      if (!mounted) return;

      final prefix = (next['prefix']?.toString().trim().isNotEmpty == true)
          ? next['prefix'].toString().trim()
          : _receiveNumberPrefix;
      final nextNumber =
          int.tryParse(next['nextNumber']?.toString() ?? '') ??
          _receiveNextNumber;
      final formatted = next['formatted']?.toString().trim();

      setState(() {
        _receiveNumberPrefix = prefix;
        _receiveNextNumber = nextNumber;
        if (_isReceiveAutoGenerate) {
          _receiveNumberCtrl.text =
              (formatted != null && formatted.isNotEmpty)
                  ? formatted
                  : _generateReceiveNumber();
        }
      });
    } catch (e, st) {
      AppLogger.warning(
        'Failed to load next purchase receive number',
        error: e,
        stackTrace: st,
        module: 'purchases',
      );
  ...
  void _switchToManualMode() {
    setState(() {
      ...
      if (_isManualMode) {
        _hiddenManualIndices.clear();
        // Find PO items that haven't been touched yet (no batches, no qty)
        // and hide them from the manual view to avoid "autoloading" the entire PO
        for (int i = 0; i < _items.length; i++) {
          final item = _items[i];
          final hasWorkDone =
              item.batches.isNotEmpty || item.quantityToReceive > 0;
          final isPOItem = i < (_selectedPO?.items.length ?? 0);

          if (isPOItem && !hasWorkDone) {
            _hiddenManualIndices.add(i);
          }
        }

        // Ensure there's at least one empty row to work with
        // but only if the visible list is empty
        final bool anyVisible = _items.asMap().entries.any(
          (e) => !_hiddenManualIndices.contains(e.key),
        );
        if (!anyVisible) {
          _items.add(PurchaseReceiveItem());
          _rowControllers.add(_ReceiveItemRowController());
    ...
    // If switching back to normal mode and it's empty, repopulate from PO
    final bool hasMeaningfulData = _items.any(
      (item) =>
    ...
    if (!_isManualMode && !hasMeaningfulData && _selectedPO != null) {
                        ...
                        child: Scrollbar(
                          ...
                          child: SingleChildScrollView(
                            ...
                            child: Column(
                              ...
                              children: List.generate(_uploadedFiles.length, (
                                index,
                              ) {
                                  ...
                                  child: Container(
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        ...
                                        InkWell(
                                          onTap: () {
                                            _removeUploadedFile(index);
                                            // Close overlay on removal to avoid state issues or just rebuild
                                            _hideFilePopupOverlay();
                                          },
                                          child: Icon(
                                            LucideIcons.trash2,
  ...
  Future<void> _onPOSelected(PurchaseOrder po) async {
    ...
    try {
      ...
      resolvedWarehouseId =
          poToUse.deliveryWarehouseId?.trim() ??
          poToUse.warehouseId?.trim() ??
          poToUse.outletId?.trim();
      ...
      if (idToLookup != null && idToLookup.isNotEmpty) {
        ...
        final match =
            warehouseList
                .where((w) => w.id.toLowerCase() == idToLookup)
                .firstOrNull ??
            warehouseList
                .where((w) => w.parentOutletId?.toLowerCase() == idToLookup)
                .firstOrNull;

        if (match != null) {
          resolvedName = match.name;
        } else if (poToUse.warehouseName != null &&
            poToUse.warehouseName!.isNotEmpty) {
          // 2. Try matching by name string if ID lookup failed
          final nameMatch = warehouseList
              .where(
                (w) =>
                    w.name.toLowerCase() ==
                    poToUse.warehouseName!.toLowerCase(),
              )
              .firstOrNull;
        ...
        resolvedName =
            (poToUse.warehouseName != null && poToUse.warehouseName!.isNotEmpty)
                        ...
                        ..._items
                            ...
                            .where((e) {
                              ...
                              final poItemIds =
                                  ...
                                  {};
                              final isPOItem =
                                  i < (_selectedPO?.items.length ?? 0);
                              if (isPOItem) return true;

                              // Show manual items only if they have an item selected AND it's not already in the PO
                              final hasId = (item.itemId ?? '').isNotEmpty;
                              if (!hasId) return false;

                              return !poItemIds.contains(item.itemId);
                            })
                            .map(
  ...
  void _adjustRowQuantity(int index, {required int delta}) {
    ...
    final display = nextQty <= 0
        ? ''
        : (nextQty == nextQty.roundToDouble()
  ...
  void _fillAllUnreceivedQuantities() {
    ...
    setState(() {
      for (var i = 0; i < _items.length; i++) {
        ...
        final display = ordered <= 0
            ? ''
            : (ordered == ordered.roundToDouble()
  ...
  Widget _buildQtyAndFocBreakdown(PurchaseReceiveItem item) {
    // We use the item's current quantityToReceive (sum of batches) and total FOC
    final qty = item.batches.fold<double>(0, (sum, b) => sum + b.quantity);
    final foc = item.batches.fold<double>(0, (sum, b) => sum + b.foc);

    // If no quantity and no FOC entered, show nothing
    if (qty <= 0 && foc <= 0) return const SizedBox.shrink();

    String label = "${_fmtPcs(qty)} pcs";
    if (foc > 0) {
      label += " + ${_fmtPcs(foc)} foc";
    }
  ...
  Widget _buildVendorDropdownItem(
    Vendor vendor,
    bool isSelected,
    bool isHovered,
  ) {
    final vendorNumber = (vendor.vendorNumber ?? '').trim();
    final email = (vendor.email ?? '').trim();
    final companyName = (vendor.companyName ?? '').trim();
    final firstName = (vendor.firstName ?? '').trim();

    final topLine = vendorNumber.isEmpty
        ? vendor.displayName
        : '${vendor.displayName} | $vendorNumber';
    ...
    final initialSource = firstName.isNotEmpty
        ? firstName
        : (vendor.displayName.isNotEmpty ? vendor.displayName : '?');
  ...
  Widget _buildManualRow(
    ...
    final selectedIds = _items
        ...
        .whereType<String>()
        .toSet();
    final visibleManualRowsCount = _items
        .asMap()
        .entries
        .where((e) => !_hiddenManualIndices.contains(e.key))
        .length;
    ...
    // Remove the explicit '0' setting to allow hint text to show
        ...
        child: Row(
          ...
          children: [
                    ...
                    Expanded(
                      child: Focus(
                        onFocusChange: (hasFocus) {
                          setState(() {
                            if (hasFocus) {
                              _focusedManualItemFields.add(
                                "manual-item-$index",
                              );
                            } else {
                              _focusedManualItemFields.remove(
                                "manual-item-$index",
                              );
                            }
                          });
                        ...
                        child: Container(
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              color:
                                  _focusedManualItemFields.contains(
                                    "manual-item-$index",
                                  )
                                  ? _focusBorder
                                  : Colors.transparent,
                          ...
                          child: FormDropdown<PurchaseOrderItem>(
                            ...
                            onChanged: (poItem) {
                              ...
                              setState(() {
                                // Check if this PO item already exists but is hidden
                                final existingHiddenIndex = _items
                                    .asMap()
                                    .entries
                                    .toList()
                                    .indexWhere(
                                      (e) =>
                                          _hiddenManualIndices.contains(
                                            e.key,
                                          ) &&
                                          e.value.itemId == poItem.productId,
                                    );

                                if (existingHiddenIndex != -1) {
                                  // It was hidden, so unhide it!
                                  _hiddenManualIndices.remove(
                                    existingHiddenIndex,
                                  );

                                  // If we were selecting this in an empty "isEphemeral" row,
                                  // we should probably remove the empty row if it's purely redundant
                                  if (isEphemeral) {
                                    _removeItem(index, isFromManual: true);
                                  }
                                } else {
                                  // Normal addition/update logic
                                  if (isEphemeral) {
                                    _items.add(
            ...
            _tableBodyCell(
              fixedWidth: 32,
              isLastColumn: true,
              child: (isEphemeral || visibleManualRowsCount <= 1)
                  ? const SizedBox()
                  : Center(
                      child: InkWell(
                        onTap: () => _removeItem(index, isFromManual: true),
                        borderRadius: BorderRadius.circular(4),
                        child: const Icon(
                          LucideIcons.x,
                          size: 14,
                                    ...
                                    child: Container(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          _batchText('MRP: ${batch.mrp}'),
                                          _batchText('Purchase Rate: ${batch.ptr}'),
                                          _batchText(
                                            'Exp: ${batch.expiryDate != null ? DateFormat('dd-MM-yyyy').format(batch.expiryDate!) : ''}',
                                          ),
                                        ],
  ...
  void _handleSave(String status) async {
    ...
    if (success) {
      ...
      ZerpaiToast.error(
        context,
        'Failed to save purchase receive. Please try again.',
      );
    }
  }
  ...
  void _addAllItemsFromPO() async {
    ...
    if (fullPO != null && fullPO.items.isNotEmpty) {
      if (!mounted) return;
      setState(() {
        // DO NOT clear all rows, just ensure they are all visible
        _hiddenManualIndices.clear();

        // Get set of existing item IDs to avoid duplicates
        final existingItemIds = _items
            .map((it) => (it.itemId ?? '').trim())
            .where((id) => id.isNotEmpty)
            .toSet();

        for (var poItem in fullPO!.items) {
          final poProductId = poItem.productId.trim();
          if (poProductId.isEmpty) continue;

          // If item already exists, keep it (preserving its batches/state)
          if (existingItemIds.contains(poProductId)) continue;

          // Check if there's a purely empty row (no ID, no name) to fill first
          int emptyRowIndex = _items.indexWhere(
            (it) => (it.itemId ?? '').isEmpty && it.itemName.isEmpty,
          );

          if (emptyRowIndex != -1) {
            // Update existing empty row
            _items[emptyRowIndex] = PurchaseReceiveItem(
              itemId: poItem.productId,
              itemName: poItem.productName ?? poItem.itemCode ?? "",
              description: poItem.description,
              ordered: poItem.quantity,
            ...
            if (emptyRowIndex < _rowControllers.length) {
              _rowControllers[emptyRowIndex].qtyCtrl.text = '';
            }
          } else {
            // Add new row since no empty row was found
            _items.add(
              PurchaseReceiveItem(
                itemId: poItem.productId,
                itemName: poItem.productName ?? poItem.itemCode ?? "",
          ...
          existingItemIds.add(poProductId);
        }
      });
    }
  }
  ...
  Future<void> _showSelectBatchDialog(int itemIndex) async {
    ...
    if (itemId != null && itemId.isNotEmpty) {
      try {
        ...
        // Also provide full batch data if we need it for auto-fill logic in the dialog
...
class _SelectBatchDialogState extends State<_SelectBatchDialog> {
        ...
        child: SizedBox(
          ...
          child: RawAutocomplete<String>(
            ...
            optionsBuilder: (textEditingValue) {
              ...
              final sortedBatchItems = widget.batchOptions.toList()..sort();
              if (query.isEmpty) {
            ...
            onSelected: (selection) {
              ...
              row.selectedBatchNo = selection;

              // Find matching batch data for auto-fill
              final match = widget.batchDataOptions.firstWhere(
                (b) => b.batchReference.trim() == selection.trim(),
                orElse: () => BatchData(
                  batchReference: selection,
                  manufacturerBatch: '',
                  unitPack: '',
                  manufacturedDate: '',
                  expiryDate: '',
              ...
              if (match.batchReference == selection) {
                ...
                // Try to parse dates for the controllers
                try {
                ...
                (context, textEditingController, focusNode, onFieldSubmitted) {
                  return TextField(
                    ...
                    onChanged: (value) {
                      ...
                      if (row.isAutoLoaded && value != row.selectedBatchNo) {
                        row.isAutoLoaded = false;
                        row.selectedBatchNo = null;
                        // Clear auto-filled fields
                        row.unitPackCtrl.clear();
                        row.mrpCtrl.clear();
                        row.ptrCtrl.clear();
                        row.expDateCtrl.clear();
                        row.mfgBatchCtrl.clear();
                        row.mfgDateCtrl.clear();
                        row.expDate = null;
                        row.mfgDate = null;
                      }
                      setState(() {});
    ...
    final dateField = Padding(
      ...
      child: SizedBox(
        ...
        child: TextField(
          ...
          readOnly:
              readOnly ||
              true, // Keep original behavior but combine with parameter
  ...
  @override
  Widget build(BuildContext context) {
    // Calculate dynamic width based on visible columns to prevent shrinking.
    // Base columns (Batch, Unit Pack, MRP, Purchase Rate, Expiry, Quantity) now = 13 flex units.
    // Weight per unit is ~68 pixels.
    double dialogWidth = 920; // Adjusted base width for 13 units (reduced from 14)
    if (_showMfgDetails) dialogWidth += 350; // +5 units (reduced from 6)
    if (_showFoc) dialogWidth += 140; // +2 units
    if (_showDamage) dialogWidth += 140; // +2 units
    ...
    return Dialog(
      ...
      child: Container(
        width: dialogWidth.clamp(850.0, MediaQuery.of(context).size.width * 0.95),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  if (_showMfgDetails) ...[
                    _headerCell('MANUFACTURED DATE', 2),
                    _headerCell('MANUFACTURER BATCH', 2),
                  ],
                ...
                children: [
                  ElevatedButton(
                    onPressed: () {
                      ...
                      // 2. PASS 1: MANDATORY BATCH NO (ALL ROWS)
                      for (final row in _rows) {
                        if (row.batchNoCtrl.text.trim().isEmpty) {
                          setState(() {
                            _dialogErrorMessage = _batchNoMissingMessage;
                          });
                          return;
                        }
                      }

                      // 3. PASS 2: MANDATORY UNIT PACK (ALL ROWS)
                      for (final row in _rows) {
                        if (row.unitPackCtrl.text.trim().isEmpty) {
                          setState(() {
                            _dialogErrorMessage = _unitPackMissingMessage;
                          });
                          return;
                        }
                      }

                      // 4. PASS 3: MANDATORY MRP (ALL ROWS)
                      for (final row in _rows) {
                        if (row.mrpCtrl.text.trim().isEmpty) {
                          setState(() {
                            _dialogErrorMessage = _mrpMissingMessage;
                          });
                          return;
                        }
                      }

                      // 5. PASS 4: MANDATORY EXPIRY DATE (ALL ROWS)
                      for (final row in _rows) {
                        // Double-checked for both text content and the Date object
                        if (row.expDateCtrl.text.trim().isEmpty ||
                            row.expDate == null) {
                          setState(() {
                            _dialogErrorMessage = _expDateMissingMessage;
                          });
                          return;
                        }
                      }

                      // 6. PASS 5: AT LEAST ONE QUANTITY OR FOC (ALL ROWS)
                      for (final row in _rows) {
                        ...
                        if (parsedQty <= 0 && parsedFoc <= 0) {
                          setState(() {
                            _dialogErrorMessage = _qtyOrFocMessage;
                          });
                          return;
                        }
                      }

                      // 7. PASS 6: TOTAL QUANTITY LIMITS
                      ...
                      // 8. PASS 7: FINAL QUANTITY MATCH CHECK (IF NOT OVERWRITING)
                      if (!_overwriteLineItem &&
                          (_totalEnteredQtyOnly - widget.ordered).abs() >
                              0.0001) {
                        setState(() {
