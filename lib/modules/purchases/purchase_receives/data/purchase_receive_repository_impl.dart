...
class PurchaseReceiveRepositoryImpl implements PurchaseReceiveRepository {
  ...
  @override
  Future<Map<String, dynamic>> getNextPurchaseReceiveNumber({
    String? prefix,
  }) async {
    try {
      final response = await _apiClient.get(
        ApiEndpoints.purchaseReceiveNextNumber,
        queryParameters: {
      ...
      return {
        'prefix': prefix ?? 'PR-',
        'nextNumber': 1,
        'formatted': '${prefix ?? 'PR-'}00001',
      };
    } catch (e) {
      AppLogger.error(
        'getNextPurchaseReceiveNumber error',
        error: e,
        module: 'purchases',
      );
      return {
