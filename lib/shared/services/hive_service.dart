...
class HiveService {
  ...
  static const Duration _transfersOpenTimeout = Duration(seconds: 5);
  static const Duration _transfersRetryCooldown = Duration(seconds: 30);
  DateTime? _transfersRetryAllowedAt;
  Future<Box<StockTransfer>?>? _transfersOpenFuture;
  ...
  Box<PurchaseOrder> get purchaseOrdersBox =>
      Hive.box<PurchaseOrder>('purchase_orders');
  ...
  Box<StockTransfer> get transfersBox =>
      Hive.box<StockTransfer>(_transfersBoxName);
  ...
  Future<Box<StockTransfer>?> ensureTransfersBox() async {
    if (_transfersOpenFuture != null) {
      return _transfersOpenFuture;
    }

    final now = DateTime.now();
    if (_transfersRetryAllowedAt != null &&
        now.isBefore(_transfersRetryAllowedAt!)) {
      return null;
    }

    _transfersOpenFuture = (() async {
      try {
        if (Hive.isBoxOpen(_transfersBoxName)) {
          _transfersRetryAllowedAt = null;
          return Hive.box<StockTransfer>(_transfersBoxName);
        }
        final box = await Hive.openBox<StockTransfer>(
          _transfersBoxName,
        ).timeout(_transfersOpenTimeout);
        _transfersRetryAllowedAt = null;
        return box;
      } on TimeoutException {
        _transfersRetryAllowedAt = now.add(_transfersRetryCooldown);
        AppLogger.warning(
          'Transfers box open timeout; cache cooldown active',
          module: 'hive_service',
          data: {
            'box': _transfersBoxName,
            'timeout_seconds': _transfersOpenTimeout.inSeconds,
            'cooldown_seconds': _transfersRetryCooldown.inSeconds,
          },
        );
        return null;
      } catch (e) {
        _transfersRetryAllowedAt = now.add(_transfersRetryCooldown);
        AppLogger.error(
          '❌ Failed to ensure transfers box',
          error: e,
          module: 'hive_service',
        );
        return null;
      } finally {
        _transfersOpenFuture = null;
      }
    })();
  ...
  Future<void> saveTransfers(List<StockTransfer> transfers) async {
    try {
      final box = await ensureTransfersBox();
      if (box == null) return;
      await box.clear();

      final Map<String, StockTransfer> map = {};
  ...
  List<StockTransfer> getTransfers() {
    try {
      if (!Hive.isBoxOpen(_transfersBoxName)) {
        return const <StockTransfer>[];
      }
      return Hive.box<StockTransfer>(_transfersBoxName).values.toList();
    } catch (_) {
      return const <StockTransfer>[];
    }
  }
  ...
  StockTransfer? getTransfer(String id) {
    try {
      ...
      return Hive.box<StockTransfer>(_transfersBoxName).get(id);
    } catch (_) {
      return null;
    }
  }
  ...
  Future<void> deleteTransfer(String id) async {
    try {
      ...
      await Hive.box<StockTransfer>(_transfersBoxName).delete(id);
    } catch (_) {}
  }
