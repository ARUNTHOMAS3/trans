...
class BinLocationsService {
  ...
  bool _isMissingWarehouseScopeError(Object error) {
    final String raw = error.toString().toLowerCase();
    return raw.contains('no warehouse linked');
  }
    ...
    final results = await Future.wait(
      ids.map((branchId) async {
        try {
          ...
          // Branch list should render even when zone scope is not configured yet.
          if (_isMissingWarehouseScopeError(error)) {
            return MapEntry(
              branchId,
              const BranchZoneBinsSummary(zoneCount: 0, binCount: 0),
            );
          }
          rethrow;
        }
