...
class _ManageReorderTermsDialogState extends State<ManageReorderTermsDialog> {
  ...
  Future<void> _saveChanges() async {
    ...
    for (var i = 0; i < _rows.length; i++) {
      ...
      final comboKey = '${termName.toLowerCase()}::$quantity';
      if (seenRuleCombos.contains(comboKey)) {
        _showError(
          'The reorder rule "$termName (+$quantity additional units)" already exists.',
        );
        return;
      }
      seenRuleCombos.add(comboKey);
