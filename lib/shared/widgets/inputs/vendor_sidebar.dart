...
class _VendorSidebarState extends State<VendorSidebar>
  ...
  Widget _buildContactDetailsCard(Vendor v) {
    ...
    final paymentTermLabel = term['term_name'] ?? '';
...
/// Collapsed dark "side tag" that re-opens the [VendorSidebar] when the panel
/// is hidden. The host page is responsible for positioning it (e.g. a
/// `Positioned`/`AnimatedOpacity` anchored to the right edge); this widget only
/// owns the tag's visual + tap behaviour so the look stays consistent across
/// screens that show a vendor details panel.
