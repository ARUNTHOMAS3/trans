...
class _PicklistSelectBatchesDialogState
  ...
  List<Map<String, String>> _binList = [];
  List<Map<String, dynamic>> _batchItems = [];

  List<String> get _binLocations => _binList.map((b) => b['bin_code']!).toList();
  ...
  Future<void> _loadBins() async {
    try {
      ...
      if (widget.warehouseId.isNotEmpty) {
      ...
      if (mounted) {
        setState(() {
          _binList = (response as List)
              .where((b) => (b['bin_code'] ?? '').toString().isNotEmpty)
              .map((b) => {
                    'id': (b['id'] ?? '').toString(),
  ...
  Future<void> _loadBatches() async {
    ...
    try {
      ...
      final layerRows = await supabase
          .from('batch_stock_layers')
          .select('id, batch_id, mrp, purchase_rate, qty, reserved_qty')
          .eq('product_id', widget.productId);

      final balanceMap = <String, double>{};
      final priceMap = <String, Map<String, dynamic>>{};
      for (final layer in layerRows as List) {
        final batchId = layer['batch_id']?.toString() ?? '';
        if (batchId.isEmpty) continue;
        final qty = (layer['qty'] as num?)?.toDouble() ?? 0.0;
        final reserved = (layer['reserved_qty'] as num?)?.toDouble() ?? 0.0;
        balanceMap[batchId] = (balanceMap[batchId] ?? 0.0) + (qty - reserved);
        if (!priceMap.containsKey(batchId)) {
          priceMap[batchId] = {
            'layer_id': layer['id']?.toString() ?? '',
            'mrp': layer['mrp']?.toString() ?? '0.00',
            'prate': layer['purchase_rate']?.toString() ?? '0.00',
          };
        }
      }

      final items = (batchRows as List).map((b) {
        final id = b['id']?.toString() ?? '';
        return <String, dynamic>{
          ...Map<String, dynamic>.from(b as Map),
          'balance': (balanceMap[id] ?? 0.0).toStringAsFixed(2),
          'layer_id': priceMap[id]?['layer_id'] ?? '',
          'mrp': priceMap[id]?['mrp'] ?? '0.00',
          'prate': priceMap[id]?['prate'] ?? '0.00',
        };
      }).toList();

      if (mounted) {
        setState(() => _batchItems = items);
      }
    } catch (_) {}
  }
                                          ...
                                          onChanged: (v) {
                                            setState(() {
                                              if (v != null) {
                                                ...
                                                row.layerId = v['layer_id']?.toString();
                                              }
                                            });
                                          },
                                        ),
