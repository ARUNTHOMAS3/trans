...
Future<_OpenedBox> _openTypedBox(String boxName, Type? type) async {
  Future<Box> doOpen() async {
    if (type == Item) {
      return Hive.openBox<Item>(boxName);
    } else if (type == SalesCustomer) {
      return Hive.openBox<SalesCustomer>(boxName);
    } else if (type == SalesOrder) {
      return Hive.openBox<SalesOrder>(boxName);
    } else if (type == SalesPayment) {
      return Hive.openBox<SalesPayment>(boxName);
    } else if (type == SalesEWayBill) {
      return Hive.openBox<SalesEWayBill>(boxName);
    } else if (type == Vendor) {
      return Hive.openBox<Vendor>(boxName);
    } else if (type == PurchaseOrder) {
      return Hive.openBox<PurchaseOrder>(boxName);
    } else if (type == PurchasesBill) {
      return Hive.openBox<PurchasesBill>(boxName);
    } else if (type == Stock) {
      return Hive.openBox<Stock>(boxName);
    } else if (type == InventoryAdjustment) {
      return Hive.openBox<InventoryAdjustment>(boxName);
    } else if (type == StockTransfer) {
      return Hive.openBox<StockTransfer>(boxName);
    } else if (type == AccountNode) {
      return Hive.openBox<AccountNode>(boxName);
    } else if (type == BatchData) {
      return Hive.openBox<BatchData>(boxName);
    } else if (type == SerialData) {
      return Hive.openBox<SerialData>(boxName);
    } else if (type == TransactionData) {
      return Hive.openBox<TransactionData>(boxName);
    }
    return Hive.openBox(boxName);
  }

  try {
    final box = await doOpen();
    return _OpenedBox(name: boxName, box: box);
  } catch (error) {
    debugPrint(
      '⚠️ Hive open failed for "$boxName" (likely stale cache decode). '
      'Resetting local box. Error: $error',
    );
    try {
      await Hive.deleteBoxFromDisk(boxName);
    } catch (deleteError) {
      debugPrint('⚠️ Failed to delete box "$boxName": $deleteError');
    }
    final box = await doOpen();
    return _OpenedBox(name: boxName, box: box);
  }
...
Future<void> _initApp() async {
  ...
  try {
    DomainEventHandlers.registerDefaults();
    ...
    // 2. Open all other core boxes with per-box guardrails.
    // A single slow/corrupt box must not crash full app initialization.
    final hiveBoxesWatch = Stopwatch()..start();
    final openedBoxes = <_OpenedBox>[];
    for (final entry in boxDefinitions.entries.where(
      (entry) => entry.key != 'config',
    )) {
      try {
        final opened = await _openTypedBox(
          entry.key,
          entry.value,
        ).timeout(const Duration(seconds: 6));
        openedBoxes.add(opened);
      } on TimeoutException catch (_) {
        debugPrint(
          '⚠️ Hive open timeout for "${entry.key}". Continuing startup and '
          'leaving this cache cold for this session.',
        );
      } catch (error) {
        debugPrint(
          '⚠️ Hive open failed for "${entry.key}" after recovery: $error',
        );
      }
    }
    hiveBoxesWatch.stop();
    debugPrint(
      '⏱️ [boot] core_boxes_open_guarded=${hiveBoxesWatch.elapsedMilliseconds}ms',
    );
    ...
    debugPrint(
      '⏱️ [boot] supabase_init=${supabaseWatch.elapsedMilliseconds}ms',
    );
