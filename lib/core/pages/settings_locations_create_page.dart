...
class SettingsLocationsCreatePage extends ConsumerStatefulWidget {
  ...
  const SettingsLocationsCreatePage({super.key, this.branchId});
...
class _SettingsLocationsCreatePageState
  ...
  List<Map<String, String>> _withLocationTransactionRows(
    ...
    final next = List<Map<String, String>>.from(modules);
    final existing = next
        .map((item) => (item['code'] ?? item['id'] ?? '').toLowerCase())
        .toSet();
    if (!existing.contains('branch')) {
      next.add(const {'code': 'branch', 'label': 'Branch'});
    }
    if (!existing.contains('warehouse')) {
      next.add(const {'code': 'warehouse', 'label': 'Warehouse'});
    }
    return next;
  }
      ...
      if (res.success && res.data is List) {
        ...
        setState(() {
          _branches = list
              .where(
                (o) =>
                    o['id'] != widget.branchId &&
                    (o['parent_branch_id'] == null ||
                        (o['parent_branch_id'] as String?)?.isEmpty != false),
              )
              .map(
                (o) => _BranchOption(
                  id: o['id'].toString(),
  ...
  Future<void> _loadLookupOptions() async {
    try {
      ...
      if (!mounted) return;
      final gstTypes = responses[0].success && responses[0].data is List
          ? (responses[0].data as List)
                ...
                .toList()
          : <Map<String, String>>[];
      final transactionModules =
          responses[1].success && responses[1].data is List
          ? _withLocationTransactionRows(
          ...
          : <Map<String, String>>[];
      setState(() {
        _gstRegistrationTypeOptions = gstTypes;
        _transactionModuleOptions = transactionModules;
      });
    } catch (_) {}
  }
  ...
  Future<void> _loadStates() async {
    try {
      final res = await _apiClient.get(
        '/lookups/states',
        queryParameters: {'countryId': 'india'},
        useCache: true,
      );
      if (!mounted) return;
      if (res.success && res.data is List) {
        final List<String> list = [];
        for (final item in (res.data as List).whereType<Map>()) {
  ...
  Future<void> _loadExisting() async {
    ...
    try {
      ...
      if (res.success && res.data is Map<String, dynamic>) {
        ...
        final parentId = d['parent_branch_id']?.toString();
        ...
        final rawPhone = (d['phone'] ?? '').toString();
        if (rawPhone.startsWith('+')) {
          final parts = rawPhone.split(' ');
          if (parts.length > 1) {
            _phonePrefix = parts[0];
            _phoneCtrl.text = parts.sublist(1).join(' ');
          } else {
            // Best effort handle case like +911234567890
            if (rawPhone.startsWith('+91') && rawPhone.length > 3) {
              _phonePrefix = '+91';
              _phoneCtrl.text = rawPhone.substring(3);
            } else {
              _phonePrefix = '+91';
        ...
        setState(() {
          ...
          _selectedState = _stateOptions.contains(state) ? state : null;
  ...
  Future<void> _save() async {
    ...
    final bool parentMissing = needsParent && _parentBranchId == null;
    if (parentMissing)
    ...
    try {
      ...
      final body = <String, dynamic>{
        ...
        'branch_code': _nameCtrl.text.trim().toUpperCase().replaceAll(' ', '-'),
        ...
        'phone': '$_phonePrefix ${_phoneCtrl.text.trim()}'.trim(),
        ...
        'parent_branch_id': needsParent ? _parentBranchId : null,
      ...
      final res = _isEditing
          ? await _apiClient.patch('/branches/${widget.branchId}', data: body)
          : await _apiClient.post('/branches', data: body);
                        ...
                        children: [
                          ...
                          _buildDialogRow(
                            ...
                            child: FormDropdown<Map<String, String>>(
                              value: _gstRegistrationTypeOptions
                                  .where((t) => t['code'] == regType)
                                  .firstOrNull,
  ...
  Widget _buildParentDropdown({required bool required}) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: AppTheme.space6),
        FormDropdown<_BranchOption>(
          value: _branches.where((o) => o.id == _parentBranchId).firstOrNull,
          items: _branches,
          displayStringForValue: (o) => o.name,
          hint: 'Select parent location',
          errorText: _parentError,
          onChanged: (o) => setState(() {
            _parentBranchId = o?.id;
            _parentError = null;
          }),
        ),
      ],
  ...
  Future<void> _showCreateSeriesDialog() async {
    ...
    final rows = _transactionModuleOptions
        .map((_) => _SeriesModuleRow(prefix: '', starting: '1'))
        .toList();
    ...
    await showDialog<void>(
      ...
      builder: (_) => StatefulBuilder(
        builder: (ctx, setS) {
          ...
          final generatedLocationCode = _nameCtrl.text
              .trim()
              .toUpperCase()
                            ...
                            children: [
                              TextField(
                                ...
                                onChanged: (value) => setS(() {
                                  nameError = null;
                                  seriesCodeCtrl.text = buildAutoCode(value);
                                }),
                                decoration: _dialogInputDecoration(
                                  '',
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: Padding(
                            padding: const EdgeInsets.only(top: 8),
                            child: Text(
                              _isBusiness ? 'Branch Code' : 'Warehouse Code',
                              style: const TextStyle(
                                fontSize: 13,
                                color: AppTheme.textPrimary,
                                fontWeight: FontWeight.w500,
                              ...
                              : () async {
                                  ...
                                  try {
                                    ...
                                    for (
                                      ...
                                      modules.add({
                                        'module_key':
                                            _transactionModuleOptions[i]['code'],
