...
class _SettingsOrganizationProfilePageState
  ...
  List<Map<String, String>> _fiscalYearOptions = <Map<String, String>>[];
  ...
  final TextEditingController _companyIdValueController =
      TextEditingController();
  final TextEditingController _addressAttentionController =
      TextEditingController();
  final TextEditingController _addressStreet1Controller =
      TextEditingController();
  final TextEditingController _addressStreet2Controller =
      TextEditingController();
  ...
  final GlobalKey _organizationAddressKey = GlobalKey();
  ...
  List<_WardOption> _wardOptions = const [];
  ...
  Future<void> _loadDistrictsForSelectedState() async {
    ...
    try {
      final response = await _apiClient.get(
        '/lookups/districts',
        queryParameters: <String, dynamic>{'stateId': stateId},
        useCache: false,
      );
      if (!mounted) return;
      final districts = response.success && response.data is List
  ...
  Future<void> _loadLocalBodiesForSelectedDistrict(String? districtId) async {
    ...
    try {
      final response = await _apiClient.get(
        ...
        queryParameters: <String, dynamic>{'districtId': normalizedDistrictId},
        useCache: false,
      );
      if (!mounted) return;
      final allLocalBodies = response.success && response.data is List
          ? (response.data as List)
                .whereType<Map<String, dynamic>>()
                .map(
                  (item) => _LocalBodyOption(
                    id: (item['id'] ?? '').toString(),
                    name: (item['name'] ?? '').toString(),
                    code: item['code']?.toString(),
                    bodyType: (item['body_type'] ?? '').toString(),
                  ),
                )
                .where(
                  (item) =>
      ...
      setState(() {
        ...
        _localBodyOptions = filteredLocalBodies;
      });
    } catch (_) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _loadAssembliesForSelectedDistrict(String? districtId) async {
    final normalizedDistrictId = districtId?.trim() ?? '';
    if (normalizedDistrictId.isEmpty) {
      if (!mounted) return;
      setState(() {
    ...
    try {
      ...
      if (!mounted) return;
      final assemblies = response.success && response.data is List
          ? (response.data as List)
                .whereType<Map<String, dynamic>>()
                .map(
                  (item) => _AssemblyOption(
  ...
  Future<void> _loadWardsForSelectedLocalBody(String? localBodyId) async {
    final normalizedLocalBodyId = localBodyId?.trim() ?? '';
    if (normalizedLocalBodyId.isEmpty) {
      if (!mounted) return;
      setState(() {
        _wardOptions = const [];
      });
    ...
    try {
      ...
      if (!mounted) return;
      final wards = response.success && response.data is List
          ? (response.data as List)
                .whereType<Map<String, dynamic>>()
                .map(
                  (item) => _WardOption(
                    id: (item['id'] ?? '').toString(),
                    wardNo: int.tryParse((item['ward_no'] ?? '').toString()),
                    name: (item['name'] ?? '').toString(),
                    code: item['code']?.toString(),
  ...
  Future<void> _loadAssembliesForSelectedPaymentStubDistrict({
    String? preferredAssemblyCode,
  }) async {
    if (_selectedPaymentStubDistrictId == null ||
        _selectedPaymentStubDistrictId!.isEmpty) {
      if (!mounted) return;
      setState(() {
        _paymentStubAssemblyOptions = <_AssemblyOption>[];
    ...
    try {
      ...
      if (!mounted) return;
      final assemblies = res.success && res.data is List
          ? (res.data as List)
                .whereType<Map<String, dynamic>>()
                .map(
                  (assembly) => _AssemblyOption(
                    code: ((assembly['code'] ?? assembly['name']) ?? '')
                        .toString(),
      ...
      setState(() {
        ...
        if (preferredAssemblyCode != null &&
            assemblies.any(
              (assembly) =>
                  assembly.code == preferredAssemblyCode ||
                  assembly.name == preferredAssemblyCode,
            )) {
          _selectedPaymentStubAssemblyCode = preferredAssemblyCode;
        } else if (_selectedPaymentStubAssemblyCode != null &&
            assemblies.any(
              (assembly) =>
  ...
  String _selectedReportBasis = 'accrual';
  ...
  void dispose() {
    ...
    _addressAttentionController.dispose();
  ...
  String? _matchCodeLabelOption(
    ...
    for (final option in options) {
      final code = option[codeKey] ?? '';
      final label = option[labelKey] ?? '';
      if (code.toLowerCase() == raw.toLowerCase() ||
          label.toLowerCase() == raw.toLowerCase()) {
        return code.isNotEmpty ? code : label;
      }
    }
    return null;
  }

  String _displayCodeLabelOption(
    String? raw,
    List<Map<String, String>> options, {
    String codeKey = 'code',
    String labelKey = 'label',
    ...
    for (final option in options) {
      if ((option[codeKey] ?? '') == raw) {
        return option[labelKey] ?? raw;
      }
    }
    return raw;
  }
  ...
  Future<Response<dynamic>?> _safeGet(
    ...
    try {
      ...
      debugPrint('[OrgProfile] _safeGet failed: $path → $e');
      return null;
    }
  }
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      final String effectiveOrgId = user?.orgId.trim() ?? '';
      ...
      final List<Future<Response<dynamic>?>> futures =
          <Future<Response<dynamic>?>>[
            ...
            _safeGet('/lookups/fiscal-year-presets', useCache: false),
            _safeGet('/lookups/date-format-options', useCache: false),
            _safeGet('/lookups/date-separator-options', useCache: false),
            _safeGet('/lookups/drug-licence-types', useCache: false),
          ];
      ...
      final Response<dynamic>? fiscalYearResponse = responses[offset + 5];
      final Response<dynamic>? dateFormatResponse = responses[offset + 6];
      final Response<dynamic>? dateSeparatorResponse = responses[offset + 7];
      final Response<dynamic>? drugLicenceTypeResponse = responses[offset + 8];
      ...
      final List<String> companyIdOptions = companyIdLabelsResponse != null
          ...
          : <String>[];
      final List<Map<String, String>> fiscalYearOptions =
          fiscalYearResponse != null &&
              fiscalYearResponse.success &&
              fiscalYearResponse.data is List
          ? (fiscalYearResponse.data as List)
                .whereType<Map>()
                .map(
                  (item) => <String, String>{
                    'code': (item['code'] ?? '').toString(),
                ...
                .toList()
          : <Map<String, String>>[];
      final List<Map<String, String>> dateFormatOptions =
          dateFormatResponse != null &&
              dateFormatResponse.success &&
              dateFormatResponse.data is List
          ? (dateFormatResponse.data as List)
                .whereType<Map>()
                .map(
                  (item) => <String, String>{
                    'code': (item['code'] ?? '').toString(),
                    'label': (item['label'] ?? '').toString(),
                    'format_pattern': (item['format_pattern'] ?? '').toString(),
                    'group_name': (item['group_name'] ?? '').toString(),
                  },
                )
                .where((item) => item['format_pattern']!.isNotEmpty)
                .toList()
          : <Map<String, String>>[];
      final List<Map<String, String>> dateSeparatorOptions =
          dateSeparatorResponse != null &&
              dateSeparatorResponse.success &&
              dateSeparatorResponse.data is List
          ? (dateSeparatorResponse.data as List)
                .whereType<Map>()
                .map(
                  (item) => <String, String>{
                    'code': (item['code'] ?? '').toString(),
                    'label': (item['label'] ?? '').toString(),
                    'separator': (item['separator'] ?? '').toString(),
                  },
                )
                .where((item) => item['separator']!.isNotEmpty)
                .toList()
          : <Map<String, String>>[];
      final List<Map<String, String>> drugLicenceTypeOptions =
          drugLicenceTypeResponse != null &&
              drugLicenceTypeResponse.success &&
              drugLicenceTypeResponse.data is List
          ? (drugLicenceTypeResponse.data as List)
                .whereType<Map>()
                .map(
                  (item) => <String, String>{
                    'code': (item['code'] ?? '').toString(),
      ...
      setState(() {
        ...
        _fiscalYearOptions = fiscalYearOptions;
        ...
        _selectedDateFormat = _matchCodeLabelOption(
          (orgData['date_format'] ?? '').toString(),
          _dateFormatOptions,
          codeKey: 'format_pattern',
          labelKey: 'label',
        );
        _selectedDateSeparator = _matchCodeLabelOption(
        ...
        _selectedWardId = (orgData['ward_id'] ?? '').toString();
        ...
        _drugLicenceType = _matchCodeLabelOption(
        ...
        for (final item in additionalFieldsList) {
          if (item is Map) {
            final field = _ProfileAdditionalField();
            field.labelController.text = (item['label'] ?? '').toString();
            field.valueController.text = (item['value'] ?? '').toString();
        ...
        // Populate doc lists from URLs
        String? _getUrl(String key) => (orgData[key] ?? '').toString().trim();
        List<PlatformFile> _urlsToDocs(String? urlStr) {
          if (urlStr == null || urlStr.isEmpty) return [];
          return urlStr.split(',').where((u) => u.isNotEmpty).map((u) {
            final name = u.split('/').last.split('-').last;
            return PlatformFile(
              name: name,
              size: 0,
              path: u, // Store URL in path
            );
          }).toList();
        }

        _drugLicense20Docs = _urlsToDocs(_getUrl('drug_license_20_url'));
        _drugLicense21Docs = _urlsToDocs(_getUrl('drug_license_21_url'));
        _drugLicense20BDocs = _urlsToDocs(_getUrl('drug_license_20b_url'));
        _drugLicense21BDocs = _urlsToDocs(_getUrl('drug_license_21b_url'));
        _fssaiDocs = _urlsToDocs(_getUrl('fssai_url'));
        _msmeDocs = _urlsToDocs(_getUrl('msme_url'));
          ...
          children: [
            ...
            Positioned(
              ...
              child: InkWell(
                ...
                onTap: () => ZerpaiToast.info(context, 'Coming soon'),
                child: const Padding(
            ...
            ZerpaiFormRow(
              ...
              child: FormDropdown<String>(
                ...
                items: _drugLicenceTypeOptions
                    ...
                    .toList(),
                displayStringForValue: (value) =>
                    _displayCodeLabelOption(value, _drugLicenceTypeOptions),
                onChanged: (v) => setState(() => _drugLicenceType = v),
              ),
            ),
            if (_drugLicenceType == 'retail' ||
                _drugLicenceType == 'wholesale_and_retail') ...[
              ZerpaiFormRow(
  ...
  Future<bool> _verifySavedOrgProfile(
    ...
    return matches('name') &&
        ...
        matches('industry') &&
        matches('attention') &&
        matches('address_street_1') &&
        matches('address_street_2') &&
        matches('city') &&
        matches('pincode') &&
        matches('phone') &&
        matches('district_id') &&
        matches('local_body_id') &&
        matches('assembly_id') &&
        matches('ward_id') &&
  ...
  Widget _buildGroupedDateFormatDropdown() {
    ...
    final groupedPatterns = <String, List<String>>{};
    for (final option in _dateFormatOptions) {
      final group = (option['group_name'] ?? 'other').trim();
      final pattern = (option['format_pattern'] ?? '').trim();
      if (pattern.isEmpty) {
        continue;
      }
      groupedPatterns.putIfAbsent(group, () => <String>[]).add(pattern);
    }
    for (final entry in groupedPatterns.entries) {
      flatItems.add('__header__${entry.key}');
  ...
  List<SettingsSearchItem> _buildSettingsSearchItems() {
    ...
    items.addAll(<SettingsSearchItem>[
      ...
      SettingsSearchItem(
        ...
        subtitle: 'Profile',
        onSelected: () => _scrollToKey(_organizationAddressKey),
      ),
      SettingsSearchItem(
        group: 'Organization',
        label: 'State',
  ...
  Future<void> _saveProfile() async {
    ...
    if (_selectedLocation == null) {
      ...
      _scrollToKey(_organizationLocationKey);
      return;
    }
    ...
    if (_addressStreet1Controller.text.trim().isEmpty ||
        _addressCityController.text.trim().isEmpty ||
        _addressPincodeController.text.trim().isEmpty) {
      ZerpaiToast.error(
        context,
    ...
    // Resolve org ID — prefer auth user, then API-loaded ID.
    final user = ref.read(authUserProvider);
    ...
    final payload = <String, dynamic>{
      ...
      'attention': _addressAttentionController.text.trim(),
      'street': _addressStreet1Controller.text.trim(),
      ...
      'report_basis': _selectedReportBasis,
