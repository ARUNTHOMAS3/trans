...
class _SettingsBranchCreatePageState
  ...
  List<_StateOption> _stateLookupRows = const [];
  List<_DistrictOption> _districtOptions = const [];
  List<_LocalBodyOption> _allLocalBodyOptions = const [];
  List<_LocalBodyOption> _localBodyOptions = const [];
  ...
  _StateOption? get _selectedStateRow {
    if (_selectedState == null || _selectedState!.isEmpty) return null;
    for (final option in _stateLookupRows) {
      if (option.name.toLowerCase() == _selectedState!.toLowerCase()) {
        return option;
      }
    }
    return null;
  ...
  Future<void> _loadStatesForCountry(String countryName) async {
    ...
    setState(() {
      ...
      _stateOptions = loadedStates.isNotEmpty
          ...
          : (isIndia ? _indianStates : <String>[]);
    });
  }

  Future<void> _loadDistrictsForSelectedState({
    String? preferredDistrictId,
  }) async {
    final stateRow = _selectedStateRow;
    ...
    try {
      ...
      setState(() {
        ...
        if (preferredDistrictId != null &&
          ...
          _selectedDistrictId = preferredDistrictId;
        } else if (_selectedDistrictId != null &&
            districts.any((district) => district.id == _selectedDistrictId)) {
          // keep current
        } else {
          _selectedDistrictId = null;
        }
  ...
  Future<void> _loadLocalBodiesForSelectedDistrict({
    ...
    if (_selectedDistrictId == null || _selectedDistrictId!.isEmpty) {
      if (!mounted) return;
      setState(() {
        _allLocalBodyOptions = const [];
        _localBodyOptions = const [];
        _selectedLocalBodyId = null;
        if (ignoreTypeFilter) _selectedLocalBodyType = null;
      });
      return;
    }
    ...
    try {
      final res = await _apiClient.get(
        ...
        queryParameters: <String, dynamic>{'districtId': _selectedDistrictId},
      );
      if (!mounted) return;
      ...
      setState(() {
        ...
        final availableTypes = _availableLocalBodyTypeOptions;
        if (ignoreTypeFilter ||
            (_selectedLocalBodyType != null &&
                !availableTypes.contains(_selectedLocalBodyType))) {
          _selectedLocalBodyType = null;
        }
        ...
        final filteredLocalBodies =
            ...
            : allLocalBodies
                  .where(
                    (localBody) => localBody.bodyType == _selectedLocalBodyType,
                  )
                  .toList();
        ...
        if (preferredLocalBodyId != null) {
          final preferred = allLocalBodies
              ...
              .firstOrNull;
          if (preferred != null) {
            _selectedLocalBodyId = preferred.id;
            _selectedLocalBodyType = preferred.bodyType;
            return;
          }
        }
        ...
        if (_selectedLocalBodyId != null &&
            filteredLocalBodies.any(
              (localBody) => localBody.id == _selectedLocalBodyId,
            )) {
          return;
        }
  ...
  Future<void> _loadWardsForSelectedLocalBody({String? preferredWardId}) async {
    if (_selectedLocalBodyId == null || _selectedLocalBodyId!.isEmpty) {
      if (!mounted) return;
      setState(() {
        _wardOptions = const [];
    ...
    try {
      final res = await _apiClient.get(
        'lookups/wards',
        queryParameters: <String, dynamic>{'localBodyId': _selectedLocalBodyId},
      );
      if (!mounted) return;
      ...
      setState(() {
        ...
        if (preferredWardId != null &&
          ...
          _selectedWardId = preferredWardId;
        } else if (_selectedWardId != null &&
            wards.any((ward) => ward.id == _selectedWardId)) {
          // keep current
        } else {
          _selectedWardId = null;
        }
  ...
  Future<void> _loadOrgName() async {
    try {
      ...
      if (!_isEditing &&
          orgCountry.toLowerCase() == 'india' &&
          (nextSelectedState ?? _selectedState ?? '').toLowerCase() ==
              'kerala') {
  ...
  Future<void> _loadOrgUsers() async {
    try {
      ...
      if (res.success && res.data is List) {
        setState(
          () => _orgUsers = (res.data as List).cast<Map<String, dynamic>>(),
        );
      }
    } catch (_) {}
  }
  ...
  Future<void> _loadExisting() async {
    ...
    try {
      ...
      if (res.success && res.data is Map<String, dynamic>) {
        ...
        final districtId = (d['district_id'] ?? '').toString().trim();
        final localBodyId = (d['local_body_id'] ?? '').toString().trim();
        final wardId = (d['ward_id'] ?? '').toString().trim();
        ...
        if (_showKeralaLsgdFields) {
          await _loadDistrictsForSelectedState(
            preferredDistrictId: districtId.isNotEmpty ? districtId : null,
          );
          await _loadLocalBodiesForSelectedDistrict(
            preferredLocalBodyId: localBodyId.isNotEmpty ? localBodyId : null,
            ignoreTypeFilter: true,
          );
          await _loadWardsForSelectedLocalBody(
            preferredWardId: wardId.isNotEmpty ? wardId : null,
          );
        }
      }
    } finally {
  ...
  Future<void> _save() async {
    ...
    final districtMissing =
        _showKeralaLsgdFields &&
        (_selectedDistrictId == null || _selectedDistrictId!.isEmpty);
    final localBodyTypeMissing =
        _showKeralaLsgdFields &&
        (_selectedLocalBodyType == null || _selectedLocalBodyType!.isEmpty);
    final localBodyMissing =
        _showKeralaLsgdFields &&
        (_selectedLocalBodyId == null || _selectedLocalBodyId!.isEmpty);

    if (mounted) {
      setState(() {
        _showValidationErrors = true;
        _parentBranchError = parentBranchMissing
            ? 'Parent branch is required'
            : null;
        _districtError = districtMissing ? 'District is required' : null;
        _localBodyTypeError = localBodyTypeMissing
            ? 'Local body type is required'
            : null;
        _localBodyError = localBodyMissing ? 'Local body is required' : null;
      });
    }

    if (hasFormErrors || parentBranchMissing) {
      if (_nameCtrl.text.trim().isEmpty) {
        ZerpaiToast.error(context, 'Please enter a branch name.');
      } else if (parentBranchMissing) {
        ZerpaiToast.error(context, 'Please select a parent branch.');
      } else {
        ZerpaiToast.error(context, 'Please correct the highlighted fields.');
      }
      return;
    }
    if (districtMissing || localBodyTypeMissing || localBodyMissing) {
      if (districtMissing) {
        ZerpaiToast.error(context, 'Please select a district.');
      } else if (localBodyTypeMissing) {
        ZerpaiToast.error(context, 'Please select a local body type.');
      } else {
        ZerpaiToast.error(context, 'Please select a local body.');
      }
      return;
    }
    ...
    try {
      ...
      final nameCheck = await ref
          ...
          .get('branches', queryParameters: {'org_id': orgId});
      if (nameCheck.success && nameCheck.data is List) {
        ...
        final duplicate = (nameCheck.data as List).any((o) {
          final isSelf =
              widget.branchId != null && o['id']?.toString() == widget.branchId;
          return !isSelf &&
      ...
      final body = <String, dynamic>{
        ...
        'local_body_id': _selectedLocalBodyId,
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_subscriptionTotalDays != null &&
                        ...
                        children: [
                          Expanded(
                            child: _buildStaticField(
                              '${_subscriptionTotalDays!} day${_subscriptionTotalDays == 1 ? '' : 's'} total',
                            ),
                          ),
                          const SizedBox(width: AppTheme.space8),
                          Expanded(
                            child: _buildStaticField(
                              '${_subscriptionRemainingDays!} day${_subscriptionRemainingDays == 1 ? '' : 's'} remaining',
                            ),
                          ),
                        ],
                      ),
                  ...
                  ZerpaiFormRow(
                    ...
                    child: FormDropdown<String>(
                      ...
                      displayStringForValue: (id) => _transactionSeries
                          ...
                          .name,
                      onChanged: (v) =>
                          setState(() => _selectedDefaultSeriesId = v),
                    ),
                  ),
  ...
  Widget _buildKeralaLsgdAddressFields() {
    Widget districtField() {
      return FormDropdown<String>(
        items: _districtOptions.map((district) => district.id).toList(),
        value: _selectedDistrictId,
        hint: 'District',
        errorText: _districtError,
        displayStringForValue: (id) {
          final match = _districtOptions.firstWhere(
        ...
        onChanged: (v) async {
          ...
          if (v != null && v.isNotEmpty) {
            await _loadLocalBodiesForSelectedDistrict(ignoreTypeFilter: true);
          }
        },
      );
    }

    Widget localBodyTypeField() {
      return FormDropdown<String>(
        items: _availableLocalBodyTypeOptions,
        value: _selectedLocalBodyType,
        hint: 'Local body type',
        errorText: _localBodyTypeError,
        displayStringForValue: _localBodyTypeLabel,
        onChanged: (v) async {
          if (!mounted) return;
          setState(() {
            _selectedLocalBodyType = v;
            _selectedLocalBodyId = null;
            _selectedWardId = null;
            _localBodyTypeError = null;
            _localBodyError = null;
            _localBodyOptions = v == null || v.isEmpty
    ...
    Widget localBodyField() {
      return FormDropdown<String>(
        items: _localBodyOptions.map((localBody) => localBody.id).toList(),
        value: _selectedLocalBodyId,
        hint: 'Local body name',
        errorText: _localBodyError,
        displayStringForValue: (id) {
          final match = _localBodyOptions.firstWhere(
            (localBody) => localBody.id == id,
            orElse: () => _LocalBodyOption(id: id, name: id, bodyType: ''),
          );
          return match.name;
        },
        onChanged: (v) async {
    ...
    Widget wardField() {
      return FormDropdown<String>(
        items: _wardOptions.map((ward) => ward.id).toList(),
        value: _selectedWardId,
        hint: 'Ward',
        displayStringForValue: (id) {
          final match = _wardOptions.firstWhere(
            (ward) => ward.id == id,
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final useTwoUp = constraints.maxWidth >= 320;
        if (!useTwoUp) {
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: AppTheme.space8),
                Expanded(child: localBodyTypeField()),
              ],
            ),
            const SizedBox(height: AppTheme.space8),
            Row(
              children: [
                Expanded(child: localBodyField()),
                const SizedBox(width: AppTheme.space8),
    ...
    return Column(
      ...
      children: [
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () => setState(() {
                          ...
                          if (_selectedDefaultSeriesId == id)
        ...
        FormDropdown<String>(
          ...
          onChanged: (v) {
            if (v == '__add_new__') {
              _showTransactionSeriesPreferencesDialog();
              return;
            }
  ...
  void _showTransactionSeriesPreferencesDialog() {
    ...
    final prefixCtrls = {
      for (final m in _kSeriesModules) m['id']!: TextEditingController(),
    };
    final startingCtrls = {
                                        ...
                                        child: Row(
                                          children: [
                                            ...
                                            Expanded(
                                              ...
                                              child: TextFormField(
                                                controller:
                                                    prefixCtrls[_kSeriesModules[i]['id']!],
                                            ...
                                            Expanded(
                                              ...
                                              child: TextFormField(
                                                controller:
                                                    startingCtrls[_kSeriesModules[i]['id']!],
                                                decoration: _dec('1'),
                                                keyboardType:
  ...
  void _showManageBusinessTypesDialog() {
    ...
    Future<void> saveType(StateSetter setS, BuildContext ctx) async {
      ...
      setS(() => isSaving = true);
      setState(() {
        _businessTypes.add({'id': id, 'code': code, 'label': label});
      });
      setS(() {
        isSaving = false;
        showForm = false;
