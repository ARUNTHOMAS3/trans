...
class _SettingsWarehouseCreatePageState
  ...
  Future<void> _loadOrgAndBranches() async {
    try {
      ...
      List<String> stateOptions = <String>[];
      ...
      if (orgRes.success && orgRes.data is Map<String, dynamic>) {
        ...
        _organizationName = ((orgData['name'] ?? user?.orgName ?? ''))
            .toString()
            .trim();
        stateOptions = await _fetchStatesForCountryName(orgCountry);
      } else {
  ...
  Future<List<String>> _fetchStatesForCountryName(String countryName) async {
  ...
  InputDecoration _dec(String hint) => InputDecoration(
    ...
    focusedBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(4),
      borderSide: BorderSide(color: ref.watch(appBrandingProvider).accentColor),
    ),
