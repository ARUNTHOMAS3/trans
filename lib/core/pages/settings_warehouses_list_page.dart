...
class _WarehouseRow {
  ...
  factory _WarehouseRow.fromJson(
    ...
    isDefault: j['is_default_for_branch'] as bool? ?? false,
  );
...
class _SettingsWarehousesListPageState
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final List<dynamic> all = res.success && res.data is List
          ...
          : [];
      final List<Map<String, dynamic>> rawWarehouses = all
    ...
    return Container(
      ...
      child: const Row(
        children: [
          ...
          Expanded(flex: 3, child: Text('ASSOCIATED BRANCH', style: style)),
          SizedBox(
            width: 180,
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            SizedBox(
              width: 180,
              child: warehouse.associatedZoneCount > 0
                  ? Align(
                      alignment: Alignment.centerLeft,
                      child: ZerpaiLinkText(
                        text: warehouse.associatedZonesBinsLabel,
  ...
  Widget _buildActionMenu(_WarehouseRow warehouse, bool isHovered) {
    ...
    return MenuAnchor(
      ...
      menuChildren: [
        ...
        buildItem(
          warehouse.isBinLocationsEnabled
              ? 'Disable bin locations'
              : 'Enable bin locations',
          () => warehouse.isBinLocationsEnabled
              ? _openDisableBinLocations(warehouse)
  ...
  Future<void> _openDisableBinLocations(_WarehouseRow warehouse) async {
    await _showTopCenteredWarehouseDialog(
      title: 'Disable Bin Locations',
      body: const Text(
        'Disabling bin locations removes all associated zones and bins for this warehouse. This action is allowed only when every associated bin has zero stock on hand.',
        style: TextStyle(
          fontSize: 14,
          height: 1.55,
          color: AppTheme.textSecondary,
      ...
      onConfirm: () async {
        try {
          ...
          ZerpaiToast.success(
            context,
            'Bin locations disabled for ${warehouse.name}',
          );
          _load();
        } catch (e) {
          if (!mounted) return;
