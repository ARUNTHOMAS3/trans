...
import 'package:zerpai_erp/core/services/sync/sync_service.dart';
...
class _GlobalSyncManagerState extends ConsumerState<GlobalSyncManager> {
      ...
      const SnackBar(
        content: Row(
          children: [
            ...
            SizedBox(width: 8),
            Text('You are offline. Changes will be saved as drafts.'),
          ],
        ),
  ...
  Future<void> _performSync() async {
    ...
    if (failed == 0) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Synced $synced items. Failed to sync $failed items.'),
