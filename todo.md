...
- [ ] **Multi-Tenancy Headers**: Add `X-Org-Id` and `X-Branch-Id` to API requests in `ApiClient`/Dio layer.
- [ ] **Backend Tenant Enforcement**: Re-enable `TenantMiddleware` once auth is wired, and ensure endpoints derive org/branch from token or headers.
...
- [ ] **Audit Trail UI**: Build a dedicated audit/history page later to browse `audit_logs` and `audit_logs_archive`, with filters for table, record, action, org, branch, actor, request ID, and date range.
...
- [ ] `repowiki/en/content/Backend Development/Database Layer & ORM.md:287` Production auth bypass noted; TODO to enable JWT verification and org/branch extraction.
- [ ] `repowiki/en/content/Backend Development/Authentication & Security.md:95` Tenant middleware described; TODO to enable JWT verification and org/branch extraction in production.
...
- [ ] **RBAC Home Dashboard Specification**: Define the home dashboard as a role-aware, permission-aware, user-aware, and branch-aware command center rather than a single shared summary screen.
- [ ] **Dashboard role matrix**: Document exact home widgets by role and scope:
  ...
  - Org Admin: org-wide KPIs, receivables/payables, inventory risk, branch comparison
  - Branch Admin / Manager: branch sales, low stock, expiry alerts, pending operational actions
...
- [ ] **Dashboard branch-context behavior**: Define how the dashboard changes for org-level users vs branch-scoped users, including default branch selection, branch switch persistence, and cross-branch visibility rules.
...
- [ ] **Dashboard backend payload design**: Plan a single aggregated dashboard response shaped by logged-in user role, permissions, org, and branch scope instead of one generic payload for everyone.
...
- [ ] **Branch-aware reorder alerts/reporting**: Add reorder-needed dashboard cards, report filters, and purchase suggestion outputs driven by branch-specific reorder point + reorder rule settings.
- [ ] **Branch-aware reorder rollout completion**: Treat the reorder schema/design rollout as incomplete until the DB migrations (`1006`, `1007`, `1008`) are applied in the active environment and the deferred alerts/reporting layer is implemented.
...
- [ ] **Zones/Bins DB scope shape**: When moving the temporary Zones/Bins runtime store into database tables, do not introduce a generic `branch_id`. Prefer either `branch_id` nullable + `warehouse_id` nullable, or `scope_type` + `scope_id`; current preferred direction is explicit `branch_id` / `warehouse_id`.
