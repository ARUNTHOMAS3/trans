...
## Introduction
This document provides comprehensive API documentation for ZerpAI ERP authentication and authorization endpoints. It focuses on the multi-tenant authentication flow, including the usage of X-Org-Id and X-Outlet-Id headers, JWT token generation and validation, and tenant isolation. It also documents RBAC, permission checking, session management, middleware implementation, token expiration handling, and security considerations. Where applicable, curl examples and integration patterns for frontend applications are included.
...
MW->>MW : "Validate X-Org-Id and X-Outlet-Id"
...
%% AuthHeader --> OrgHeader["Extract X-Org-Id header"]
%% OrgHeader --> OutletHeader["Extract X-Outlet-Id header (optional)"]
%% OutletHeader --> ValidateOrg{"X-Org-Id present?"}
%% ValidateOrg --> |No| Throw401["Throw UnauthorizedException"]
%% ValidateOrg --> |Yes| AttachCtx["Attach TenantContext to request"]
...
- X-Org-Id: Required header identifying the organization context. Requests missing this header receive an unauthorized response.
- X-Outlet-Id: Optional header identifying the specific outlet within an organization. If absent, the tenant context sets outletId to null.

These headers are validated in the middleware and attached to the request’s tenant context.
...
- Recommended approach:
  ...
  - Enforce RBAC in middleware or controllers using the tenant context (orgId/outletId/role).
  - Use database policies (e.g., RLS) to enforce tenant isolation and row-level permissions.

[No sources needed since this section provides recommended implementation guidance]

### Tenant Isolation
- Tenant context is attached to each request with orgId and outletId. Controllers and services can use this context to scope queries and operations to the correct tenant.
...
- Validate and sanitize all headers (X-Org-Id, X-Outlet-Id) server-side.
...
- Missing X-Org-Id header: Ensure clients always send X-Org-Id. The middleware throws an unauthorized error if missing.
...
- Authenticate and fetch tenant context:
  - curl -H "X-Org-Id: ORG_ID" -H "X-Outlet-Id: OUTLET_ID" https://your-domain.com/protected-endpoint
...
- Use X-Org-Id and X-Outlet-Id consistently across all authenticated requests.
