...
## Introduction
This document describes the ZerpAI ERP REST API, covering endpoints for products management, sales operations, customer management, and related lookup synchronization. It also documents the multi-tenant header (x-entity-id), authentication flow, CORS configuration, rate limiting, API versioning, and frontend client usage patterns. Where applicable, diagrams illustrate request flows and component interactions.
...
A_Tenant["tenant.middleware.ts<br/>Tenant Context (x-entity-id)"]
...
- Backend bootstrap and middleware:
  - CORS configuration allows selected origins and methods, including the x-entity-id header.
  - Global validation pipe enforces DTO constraints and logs detailed validation errors.
  - Tenant middleware attaches multi-tenant context to requests via the registry.
- Controllers:
...
- Authentication flow:
  - Development mode: Tenant middleware attaches a test tenant context resolved via the registry.
  - Production-ready code expects the Authorization header and the x-entity-id header.
- Required headers:
  - x-entity-id: Unique identifier for the organization or branch.
- Notes:
  - The current implementation resolves the entity context from the registry table.
...
DevMode --> |No| ProdAuth["Parse Authorization header<br/>Extract x-entity-id"]
ProdAuth --> ValidateOrg{"x-entity-id present?"}
...
- Multi-tenant headers:
  - Always set x-entity-id on requests.
- Authentication:
...
- Get products: curl -H "x-entity-id: YOUR_ENTITY_ID" http://localhost:3001/products
- Create product: curl -X POST -H "x-entity-id: YOUR_ENTITY_ID" -H "Content-Type: application/json" -d '{...}' http://localhost:3001/products
- Sync units: curl -X POST -H "x-entity-id: YOUR_ENTITY_ID" -H "Content-Type: application/json" -d '[{...}]' http://localhost:3001/products/lookups/units/sync
- Get sales orders: curl -H "x-entity-id: YOUR_ENTITY_ID" "http://localhost:3001/sales?type=order"
- Create sales order: curl -X POST -H "x-entity-id: YOUR_ENTITY_ID" -H "Content-Type: application/json" -d '{...}' http://localhost:3001/sales
...
- Tenant context issues:
  - Confirm x-entity-id is set; the middleware resolves the scope.
