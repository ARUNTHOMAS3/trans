...
- NestJS backend
  ...
  - Multi-tenant middleware enforcing org_id/outlet_id context
...
- Backend: NestJS application applies multi-tenant middleware to enforce org_id/outlet_id filtering, exposes product and lookup endpoints, and interacts with Supabase.
...
API --> MW["Tenant Middleware<br/>org_id/outlet_id"]
...
Backend->>DB : "Query with org_id/outlet_id filter"
DB-->>Backend : "Results"
...
- Tenant middleware
  - Extracts org_id and outlet_id from headers
  - Attaches tenant context to requests
  - Skips auth checks in current dev stage (placeholder for JWT)
...
MW-->>API : "Proceed"
API->>DB : "Upsert units with org_id filter"
DB-->>API : "Success"
...
- Context headers
  ...
  - X-Outlet-Id: outlet/branch identifier (optional)
- Middleware behavior
  - TenantMiddleware attaches a tenant context to each request
  - Current dev stage bypasses JWT verification (placeholder for future RBAC)
- Data isolation
  - Backend filters all queries by org_id
  - Database schema includes org_id columns for future RLS enforcement
...
Extract --> Validate{"org_id present?"}
Validate --> |No| Error["Throw Unauthorized"]
...
- Scalability considerations
  - Single database instance supporting hundreds of outlets
