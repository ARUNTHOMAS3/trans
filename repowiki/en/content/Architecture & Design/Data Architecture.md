...
- Tenant middleware: Injects entity context for multi-tenancy via `organisation_branch_master`.
...
- Tenant context injection: The middleware resolves `entityId` from `X-Org-Id`, `X-Branch-Id`, or `X-Entity-Id` headers by looking up `organisation_branch_master`. Controllers access it via `@Tenant()` decorator.
- All business tables have `entity_id uuid NOT NULL` FK to `organisation_branch_master(id)` as the single canonical scope column.
- Global lookup tables (products, categories, tax_rates, currencies, uqc, units, etc.) have no `entity_id` and are shared across all tenants.

```mermaid
flowchart TD
Start(["Incoming Request"]) --> CheckPath["Skip tenant check for health endpoints"]
CheckPath --> InjectCtx["Resolve entityId via organisation_branch_master"]
InjectCtx --> Next["Attach TenantContext; call next middleware/handler"]
