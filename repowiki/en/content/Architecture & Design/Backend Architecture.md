...
- TenantMiddleware injects a tenant context into requests, resolving `entityId` via `organisation_branch_master`.
...
- Extract `x-entity-id` from headers.
- Resolve `entityId` by looking up `organisation_branch_master`.
- Attach `TenantContext` to the request; controllers access it via `@Tenant()` decorator.

Behavior:
- Skips tenant checks for health endpoints.
- Resolves entity from `organisation_branch_master` where `ref_id` matches the org or branch UUID.
- `organisation_branch_master.type` is `'ORG'` or `'BRANCH'`; `ref_id` links to actual `organization.id` or `branches.id`.

```mermaid
flowchart TD
Start(["Request"]) --> PathCheck["Is path '/health' or '/ping'?"]
PathCheck --> |Yes| Next["Call next()"]
PathCheck --> |No| Headers["Read x-entity-id"]
Headers --> Resolve["Lookup organisation_branch_master"]
Resolve --> HasEntity{"Entity resolved?"}
HasEntity --> |No| Throw["Throw Unauthorized"]
HasEntity --> |Yes| Attach["Attach TenantContext"]
Attach --> Next
