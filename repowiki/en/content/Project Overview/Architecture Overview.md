...
The system enforces data isolation using the `entity_id` scoped to the `organisation_branch_master`, supports offline-first workflows with local caching, and synchronizes with the backend for real-time updates.
...
- Tenant middleware resolves the polymorphic `entity_id` from the `x-entity-id` header to enforce multi-tenant boundaries.
...
API["NestJS REST API"]
Tenant["Tenant Middleware<br/>x-entity-id"]
...
- Business tables define an `entity_id` referencing `organisation_branch_master` for multi-tenant isolation.
...
erDiagram
    ...
    CATEGORIES }o--|| ORG_BRANCH_MASTER : "entity_id"
    VENDORS }o--|| ORG_BRANCH_MASTER : "entity_id"
    CUSTOMERS }o--|| ORG_BRANCH_MASTER : "entity_id"
    SALES_ORDERS }o--|| ORG_BRANCH_MASTER : "entity_id"
...
- Tenant context is attached to each request via middleware, extracting `entity_id` from the `x-entity-id` header.
...
- Indexes on `entity_id` improve tenant-scoped queries across the organization hierarchy.
...
- CORS configuration: Ensure allowed origins and headers include `x-entity-id`.
- Validation errors: Review structured error messages returned by ValidationPipe.
- Tenant context: Confirm headers are present and tenant middleware resolves the context.
