...
The system enforces multi-tenancy via X-Org-Id and X-Outlet-Id headers. The frontend communicates with the backend using REST APIs, which apply validation and tenant filtering before querying Supabase.
...
UI->>API : HTTP Request (with X-Org-Id, X-Outlet-Id)
...
- Multi-tenancy filtering not applied
  - Verify X-Org-Id and X-Outlet-Id headers are present on all requests
