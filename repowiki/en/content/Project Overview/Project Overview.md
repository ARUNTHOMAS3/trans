...
- Scalability: multi-tenant architecture supporting HO → FOFO/CO/CO operations across hundreds of branches
...
- Backend (NestJS) applies multi-tenant middleware to enforce the x-entity-id context
...
The database schema defines core entities for products, categories, vendors, and sales, with indexes and placeholders for RLS policies. Seed data demonstrates typical organizations, branches, categories, vendors, and products.
...
- Tenant context missing: ensure the x-entity-id header is present for protected routes
