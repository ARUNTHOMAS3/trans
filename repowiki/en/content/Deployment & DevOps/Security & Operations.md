...
- Tenant middleware that currently bypasses auth in development; intended to enforce org/branch context and role.
- Drizzle schema defining multi-tenant tables with org_id and branch_id scopes.
...
The backend initializes Supabase client, applies CORS and validation, registers tenant middleware globally, and exposes controllers for products and sales. The database schema defines multi-tenant tables with org_id and branch_id, and RLS policies are intentionally disabled in development.
...
- Multi-tenant design: Tables include org_id and branch_id to isolate data per organization and branch.
...
Recommendations:
- Enforce RLS policies per table with org_id and branch_id checks.
...
- Intended design: Tenant context via headers (org_id, branch_id, role) and JWT extraction.
- Controllers: Use req.user for user context when available; schema includes created_by_id/updated_by_id.

Hardening steps:
- Implement JWT guard and strategy; parse org_id/branch_id/role from claims.
...
- Tenant context missing: Ensure org_id/branch_id headers are provided; middleware bypasses auth in dev-mode.
