...
- [PRD/prd_schema.md](file://PRD/prd_schema.md)
- [backend/drizzle/](file://backend/drizzle/)
...
- Multi-tenancy
  - **Polymorphic Tenancy**: Most business tables uses `entity_id` (not `org_id`/`branch_id`) referencing `organisation_branch_master`.
  - **Shared Data**: Global lookups (tax rates, units) have no `entity_id` and are shared.
  - **Scoping**: All queries are scoped via `TenantMiddleware`.
...
- Extensive indexes to optimize frequent queries by type, code, SKU, category, vendor, and entity_id

```mermaid
erDiagram
    ORG_BRANCH_MASTER ||--o{ PRODUCTS : "entity_id"
    UNITS ||--o{ PRODUCTS : "unit_id (Shared)"
    CATEGORIES ||--o{ PRODUCTS : "category_id"
    TAX_RATES ||--o{ PRODUCTS : "tax identifiers (Shared)"
    PRODUCTS ||--o{ PRODUCT_COMPOSITIONS : "product_id"
    ORG_BRANCH_MASTER {
        uuid id PK
        varchar type "ORG|BRANCH"
    }
```
...
- **Entity Identity**: The `organisation_branch_master` table is the hub for all business entities (Orgs, Branches).
- **Uniform Scoping**: Columns named `entity_id` replace legacy `org_id` and `branch_id`.
- **Query Decoration**: Backend services use `@Tenant()` to ensure data security.
...
- Initial seed data scripts insert sample organizations, branchs, categories, vendors, and products
...
- Use entity_id filters to limit scans to tenant scope
...
- Organization products (alternative design): unique(entity_id, item_code), unique(entity_id, product_master_id)
...
- Common tenant fields: entity_id
- Tenant indexes: products(entity_id), categories(entity_id), vendors(entity_id), etc.
