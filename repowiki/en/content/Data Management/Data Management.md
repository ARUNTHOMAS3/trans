...
- Middleware: Tenant middleware injects organization/outlet context for multi-tenancy.
...
- Keys and constraints: UUID primary keys, unique constraints on org-scoped identifiers, foreign keys, and JSON-like fields stored as text.
- Indexes: composite indexes on org_id/outlet_id and selective columns for performance.
...
participant DB as "PostgreSQL"
Client->>MW : Request with X-Org-Id / X-Outlet-Id
MW-->>Ctrl : tenantContext (orgId/outletId/role)
...
- Seed data script: inserts users, categories, vendors, and products; prints org/outlet IDs for testing.
...
- Indexes on org_id/outlet_id, type, category_id, vendor_id, is_selectable, item_code for efficient filtering and joins.
...
- Tenant middleware injects orgId/outletId; production code path is commented and can be enabled.
...
- Use indexes on org_id/outlet_id and frequently filtered columns.
...
- Missing tenant context: Ensure X-Org-Id header is present; middleware injects tenantContext.
