...
TEN["TenantMiddleware<br/>X-Org-Id, X-Branch-Id"]
...
- Tenant Context:
  - TenantMiddleware currently injects test context; ensure proper headers (X-Org-Id, X-Branch-Id) are passed in production.
...
- Use consistent headers: Content-Type: application/json, Authorization (when available), and tenant headers (X-Org-Id, X-Branch-Id).
