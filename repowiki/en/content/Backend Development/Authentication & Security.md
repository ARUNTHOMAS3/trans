...
## Introduction
This document provides comprehensive security documentation for the ZerpAI ERP backend. It focuses on authentication and authorization mechanisms, multi-tenant architecture, organization and outlet-level isolation, tenant context management, row-level security (RLS) policies, data access controls, input validation, sanitization, and protections against common vulnerabilities. It also covers session management, security headers, secure API development practices, data encryption, audit logging, and compliance considerations for Indian business regulations.
...
## Core Components
- Tenant Middleware: Extracts organization and outlet context from headers and attaches a tenant context to requests. Includes a development bypass and placeholders for production JWT verification and role extraction.
...
Ctrl->>DB : "Execute queries with org/outlet filters"
DB-->>Ctrl : "Results"
...
- Extract organization and outlet identifiers from headers
...
- Enforce mandatory presence of X-Org-Id and optional X-Outlet-Id
...
IsHealth --> |No| DevBypass{"Development bypass active?"}
DevBypass --> |Yes| AttachDev["Attach static tenantContext"] --> Next
DevBypass --> |No| JWTCheck["Extract Authorization header"]
...
Parse --> OrgCheck["Extract X-Org-Id & X-Outlet-Id"]
OrgCheck --> HasOrg{"Org present?"}
HasOrg --> |No| ErrOrg["Throw UnauthorizedException"] --> End
HasOrg --> |Yes| AttachProd["Attach tenantContext with userId, role, orgId, outletId"] --> Next
...
Purpose:
- Define tables with organization and outlet identifiers for multi-tenant isolation
- Provide indexes for efficient filtering by org_id and outlet_id
- Support product, category, vendor, and related lookup tables

Security implications:
- Tables include org_id and outlet_id fields to support tenant isolation
- Indexes on org_id and outlet_id improve query performance for filtered access
- Current migration comments indicate RLS was disabled for development
...
- Define precise RLS policies to restrict access to records based on org_id and outlet_id
- Ensure policies apply to authenticated sessions and enforce role-based access
- Use Supabase Auth to tie user sessions to org_id/outlet_id

```mermaid
flowchart TD
Start(["Enable RLS on tables"]) --> Grant["Grant usage & select on tables to authenticated/anon/service_role"]
Grant --> Policy["Create RLS policies:<br/>- Allow access by org_id/outlet_id<br/>- Enforce role-based restrictions"]
...
- Allowed Headers: Authorization, Content-Type, X-Org-Id, X-Outlet-Id
...
- Enforce session validation and refresh tokens via Supabase Auth
...
- Use indexes on org_id and outlet_id for tenant filtering
...
- Missing tenant context: Ensure X-Org-Id header is present; verify middleware path exemptions
