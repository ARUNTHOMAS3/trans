...
- Multi-tenancy is enforced via the x-entity-id header.
...
Browser->>Backend : "HTTP Request with x-entity-id"
Backend->>Backend : "Tenant middleware resolves entity_id via registry"
...
- CORS is enabled for localhost origins and accepts the x-entity-id header.
...
- The backend middleware automatically scopes database queries to the resolved entity_id.
...
- Use indexed columns for frequent filters (e.g., entity_id, item_code).
...
- CORS errors
  ...
  - Ensure the backend allows the x-entity-id header.
...
- x-entity-id: organization or branch identifier
