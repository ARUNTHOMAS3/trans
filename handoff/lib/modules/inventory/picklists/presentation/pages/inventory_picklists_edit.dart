...
class InventoryPicklistsEditScreen extends ConsumerStatefulWidget {
  final String id;

  const InventoryPicklistsEditScreen({super.key, required this.id});

  @override
  ConsumerState<InventoryPicklistsEditScreen> createState() =>
      _InventoryPicklistsEditScreenState();
}

class _InventoryPicklistsEditScreenState
    extends ConsumerState<InventoryPicklistsEditScreen> {
      ...
      _selectedItems.every((item) {
        ...
        if (manualQty > 0 ||
          ...
          for (final b in batches) {
            ...
            batchTotal += double.tryParse(b['foc']?.toString() ?? '0') ?? 0;
          }
          ...
          if (_savedBatchKeys.contains(rowKey)) {
            ...
            if (batches != null && batches.isNotEmpty) {
              ...
              for (final b in batches) {
                ...
                batchTotal +=
                    double.tryParse(b['foc']?.toString() ?? '0') ?? 0;
              }
  ...
  bool _isPickedQtyInvalid(WarehouseStockData item) {
    ...
    if (_savedBatchKeys.contains(rowKey)) {
      ...
      final sumPcsFoc = totalQty + totalFoc;
      final currentPicked = _manualPickedQty[rowKey] ?? _getCurrentPickedQty(item);
      if ((currentPicked - sumPcsFoc).abs() > 0.0001) {
        return true;
      }
    }
    return false;
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            ...
                            style: TextStyle(
                              fontSize: 10,
                              color: item.stock >= _currentQtyToPick(item)
                                  ? _textSecondary
                                  : _dangerRed,
                              height: 1.2,
                            ),
