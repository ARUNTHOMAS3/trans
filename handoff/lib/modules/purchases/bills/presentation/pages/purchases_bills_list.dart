...
class _PurchasesBillsListScreenState
  ...
  Widget build(BuildContext context) {
    ...
    final canCreate = user == null
        ? true
        : PermissionService.hasModuleAction(user, 'bills', action: 'create');
    final canView = user == null
        ? true
        : PermissionService.hasModuleAction(user, 'bills', action: 'view');
    final canEdit = user == null
        ? true
        : PermissionService.hasModuleAction(user, 'bills', action: 'edit');
    final canDelete = user == null
        ? true
        : PermissionService.hasModuleAction(user, 'bills', action: 'delete');
    ...
    return CallbackShortcuts(
      ...
      child: ZerpaiLayout(
        ...
        child: Column(
          ...
          children: [
            _buildTopBanner(state, canCreate, canView),
            _buildFilterBar(state),
            Expanded(child: _buildContent(state, canCreate, canEdit, canDelete)),
          ],
        ),
      ),
    );
  ...
  Widget _buildTopBanner(BillsState state, bool canCreate, bool canView) {
    return Container(
      ...
      child: Row(
        children: [
          ...
          if (canCreate) _buildNewButton(),
          const SizedBox(width: 8),
          _buildMoreActionsButton(canCreate, canView),
        ],
      ),
    );
  }
  ...
  Widget _buildMoreActionsButton(bool canCreate, bool canView) {
    return Container(
      decoration: BoxDecoration(
  ...
  Widget _buildContent(
    ...
    final displayBills = state.isLoading && state.bills.isEmpty
        ? List.generate(
            8,
            (_) => PurchasesBill(
        ...
        : state.bills;
    return Skeletonizer(
      enabled: state.isLoading && state.bills.isEmpty,
      ignoreContainers: true,
      child: _buildTable(displayBills, canEdit, canDelete),
    );
  }

  Widget _buildTable(List<PurchasesBill> bills, bool canEdit, bool canDelete) {
    final fmt = NumberFormat('#,##0.00', 'en_IN');
                ...
                rows: bills.map((bill) {
                  return DataRow(
                    cells: [
                      ...
                      DataCell(Text(bill.placeOfSupply ?? '-')),
