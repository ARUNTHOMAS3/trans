...
class _PREditState extends ConsumerState<PurchasesPurchaseReceivesEditScreen> {
  ...
  Future<void> _showSelectBatchDialog(int itemIndex) async {
    ...
    final inputQty = double.tryParse(_rowControllers[itemIndex].qtyCtrl.text) ?? item.quantityToReceive;
...
class _SelectBatchDialogState extends State<_SelectBatchDialog> {
  ...
  void _validateAndSave() {
    ...
    if (totalEnteredQtyOnly > widget.maxQuantity) {
      setState(
        () => _dialogErrorMessage =
            "Total quantity across all batches cannot exceed the remaining ordered quantity (${_fmtQty(widget.maxQuantity)}). Currently ${_fmtQty(totalEnteredQtyOnly)}.",
      );
      return;
    }
