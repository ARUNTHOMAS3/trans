...
| **Schema snapshots** | `PRD/prd_schema.md` | Update after every schema change          |
...
- All business-owned tables need `org_id uuid NOT NULL`
- Outlet-scoped tables also need `outlet_id uuid`

---

## 🌐 Multi-Tenancy Scoping (CRITICAL)

| Table type        | org_id | outlet_id   | Example                                                                   |
| ----------------- | ------ | ----------- | ------------------------------------------------------------------------- |
| **Global master** | ❌ NO  | ❌ NO       | `products`, `units`, `categories`, `brands`                               |
| **Org-scoped**    | ✅ YES | ❌ Optional | `customers`, `vendors`, `sales_orders`, `accounts`                        |
| **Outlet-scoped** | ✅ YES | ✅ YES      | `outlet_inventory`, `accounts_manual_journals`, `transactional_sequences` |

> 🔴 **The `products` table is GLOBAL. It MUST NOT have `org_id`.**
...
2. `PRD/prd_schema.md` — Context snapshot for AI agents
...
vendors           → display_name (NOT vendor_name), org_id, billing_*/shipping_* address
...
accounts          → parent_id (self-ref tree), account_type, account_group,
                    user_account_name, is_deleted, modified_at, modified_by

accounts_manual_journals → recurring_journal_id (FK → accounts_recurring_journals)

transactional_sequences → module, prefix, suffix, next_number, padding, outlet_id
```
...
// ✅ Self-referential (accounts tree)
...
-- ❌ Missing org_id filter on tenant tables
SELECT * FROM customers  -- Missing WHERE org_id = ?
...
- [ ] All business tables have `org_id` (except global masters)
- [ ] Foreign keys reference correct tables
- [ ] Migration is non-destructive by default
- [ ] `PRD/prd_schema.md` updated with new tables/columns
- [ ] Tables list in `prd_schema.md` updated alphabetically
